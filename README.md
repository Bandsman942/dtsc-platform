...
- Première itération sectorielle santé (`HEALTH_CARE`) dans `Administration [Entreprise]`: patients suivis, rendez-vous et incidents qualité persistés dans `EnterpriseSectorRecord`, avec recherche, détail, formulaire mobile plein écran, modification, archivage, audit et contrôle par `organizationId`.
