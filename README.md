...
- Première itération sectorielle santé (`HEALTH_CARE`) dans `Administration [Entreprise]`: dashboard santé, patients, rendez-vous, consultations, dossiers médicaux, équipe médicale, laboratoire, pharmacie interne, facturation médicale, assurances/prises en charge, incidents qualité, documents médicaux, confidentialité, paramètres et rapports persistés dans `EnterpriseSectorRecord`, avec recherche, détail, formulaire mobile plein écran, actions métier, archivage, audit et contrôle par `organizationId`.
