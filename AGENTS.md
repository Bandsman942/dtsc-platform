...
- Les modules partagés (`Annonces`, `Mes collaborateurs`, `Support`) doivent filtrer par contexte actif: annonces `ORGANIZATION_ONLY` limitées aux membres de l'organisation, groupes internes limités à leur `organizationId`, support client limité au ticket volontairement créé dans le contexte courant.
