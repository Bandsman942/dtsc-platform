...
- Les permissions métier internes doivent utiliser le poste officiel `DtscPosition` relié au dossier `HrcfoEmployee`, pas un texte libre. Les codes `CEO`, `COO`, `HR_CFO`, `SCO`, `CTO` et `MPO` sont centralisés dans `lib/business-roles.ts`.
...
- Les commentaires transversaux SCO/MPO/CTO doivent utiliser `CooComment` avec contrôle d'accès côté serveur; les collaborateurs ne voient que les objets qui les concernent, sauf poste de supervision autorisé.
