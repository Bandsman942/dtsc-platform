...
import { ManualSubscriptionBillingPanel, OperationalInvoiceLedger } from "@/components/admin/manual-subscription-billing-panel";
...
export async function DtscConsolePage({ forcedSection, searchParams }: { forcedSection?: string; searchParams: Promise<ConsoleSearchParams> }) {
  ...
  if (activeSection === "overview") {
    ...
    const [dataset, reconcileDecision, financeInvoiceDecision] = await Promise.all([
    ...
    const financeInvoices = activeSection === "hr-cfo" ? (await getConsoleBillingDataset({ page: 1, pageSize: 10, paymentPage: 1 })).invoiceItems : [];
    content = renderInternalSection(activeSection, internal, canManage, user.locale, ceoStart, ceoEnd, financeInvoices);
  }
...
function renderInternalSection(section: Extract<ConsoleSectionId, "hr-cfo" | "sco" | "coo" | "ceo" | "mpo" | "cto" | "legal">, data: Awaited<ReturnType<typeof getConsoleInternalModulesDataset>>, canManage: boolean, locale: string, ceoStart?: string, ceoEnd?: string, financeInvoices: Array<{ id: string; number: string; invoiceType: string; recipient: string; recipientEmail: string; planName: string; amount: number; currency: string; status: string; issuedAt: string; paidAt: string | null; emailSentAt: string | null; transactionTitle: string | null; transactionCategory: string | null }> = []) {
  if (section === "hr-cfo") return <div className="space-y-5"><OperationalInvoiceLedger invoices={financeInvoices} /><PayrollWorkflowPanel locale={locale} /><OperationsAdminPanel eyebrow="Gestion interne" title="Opérations HR & CFO" description="Dossiers RH, budgets, dépenses, comptes et contrôles depuis les moteurs canoniques." playbook={["Dossier RH", "Budget", "Dépense", "Validation", "Paiement", "Audit"]} datasets={data.hrcfoDatasets.filter((item) => item.id !== "payrolls")} canEdit={canManage} /></div>;
