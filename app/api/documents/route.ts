...
import { indexPreparedKnowledgeDocument, knowledgeUploadLimits, prepareKnowledgeDocument } from "@/lib/rag";
...
export async function GET() {
  ...
  const documents = await prisma.knowledgeDocument.findMany({
    where: { userId: session.userId, organizationId }, orderBy: { createdAt: "desc" },
    include: { _count: { select: { chunks: true } } }, take: 100,
  });
...
export async function POST(req: Request) {
  ...
  const activeSubscription = await prisma.subscription.findFirst({
    where: { userId: session.userId, status: SubscriptionStatus.ACTIVE }, orderBy: { createdAt: "desc" }, include: { plan: true },
  });
  ...
  try {
    const prepared = await prepareKnowledgeDocument({
      userId: session.userId, organizationId, title: body.data.title || undefined,
      language: user?.locale === "en" ? "en" : "fr", file,
    });
    ...
    after(async () => {
      try {
        ...
        await writeAuditLog({ userId, action: "KNOWLEDGE_DOCUMENT_INDEXED", entity: "KnowledgeDocument", entityId: documentId, metadata: { chunks: indexed._count.chunks, indexVersion: prepared.indexVersion } });
      } catch (error) {
        console.error("Personal knowledge background indexing failed", documentId, error);
      }
    });
    await writeAuditLog({ userId: session.userId, action: "KNOWLEDGE_DOCUMENT_PREPARED", entity: "KnowledgeDocument", entityId: documentId, metadata: { indexVersion: prepared.indexVersion }, request: req });
    await writeApiLog({ request: req, statusCode: 202, userId: session.userId, startedAt, metadata: { documentId, status: "PROCESSING" } });
    return NextResponse.json({ ok: true, document: { id: documentId, title: prepared.title, status: "PROCESSING", indexVersion: prepared.indexVersion } }, { status: 202 });
  } catch (error) {
