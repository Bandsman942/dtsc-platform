...
export async function POST(req: Request) {
  ...
  if (!isSameOriginRequest(req)) {
    await writeApiLog({
      ...
      statusCode: 403,
      startedAt,
      metadata: { action: "enterprise_ai_chat_origin_denied" },
    });
  ...
  const limited = await rateLimit(
    getRateLimitKey(req, `enterprise-ai-chat:${session.userId}`),
    80,
    60 * 60 * 1000
  );
  if (!limited.ok) {
  ...
  if (!quota.ok) {
    return NextResponse.json(
      { error: "MONTHLY_LIMIT_REACHED", reasonCode: "MONTHLY_LIMIT_REACHED", usage: quota.snapshot },
      { status: 429 }
    );
  }
  ...
  try {
    ...
    if (!conversation) {
      return NextResponse.json(
        { error: "CONVERSATION_NOT_FOUND", reasonCode: "CONVERSATION_NOT_FOUND" },
        { status: 404 }
      );
    }
    ...
    const [knowledge, toolResults] = await Promise.all([
      ...
      useTools && access.canUseReadTools && access.sectorCode === "PHARMACY"
        ? runPharmacyReadTools(data.organizationId, data.content)
        : Promise.resolve([]),
    ]);
    const routeDataClassifications = Array.from(
      new Set([...preparedTurn.routePolicy.dataClassifications, ...knowledge.dataClassifications])
    );
    ...
    if (toolResults.length) {
      await prisma.enterpriseAiToolCall.createMany({
        data: toolResults.map((result) => ({
          ...
          toolType: "READ",
          status: "SUCCESS",
          inputJson: jsonValue({ question: data.content, locale }),
    ...
    async function persistAssistant(result: AiStreamConsumption, completed: boolean) {
      ...
      if (result.content.trim()) {
        writes.push(
          ...
          prisma.enterpriseAiConversation.update({
            ...
            data: {
              lastMessageAt: new Date(),
              title: conversationTitle === "Nouvelle conversation" ? data.content.slice(0, 90) : undefined,
            },
          })
        );
      }
      writes.push(
        recordEnterpriseAiUsage({
          organizationId: data.organizationId,
          assistantId: authorizedAccess.assistantId,
          conversationId,
          userId: sessionUserId,
          inputTokens,
          outputTokens,
        ...
        writeAuditLog({
          userId: sessionUserId,
          action: completed ? "ENTERPRISE_AI_CHAT_COMPLETED" : "ENTERPRISE_AI_CHAT_INTERRUPTED",
    ...
    return NextResponse.json(
      {
        ...
        message: getAiErrorMessage("UNKNOWN_PROVIDER_ERROR", locale),
      },
      { status: 500 }
    );
  }
