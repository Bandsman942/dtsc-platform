...
import {
  enterpriseAiKnowledgeListSchema,
  enterpriseAiKnowledgeUploadSchema,
} from "@/lib/enterprise-ai/validators";
...
export async function POST(req: Request) {
  ...
  if (!isSameOriginRequest(req)) {
    await writeApiLog({
      ...
      statusCode: 403,
      startedAt,
      metadata: { action: "enterprise_ai_source_origin_denied" },
    });
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  const session = await getSession();
  ...
  const limited = await rateLimit(
    getRateLimitKey(req, `enterprise-ai-source:${session.userId}`),
    20,
    60 * 60 * 1000
  );
  if (!limited.ok) {
  ...
  if (!access || !access.canUploadSources) {
    await writeApiLog({ request: req, statusCode: 403, userId: session.userId, startedAt });
    return NextResponse.json(
      { error: "Forbidden", message: "Vous n'avez pas la permission d'ajouter des sources IA." },
      { status: 403 }
    );
  }
  if (!canIndexEnterpriseAiFile(file)) {
    ...
    return NextResponse.json(
      { error: "Unsupported file", message: "Fichier non supporté ou trop volumineux." },
      { status: 415 }
    );
  }
  ...
  if (!quota.ok) {
    await writeApiLog({
      ...
      userId: session.userId,
      startedAt,
      metadata: { organizationId: parsed.data.organizationId, code: quota.code },
    });
    return NextResponse.json(
      {
        error: quota.code,
        message:
          quota.code === "SOURCE_LIMIT_REACHED"
            ? "Limite de sources IA atteinte."
            : "Limite de stockage IA atteinte.",
      },
      { status: 429 }
    );
  }
  ...
  try {
    ...
    after(async () => {
      try {
        ...
        await writeAuditLog({
          userId,
          action: "ENTERPRISE_AI_SOURCE_INDEXED",
    ...
    await writeAuditLog({
      userId: session.userId,
      action: "ENTERPRISE_AI_SOURCE_PREPARED",
      entity: "EnterpriseAiKnowledgeSource",
      entityId: sourceId,
      request: req,
      metadata: {
    ...
    await writeApiLog({
      ...
      userId: session.userId,
      startedAt,
      metadata: { organizationId, sourceId, status: "PROCESSING" },
    });
    return NextResponse.json(
      {
        ok: true,
        source: { id: sourceId, title: prepared.title, status: "PROCESSING", chunkCount: 0 },
      },
      { status: 202 }
    );
  } catch (error) {
