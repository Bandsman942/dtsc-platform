...
export async function POST(req: Request, { params }: Params) {
  ...
  const limited = await rateLimit(getRateLimitKey(req, `enterprise-expense-action:${session.userId}`), 120, 60 * 60 * 1000);
  ...
  try {
    const result = await transitionEnterpriseExpense(organizationId, id, session.userId, parsed.data);
    if (parsed.data.action === "SUBMIT" && parsed.data.approverUserId) await notifyUser({ userId: parsed.data.approverUserId, organizationId, type: "ENTERPRISE_APPROVAL", title: "Dépense à valider", body: "Une dépense nécessite votre décision.", targetUrl: "/enterprise-modules/VALIDATIONS" });
    await writeAuditLog({ userId: session.userId, action: `ENTERPRISE_EXPENSE_${parsed.data.action}`, entity: "EnterpriseExpense", entityId: id, request: req, metadata: { organizationId, fromStatus: current.status } });
    await writeApiLog({ request: req, statusCode: 200, userId: session.userId, startedAt, metadata: { organizationId, domain: "expenses", expenseId: id, action: parsed.data.action } });
    return NextResponse.json({ ok: true, result });
  } catch (error) {
    const normalized = normalizeEnterpriseCoreV2Error(error);
    await writeApiLog({ request: req, statusCode: normalized.status, userId: session.userId, startedAt, metadata: { organizationId, domain: "expenses", expenseId: id, action: parsed.data.action, error: normalized.code } });
