...
export async function POST(req: Request, { params }: Params) {
  ...
  if (!parsed.success) return enterpriseValidationErrorResponse(parsed.error, "DOCUMENT_ACCESS_INPUT_INVALID", req);
  try {
...
export async function DELETE(req: Request, { params }: Params) {
  ...
  if ("response" in context) return context.response;
  const parsed = enterpriseDocumentAccessSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) return enterpriseValidationErrorResponse(parsed.error, "DOCUMENT_ACCESS_REVOKE_INPUT_INVALID", req);
