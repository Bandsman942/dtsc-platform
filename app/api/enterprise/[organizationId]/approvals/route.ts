...
export async function GET(req: Request, { params }: Params) {
  ...
  const explicitStatus = url.searchParams.get("status")?.trim() || "";
  const targetEntityType = url.searchParams.get("entityType")?.trim() || "";
  ...
  if (explicitStatus) filters.push({ status: explicitStatus });
  if (targetEntityType) filters.push({ targetEntityType });
  if (from || to) {
    const requestedAt: { gte?: Date; lte?: Date } = {};
    if (from) requestedAt.gte = new Date(from);
    if (to) requestedAt.lte = new Date(to);
    filters.push({ requestedAt });
  }
  const matchingTargets = search ? await targetSummaries(organizationId, search) : new Map<string, ApprovalTargetSummary>();
  if (search) {
    const targetPairs = [...matchingTargets.values()].map((target) => ({ targetEntityType: target.type, targetEntityId: target.id }));
    filters.push({ OR: [{ decisionComment: { contains: search, mode: "insensitive" } }, ...targetPairs] });
  }
  ...
  const [items, total] = await Promise.all([
    prisma.enterpriseApproval.findMany({ where, orderBy: { requestedAt: "desc" }, skip: (page - 1) * pageSize, take: pageSize }),
    prisma.enterpriseApproval.count({ where }),
  ]);
  const allTargetMap = search ? matchingTargets : await resolveTargets(organizationId, items);
  const enriched = items.map((item) => ({ ...item, target: allTargetMap.get(`${item.targetEntityType}:${item.targetEntityId}`) || null }));
...
export async function POST(req: Request, { params }: Params) {
  ...
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const limited = await rateLimit(getRateLimitKey(req, `enterprise-approvals:${session.userId}`), 100, 60 * 60 * 1000);
  ...
  const parsed = enterpriseApprovalCreateSchema.safeParse(await req.json().catch(() => null));
  ...
  try {
    ...
    await writeApiLog({ request: req, statusCode: normalized.status, userId: session.userId, startedAt, metadata: { organizationId, domain: "approvals", error: normalized.code } });
