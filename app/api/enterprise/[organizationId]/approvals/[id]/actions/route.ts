...
import { applyProfessionalApprovalAction, ApprovalCoordinationError, ensureApprovalSubmissionVersion, recordApprovalDecision } from "@/lib/standard-work-coordination/approval-coordination";
...
const professionalApprovalActionSchema = z.object({ action: z.enum(["REQUEST_CORRECTION", "RESUBMIT", "DELEGATE"]), revision: z.coerce.number().int().min(1), decisionComment: z.string().trim().max(3000).optional(), delegateUserId: z.string().trim().max(160).optional(), idempotencyKey: z.string().trim().max(240).optional() });
const prepareReviewSchema = z.object({ action: z.literal("PREPARE_REVIEW"), revision: z.coerce.number().int().min(1) });
...
async function targetRevision(organizationId: string, approval: CurrentApproval) {
  ...
  if (approval.targetEntityType === "EnterprisePayrollRun") return (await prisma.enterprisePayrollRun.findFirst({ where: { id: approval.targetEntityId, organizationId, archivedAt: null }, select: { revision: true } }))?.revision ?? null;
...
async function decideDomainApproval(organizationId: string, current: CurrentApproval, actorUserId: string, data: { action: "APPROVE" | "REJECT" | "CANCEL"; revision: number; decisionComment?: string }, canManage: boolean) {
  ...
  if (current.targetEntityType === "EnterprisePayrollRun") return decideEnterprisePayrollRun(organizationId, current.targetEntityId, actorUserId, { decision: data.action, revision: revision!, comment: data.decisionComment });
...
export async function POST(req: Request, { params }: Params) {
  ...
  const limited = await rateLimit(getRateLimitKey(req, `enterprise-approval-action:${session.userId}`), 120, 60 * 60 * 1000); if (!limited.ok) return NextResponse.json({ error: "RATE_LIMITED" }, { status: 429 });
  const { organizationId, id } = await params;
  const access = await getEnterpriseCoreV2Access({ session, organizationId, moduleCode: "VALIDATIONS", action: "submit" }); if (!access) return NextResponse.json({ error: "FORBIDDEN" }, { status: 403 });
  const payload = await req.json().catch(() => null); const action = typeof payload?.action === "string" ? payload.action : ""; const prepareReview = action === "PREPARE_REVIEW"; const professional = ["REQUEST_CORRECTION", "RESUBMIT", "DELEGATE"].includes(action);
  const parsed = prepareReview ? prepareReviewSchema.safeParse(payload) : professional ? professionalApprovalActionSchema.safeParse(payload) : enterpriseApprovalActionSchema.safeParse(payload);
  ...
  const current = await prisma.enterpriseApproval.findFirst({ where: { id, organizationId, archivedAt: null } }); if (!current) return NextResponse.json({ error: "NOT_FOUND" }, { status: 404 });
  ...
  try {
    ...
    if (professional) {
      ...
      const result = await applyProfessionalApprovalAction({ organizationId, approvalId: id, actorUserId: session.userId, canManage: access.canManage, action: data.action, reason: data.decisionComment, delegateUserId: data.delegateUserId, revision: data.revision });
      ...
      if (recipientId !== session.userId) await notifyUser({ userId: recipientId, organizationId, type: "ENTERPRISE_APPROVAL", title: data.action === "REQUEST_CORRECTION" ? "Correction demandée" : data.action === "DELEGATE" ? "Validation déléguée" : "Correction soumise", body: data.decisionComment || "Une action est attendue sur cette validation.", targetUrl: workCoordinationDeepLink("APPROVAL", id) });
    ...
    if (error instanceof ApprovalCoordinationError) { await writeApiLog({ request: req, statusCode: error.status, userId: session.userId, startedAt, metadata: { organizationId, domain: "approvals", approvalId: id, action, error: error.code } }); return NextResponse.json({ error: error.code, message: error.message }, { status: error.status }); }
    if (error instanceof EnterpriseDomainError || error instanceof EnterpriseAccountingError) { await writeApiLog({ request: req, statusCode: error.status, userId: session.userId, startedAt, metadata: { organizationId, domain: "approvals", approvalId: id, action, error: error.code } }); return NextResponse.json({ error: error.code, message: error.message }, { status: error.status }); }
