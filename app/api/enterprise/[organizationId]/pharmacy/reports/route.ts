...
export async function POST(request: Request, { params }: Params) {
  ...
  const parsed = pharmacySavedReportSchema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return enterpriseValidationErrorResponse(parsed.error, "PHARMACY_REPORT_INPUT_INVALID", request);
