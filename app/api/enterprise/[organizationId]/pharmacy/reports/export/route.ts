...
export async function POST(request: Request, { params }: Params) {
  ...
  const { organizationId } = await params; const parsed = pharmacyReportExportSchema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return NextResponse.json({ error: "ENTERPRISE_INPUT_INVALID", message: "Demande d'export invalide." }, { status: 400 });
  const sensitiveType = ["CASH_PAYMENTS", "PRESCRIPTIONS", "QUALITY_INCIDENTS", "DOCUMENTS_COMPLIANCE"].includes(parsed.data.reportType); const reportSettings = (await getEffectivePharmacySettings(organizationId, session.userId)).sections["reports-exports"]; if (sensitiveType && !reportSettings.allowSensitiveReportExports) return NextResponse.json({ error: "Sensitive export disabled", message: "Les exports sensibles sont désactivés dans les paramètres pharmacie." }, { status: 403 }); const action = sensitiveType ? "export_sensitive" : "export"; if (!(await canAccessPharmacyReports(session.userId, organizationId, action))) return NextResponse.json({ error: "Forbidden", message: "Vous n'avez pas la permission d'exporter ce rapport." }, { status: 403 });
