...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "ENTERPRISE_INPUT_INVALID", message: parsed.error.issues[0]?.message || "Données de caisse invalides." }, { status: 400 });
  ...
  try {
    ...
    const messages: Record<string, string> = { CASHIER_NOT_FOUND: "Le caissier n'appartient pas à cette pharmacie.", DEPARTMENT_NOT_FOUND: "Le département n'appartient pas à cette pharmacie.", CASHIER_ALREADY_OPEN: "Ce caissier possède déjà une session ouverte.", SALE_NOT_FOUND: "La vente sélectionnée est introuvable.", INVOICE_NOT_FOUND: "La facture sélectionnée est introuvable.", SALE_INVOICE_MISMATCH: "La facture et la vente sélectionnées ne correspondent pas.", PAYMENT_CURRENCY_MISMATCH: "La devise du paiement doit correspondre à la devise de la vente.", PAYMENT_NOT_FOUND: "Le paiement sélectionné est introuvable.", SESSION_REQUIRED: "Une session ouverte est obligatoire pour cet encaissement.", SESSION_NOT_OPEN: "La session de caisse sélectionnée n'est pas ouverte.", PAYMENT_EXCEEDS_REMAINING: "Le paiement dépasse le reste à payer.", REFUND_EXCEEDS_PAID: "Le remboursement dépasse le montant réellement payé.", RESTOCK_REQUIRES_FULL_REFUND: "La remise en stock complète exige un remboursement total de la vente." };
