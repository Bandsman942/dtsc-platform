...
import { createEnterpriseOperationalObject } from "@/lib/enterprise/core-v2/dispatcher";
...
export async function GET(request: Request, { params }: Params) {
  ...
  try {
    const dataset = await getPharmacyActivityDashboard(organizationId, session.userId);
...
export async function POST(request: Request, { params }: Params) {
  ...
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const limited = await rateLimit(getRateLimitKey(request, `pharmacy-activities:${session.userId}`), 80, 3600000);
  ...
  try {
    ...
    await createEnterpriseOperationalObject({
      organizationId,
      ...
      data: {
        ...
        sectorCode: "PHARMACY",
        metadata: { activityType: parsed.data.activityType, resultEntityType: activity.resultEntityType, resultEntityId: activity.resultEntityId, requestType: parsed.data.activityType },
      },
    });
    await writeAuditLog({ userId: session.userId, action: "PHARMACY_ACTIVITY_CREATED", entity: "PharmacyActivityItem", entityId: activity.id, request, metadata: { organizationId, activityType: parsed.data.activityType, resultEntityId: activity.resultEntityId, erpCoreV2Dispatch: true } });
