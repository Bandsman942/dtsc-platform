...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const parsed = pharmacyActivityActionSchema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return enterpriseValidationErrorResponse(parsed.error, "PHARMACY_ACTIVITY_INPUT_INVALID", request);
  try {
