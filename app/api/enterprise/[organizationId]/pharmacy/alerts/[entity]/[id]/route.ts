...
export async function PATCH(request: Request, { params }: Params) {
  ...
  } catch (error) { const code = error instanceof Error && /^[A-Z][A-Z0-9_]+$/.test(error.message) ? error.message : "UNKNOWN"; return NextResponse.json({ error: code, message: code === "COMMENT_REQUIRED" ? "Un motif ou commentaire est obligatoire." : code === "ASSIGNEE_INVALID" ? "Le responsable sélectionné n'appartient pas à cette organisation." : "Action impossible." }, { status: 400 }); }
}
