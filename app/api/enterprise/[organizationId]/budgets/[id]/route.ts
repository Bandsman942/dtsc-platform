...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const parsed = enterpriseBudgetUpdateSchema.safeParse(await req.json().catch(() => null)); if (!parsed.success) return NextResponse.json({ error: "ENTERPRISE_INPUT_INVALID", message: parsed.error.issues[0]?.message || "Budget invalide." }, { status: 400 });
