...
function permitted(moduleCode: SupportedModule, kind: ReferenceKind) {
  if (moduleCode === "FINANCE_RECEIVABLES") return ["customer", "sales-order", "fulfillment", "contract", "project"].includes(kind);
  if (moduleCode === "FINANCE_PAYABLES") return ["supplier", "purchase", "purchase-receipt", "project", "expense-account"].includes(kind);
  ...
  if (kind === "customer") {
    items = await prisma.enterpriseBusinessParty.findMany({
      where: {
        ...
        ...(search ? { OR: [{ code: { contains: search, mode: "insensitive" } }, { legalName: { contains: search, mode: "insensitive" } }, { displayName: { contains: search, mode: "insensitive" } }] } : {}),
      },
    ...
    const suppliers = await prisma.enterpriseSupplier.findMany({
      where: {
        ...
        archivedAt: null,
        ...(search ? { OR: [{ legalName: { contains: search, mode: "insensitive" } }, { displayName: { contains: search, mode: "insensitive" } }] } : {}),
      },
      orderBy: [{ displayName: "asc" }, { legalName: "asc" }],
      take,
      select: { id: true, legalName: true, displayName: true },
    });
    const links = suppliers.length ? await prisma.enterpriseSupplierPartyLink.findMany({
      where: { organizationId, supplierId: { in: suppliers.map((supplier) => supplier.id) }, archivedAt: null },
    ...
    items = await prisma.enterpriseSalesOrder.findMany({
      where: {
        ...
        ...(parentId ? { businessPartyId: parentId } : {}),
        ...(search ? { OR: [{ reference: { contains: search, mode: "insensitive" } }, { title: { contains: search, mode: "insensitive" } }] } : {}),
      },
    ...
    items = await prisma.enterpriseFulfillment.findMany({
      where: {
        ...
        ...(parentId ? { salesOrderId: parentId } : {}),
    ...
    items = await prisma.enterprisePayrollRun.findMany({
      where: {
        ...
        archivedAt: null,
        status: "APPROVED",
        ...(search ? { OR: [{ reference: { contains: search, mode: "insensitive" } }, { payrollPeriod: { OR: [{ code: { contains: search, mode: "insensitive" } }, { name: { contains: search, mode: "insensitive" } }] } }] } : {}),
      },
    ...
    items = await prisma.enterpriseEmployee.findMany({
      where: {
        ...
        archivedAt: null,
        ...(search ? { OR: [{ employeeNumber: { contains: search, mode: "insensitive" } }, { displayName: { contains: search, mode: "insensitive" } }, { workEmail: { contains: search, mode: "insensitive" } }] } : {}),
      },
