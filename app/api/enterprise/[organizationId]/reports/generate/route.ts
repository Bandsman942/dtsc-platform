...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const report = await generateEnterpriseReport(organizationId, session.userId, parsed.data);
    await writeAuditLog({ userId: session.userId, action: "ENTERPRISE_REPORT_GENERATED", entity: "EnterpriseReport", entityId: report.id, request: req, metadata: { organizationId, reportType: report.reportType, schemaVersion: report.schemaVersion } });
    await writeApiLog({ request: req, statusCode: 201, userId: session.userId, startedAt, metadata: { organizationId, domain: "reports", reportType: report.reportType } });
