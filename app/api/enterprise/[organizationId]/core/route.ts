...
export async function GET(req: Request, { params }: Params) {
  ...
  await writeApiLog({ request: req, statusCode: 200, userId: session.userId, startedAt, metadata: { organizationId, moduleCode, legacyCompatibility: isDedicatedCoreDomain(moduleCode) } });
  return NextResponse.json({ records, canManage: access.canManage, legacyReadOnly: isDedicatedCoreDomain(moduleCode) });
}
...
export async function POST(req: Request, { params }: Params) {
  ...
  if (isDedicatedCoreDomain(data.moduleCode, data.recordType)) {
    return NextResponse.json({ error: "Dedicated domain required", message: "Ce métier utilise désormais son espace ERP dédié. Les anciens objets restent consultables en historique." }, { status: 409 });
  }
  ...
  if (relatedUserIds.length) {
    const memberCount = await prisma.organizationMember.count({ where: { organizationId, userId: { in: relatedUserIds }, status: "ACTIVE", removedAt: null } });
    if (memberCount !== new Set(relatedUserIds).size) return NextResponse.json({ error: "Invalid collaborator", message: "Les responsables doivent être des collaborateurs actifs de cette entreprise." }, { status: 400 });
  }
  ...
  await writeAuditLog({ userId: session.userId, action: "ENTERPRISE_CORE_RECORD_CREATED", entity: "EnterpriseCoreRecord", entityId: record.id, request: req, metadata: { organizationId, moduleCode: data.moduleCode, recordType: data.recordType, legacyDomain: true } });
