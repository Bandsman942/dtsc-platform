...
import { payrollRunDecisionSchema } from "@/lib/enterprise/hr-payroll/schemas";
...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const payrollRun = await decideEnterprisePayrollRun(organizationId, payrollRunId, session.userId, parsed.data);
    await writeAuditLog({ userId: session.userId, action: `ENTERPRISE_PAYROLL_RUN_${parsed.data.decision}`, entity: "EnterprisePayrollRun", entityId: payrollRun.id, request: req, metadata: { organizationId, paymentCreated: false } });
    await writeApiLog({ request: req, statusCode: 200, userId: session.userId, startedAt, metadata: { organizationId, domain: "payroll-runs", action: parsed.data.decision } });
    return NextResponse.json({ ok: true, payrollRun });
  } catch (error) {
    return enterpriseDomainErrorResponse(error, "PAYROLL_RUN_DECISION_FAILED");
  }
}
