...
export async function GET(req: Request, { params }: Params) {
  ...
  const [items, total, openAmount, overdueAmount] = await Promise.all([
    prisma.enterprisePayable.findMany({ where, orderBy: [{ dueDate: "asc" }, { createdAt: "desc" }], skip: (page - 1) * pageSize, take: pageSize, include: { supplierInvoice: { include: { threeWayMatch: true } }, _count: { select: { paymentAllocations: true, allocations: true } } } }),
    ...
    prisma.enterprisePayable.aggregate({ where: { organizationId, status: "OPEN", dueDate: { lt: startOfToday() } }, _sum: { outstandingAmount: true } }),
  ]);
