...
export async function POST(req: Request, { params }: Params) {
  ...
  const parsed = enterpriseSupplierContactCreateSchema.safeParse(await req.json().catch(() => null)); if (!parsed.success) return enterpriseValidationErrorResponse(parsed.error, "SUPPLIER_CONTACT_INPUT_INVALID", req);
  try {
