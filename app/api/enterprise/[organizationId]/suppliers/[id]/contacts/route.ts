...
export async function POST(req: Request, { params }: Params) {
  ...
  const parsed = enterpriseSupplierContactCreateSchema.safeParse(await req.json().catch(() => null)); if (!p.success) return enterpriseValidationErrorResponse(p.error, "SUPPLIER_CONTACT_INPUT_INVALID", req);
  try {
