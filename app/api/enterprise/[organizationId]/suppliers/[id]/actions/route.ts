...
export async function POST(req: Request, { params }: Params) {
  ...
  const parsed = enterpriseSupplierActionSchema.safeParse(await req.json().catch(() => null)); if (!p.success) return enterpriseValidationErrorResponse(p.error, "SUPPLIER_ACTION_INPUT_INVALID", req);
  try {
