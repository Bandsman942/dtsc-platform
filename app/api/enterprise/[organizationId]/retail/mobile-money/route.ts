...
export async function GET(req: Request, { params }: Params) {
  ...
  const masked = items.map((item) => ({ ...item, customerPhone: item.customerPhone.length > 6 ? `${item.customerPhone.slice(0, 3)}••••${item.customerPhone.slice(-3)}` : item.customerPhone }));
...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const result = await createMobileMoneyTransaction(organizationId, auth.session.userId, parsed.data);
