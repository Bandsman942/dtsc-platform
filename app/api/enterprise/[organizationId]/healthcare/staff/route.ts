...
export async function POST(req:Request,{params}:Params){
  ...
  const parsed=healthStaffCreateSchema.safeParse(payload);if(!parsed.success)return NextResponse.json({error:"ENTERPRISE_INPUT_INVALID",message:"Vérifiez le membre, le poste et le service Santé."},{status:400});
