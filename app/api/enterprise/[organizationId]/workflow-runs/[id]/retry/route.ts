...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const run = await retryWorkflowStep(id, session.userId, parsed.data.reason); if (run.organizationId !== organizationId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
