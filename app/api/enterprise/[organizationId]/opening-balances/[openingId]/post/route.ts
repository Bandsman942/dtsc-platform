...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    ...
    await writeApiLog({ request: req, statusCode: 200, userId: auth.session.userId, startedAt, metadata: { organizationId, domain: "opening-balances", action: "post" } });
    return NextResponse.json({ ok: true, opening });
  } catch (error) {
    return financeErrorResponse(error, "OPENING_BALANCE_POST_FAILED");
  }
}
