...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    const next = await tx.billingPlan.update({ where: { id }, data: { audience: parsed.data.audience, name: parsed.data.name, description: parsed.data.description, priceUsd: parsed.data.priceUsd, dailyMessageLimit: parsed.data.dailyMessageLimit, dailyTokenLimit: parsed.data.dailyTokenLimit, maxDocuments: parsed.data.maxDocuments, sortOrder: parsed.data.sortOrder, isActive: parsed.data.isActive } });
  ...
  await writeAuditLog({ userId: access.session.userId, action: "BILLING_PLAN_UPDATED", entity: "BillingPlan", entityId: updated.id, before: { audience: current.audience, name: current.name, priceUsd: Number(current.priceUsd), dailyMessageLimit: current.dailyMessageLimit, dailyTokenLimit: current.dailyTokenLimit, maxDocuments: current.maxDocuments, sortOrder: current.sortOrder, isActive: current.isActive }, after: { audience: updated.audience, name: updated.name, priceUsd: Number(updated.priceUsd), dailyMessageLimit: updated.dailyMessageLimit, dailyTokenLimit: updated.dailyTokenLimit, maxDocuments: updated.maxDocuments, sortOrder: updated.sortOrder, isActive: updated.isActive }, reasonCode: access.reasonCode, riskLevel: "HIGH", metadata: { reason: parsed.data.reason, slug: current.slug, effectiveAt: effectiveAt.toISOString(), versioned: true }, request: req });
