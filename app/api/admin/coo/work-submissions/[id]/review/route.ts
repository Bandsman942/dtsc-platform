...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isSameOriginRequest(req)) {
    await writeApiLog({ request: req, statusCode: 403, startedAt, metadata: { action: "coo_work_review_origin_denied" } });
  ...
  const limited = await rateLimit(getRateLimitKey(req, `coo-work-review:${session.userId}`), 80, 60 * 60 * 1000);
  if (!limited.ok) {
  ...
  try {
    const submission = await reviewWorkSubmission({ actor, submissionId: id, action: parsed.data.action, comment: parsed.data.comment, expectedReviewerCode: "COO" });
    ...
    await writeApiLog({ request: req, statusCode: status, userId: session.userId, startedAt, metadata: { action: "coo_work_review_failed", code: isWorkPrestationError(error) ? error.code : "UNKNOWN" } });
