...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const submission = await getWorkSubmissionReviewDetail(actor, id, "COO");
    await writeApiLog({ request: req, statusCode: 200, userId: session.userId, startedAt });
    return NextResponse.json({ submission });
  } catch (error) {
    const status = isWorkPrestationError(error) ? error.status : 500;
    await writeApiLog({ request: req, statusCode: status, userId: session.userId, startedAt, metadata: { action: "coo_work_submission_detail_failed", code: isWorkPrestationError(error) ? error.code : "UNKNOWN" } });
