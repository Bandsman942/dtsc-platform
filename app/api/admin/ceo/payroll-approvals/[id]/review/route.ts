...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const limited = await rateLimit(getRateLimitKey(req, `ceo-payroll-review:${session.userId}`), 50, 60 * 60 * 1000);
  ...
  try {
    const payroll = await reviewPayroll({ actor, payrollId: id, expectedApproverCode: "CEO", action: parsed.data.action, comment: parsed.data.comment });
    ...
    await writeApiLog({ request: req, statusCode: status, userId: session.userId, startedAt, metadata: { action: "ceo_payroll_review_failed", code: isPayrollWorkflowError(error) ? error.code : "UNKNOWN" } });
