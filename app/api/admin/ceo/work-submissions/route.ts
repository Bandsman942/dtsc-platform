...
export async function GET(req: Request) {
  ...
  try {
    const submissions = await getReviewerQueue(actor, "CEO");
    await writeApiLog({ request: req, statusCode: 200, userId: session.userId, startedAt });
    return NextResponse.json({ submissions });
  } catch (error) {
    const status = isWorkPrestationError(error) ? error.status : 500;
    await writeApiLog({ request: req, statusCode: status, userId: session.userId, startedAt, metadata: { action: "ceo_work_submission_queue_failed", code: isWorkPrestationError(error) ? error.code : "UNKNOWN" } });
