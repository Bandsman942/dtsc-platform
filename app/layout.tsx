...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = isPublic
    ? "DTSC | 7 leviers numériques de performance"
    : `${product.label.fr} | DTSC`;
  const description = isPublic
    ? "DTSC aide les organisations à améliorer leur performance grâce à sept leviers numériques complémentaires."
  ...
  return {
    ...
    description,
    keywords: isPublic
      ? ["DTSC", "Data & BI", "intelligence artificielle", "solutions digitales", "audit", "formations", "marketing digital", "imprimerie numérique", "Kinshasa", "RDC", "Afrique"]
      : undefined,
    ...
    openGraph: isPublic
      ? {
          ...
          title,
          description,
          images: [{ url: "/dtsc-logo.png", width: 1536, height: 864, alt: "Logo DTSC" }],
        }
      : undefined,
    twitter: isPublic
      ? { card: "summary_large_image", title, description, images: ["/dtsc-logo.png"] }
      : undefined,
    robots: isPublic
      ? {
          index: true,
...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  const locale = getRequestedLocale(
    cookieStore.get("dtsc_locale")?.value || cookieStore.get("NEXT_LOCALE")?.value,
  ...
  return (
    <html
      ...
      data-dtsc-product={product.code.toLowerCase()}
