...
export function AnnouncementDeepLinkActivator({ commentId }: { commentId?: string | null }) {
  useEffect(() => {
    ...
    function focusComment() {
      ...
      if (attempts === 0) {
        const announcementRoot = document.querySelector<HTMLElement>("[data-announcement-media-root]");
        const commentsButton = Array.from(announcementRoot?.querySelectorAll<HTMLButtonElement>("button") || []).find((button) =>
