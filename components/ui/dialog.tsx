...
export function Dialog({ open, title, description, children, footer, onClose, className }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#001736]/75 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className={cn("w-full max-w-2xl overflow-hidden rounded-2xl border border-dtsc-border bg-dtsc-surface shadow-[0_24px_80px_rgba(0,23,54,0.35)]", className)}>
        <div className="flex items-start justify-between gap-4 border-b border-dtsc-border bg-dtsc-page px-5 py-4">
          <div>
            <h2 className="text-lg font-black text-dtsc-ink">{title}</h2>
