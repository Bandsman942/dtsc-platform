...
type MeetingCoordinationResponse = {
  meeting: { id: string; title: string; status: string; participants: Array<{ userId: string }>; organizerUserId: string };
  coordination: MeetingCoordination;
  tasks: Array<{ id: string; title: string; status: string; assignedToUserId: string | null }>;
  capabilities: {
    canUpdateAgenda: boolean;
    canAddOrDeleteAgenda: boolean;
    canSaveMinutes: boolean;
    canPublishMinutes: boolean;
    canCreateFollowUpActions: boolean;
  };
};
...
export function MeetingCoordinationPanel({ organizationId, meetingId, members, locale, onChanged }: { organizationId: string; meetingId: string; members: EnterpriseChoice[]; locale?: string | null; onChanged?: () => void }) {
  ...
  return <div className="grid min-w-0 gap-5 border-t border-dtsc-border pt-4">
    ...
    <section className="grid gap-3">
      ...
      {data.coordination.agendaItems.map((item) => <article key={item.id} className="grid gap-2 rounded-xl border border-dtsc-border p-3"><div className="flex min-w-0 flex-wrap items-center gap-2"><StatusBadge>{coreStatusLabel(locale, item.status)}</StatusBadge><h5 className="min-w-0 flex-1 break-words font-black text-dtsc-ink">{item.title}</h5>{item.durationMinutes ? <span className="text-xs text-dtsc-muted">{item.durationMinutes} min</span> : null}{data.capabilities.canAddOrDeleteAgenda ? <button type="button" disabled={submitting} onClick={() => void mutate({ action: "DELETE_AGENDA_ITEM", agendaItemId: item.id })} className="flex h-9 w-9 items-center justify-center rounded-xl text-red-600 hover:bg-red-50 disabled:opacity-50" aria-label={enterpriseCoreT(locale, "meetings.coordination.delete.agenda.item")}><Trash2 className="h-4 w-4" /></button> : null}</div>{item.description ? <p className="text-sm text-dtsc-muted">{item.description}</p> : null}{data.capabilities.canUpdateAgenda ? <div className="flex min-w-0 gap-2 overflow-x-auto pb-1">{["PENDING", "DISCUSSED", "DEFERRED", "CANCELLED"].map((status) => <button key={status} type="button" disabled={submitting} onClick={() => void mutate({ action: "SET_AGENDA_STATUS", agendaItemId: item.id, status })} className="h-9 shrink-0 rounded-xl border border-dtsc-border px-3 text-xs font-black disabled:opacity-50">{coreStatusLabel(locale, status)}</button>)}</div> : null}</article>)}
      {data.capabilities.canAddOrDeleteAgenda ? <form onSubmit={addAgenda} className="grid gap-2 sm:grid-cols-2"><Input name="title" required minLength={2} maxLength={240} placeholder={enterpriseCoreT(locale, "meetings.coordination.agenda.topic")} /><Input name="description" maxLength={3000} placeholder={enterpriseCoreT(locale, "tasks.form.description")} /><select name="ownerUserId" defaultValue="" className="h-11 rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm"><option value="">{enterpriseCoreT(locale, "meetings.coordination.no.topic.owner")}</option>{members.map((member) => <option key={member.id} value={member.id}>{member.label}</option>)}</select><div className="flex gap-2"><Input name="durationMinutes" type="number" min={1} max={480} placeholder={enterpriseCoreT(locale, "meetings.coordination.minutes")} /><Button type="submit" variant="outline" disabled={submitting}><Plus className="h-4 w-4" />{enterpriseCoreT(locale, "common.add")}</Button></div></form> : null}
    </section>
    ...
    <section className="grid gap-3 border-t border-dtsc-border pt-4">
      ...
      <textarea value={minutes} onChange={(event) => setMinutes(event.target.value)} disabled={!data.capabilities.canSaveMinutes || submitting} aria-invalid={data.capabilities.canSaveMinutes && !minutes.trim()} className="min-h-44 w-full rounded-xl border border-dtsc-border bg-dtsc-surface p-3 text-sm disabled:opacity-60" placeholder={enterpriseCoreT(locale, "meetings.coordination.meeting.minutes.decisions.and.context")} />
      {data.capabilities.canSaveMinutes ? <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={submitting || !minutes.trim()} onClick={() => void saveMinutes(false)}>{enterpriseCoreT(locale, "meetings.coordination.save.draft.version")}</Button>{data.capabilities.canPublishMinutes ? <Button type="button" disabled={submitting || !minutes.trim()} onClick={() => void saveMinutes(true)} className="bg-dtsc-blue text-white">{enterpriseCoreT(locale, "meetings.coordination.publish.minutes")}</Button> : null}</div> : null}
