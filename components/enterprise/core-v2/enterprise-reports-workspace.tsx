...
export function EnterpriseReportsWorkspace({ organizationId, organizationName, organizationLogoUrl, canCreate, canManage, locale, legacyRecords = [] }: { organizationId: string; organizationName: string; organizationLogoUrl?: string | null; canCreate: boolean; canManage: boolean; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  const humanActionError = locale === "en" ? "This action could not be completed. Check the information and try again." : "Cette action n’a pas pu être terminée. Vérifiez les informations puis réessayez.";
  const humanLoadError = locale === "en" ? "The report could not be loaded. Refresh the list and try again." : "Le rapport n’a pas pu être chargé. Actualisez la liste puis réessayez.";
  ...
  async function runAction(item: ReportItem, action: "PUBLISH" | "ARCHIVE") {
    ...
    setMessage(""); setErrorMessage(""); setBusyActionId(item.id);
    try {
