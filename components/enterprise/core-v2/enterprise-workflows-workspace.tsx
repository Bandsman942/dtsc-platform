...
export function EnterpriseWorkflowsWorkspace({ organizationId, locale, members, departments, legacyWorkflows }: Props) {
  ...
  async function confirmPublish() {
    if (!selected || !currentDraft || publishing) return;
    const readinessForVersion = readiness[currentDraft.id];
    if (!readinessForVersion?.ready) return toastError(en ? "Resolve publication blockers before publishing." : "Résolvez les blocages de publication avant de publier.");
    setPublishing(true);
    ...
    if (!response.ok) {
      setPublishing(false);
      return toastError(body?.message || (en ? "Publication failed." : "Publication impossible."));
    }
    setPublishReviewOpen(false);
    setPublishing(false);
  ...
  return <div className="grid min-w-0 gap-4">
    ...
    {tab === "definitions" ? <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
      ...
      <section className="min-w-0 rounded-2xl border border-dtsc-border bg-dtsc-surface p-3 sm:p-4">{selected ? <div className="grid gap-4"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-[0.1em] text-cyan-600">{selected.code}</p><h2 className="truncate text-lg font-black text-dtsc-ink">{selected.name}</h2><p className="text-sm text-dtsc-muted">{selected.description}</p></div><StatusBadge>{statusLabel(selected.status, en)}</StatusBadge></div>
        <div className="flex flex-wrap gap-2">{permissions.canCreateDraft && published ? <Button size="sm" variant="outline" onClick={() => void duplicateVersion()}><Copy className="h-4 w-4" />{en ? "New version" : "Nouvelle version"}</Button> : null}{permissions.canEditDraft && currentDraft ? <Button size="sm" variant="outline" onClick={() => setStepOpen(true)}><Plus className="h-4 w-4" />{en ? "Step" : "Étape"}</Button> : null}{permissions.canEditDraft && currentDraft ? <Button size="sm" variant="outline" onClick={() => setTransitionOpen(true)}><GitBranch className="h-4 w-4" />{en ? "Transition" : "Transition"}</Button> : null}{permissions.canEditDraft && currentDraft ? <Button size="sm" onClick={() => void saveVersion()}><Save className="h-4 w-4" />{en ? "Save" : "Enregistrer"}</Button> : null}{permissions.canPublish && currentDraft ? <Button size="sm" disabled={!readinessForDraft?.ready} onClick={() => setPublishReviewOpen(true)}><Send className="h-4 w-4" />{en ? "Review publication" : "Examiner la publication"}</Button> : null}</div>
