...
export function EnterpriseMeetingForm({ locale, members, departments, value, onSubmit }: { locale?: string | null; members: EnterpriseChoice[]; departments: EnterpriseChoice[]; value?: MeetingFormValue; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <form onSubmit={onSubmit} className="grid gap-4">
    <div className="grid gap-3 md:grid-cols-2">
      ...
      {needsPhysicalLocation ? <Field label={enterpriseCoreT(locale, "meetings.form.physical.location")} help={locale === "en" ? "Required for on-site and hybrid meetings." : "Obligatoire pour les réunions sur site et hybrides."}><Input name="physicalLocation" required defaultValue={value?.physicalLocation || ""} /></Field> : null}
      {needsMeetingLink ? <Field label={enterpriseCoreT(locale, "meetings.form.meeting.link")} help={locale === "en" ? "Required for online and hybrid meetings." : "Obligatoire pour les réunions en ligne et hybrides."}><Input name="meetingLink" type="url" required defaultValue={value?.meetingLink || ""} /></Field> : null}
    </div>
    ...
    <p className="text-xs leading-5 text-dtsc-muted">{locale === "en" ? "Minutes are managed from the versioned meeting minutes section after the meeting is opened." : "Le compte rendu se gère dans la section versionnée dédiée après ouverture de la réunion."}</p>
