...
export function EnterprisePurchasesWorkspace({ organizationId, members, departments, capabilities, locale, legacyRecords = [] }: { organizationId: string; members: EnterpriseChoice[]; departments: EnterpriseChoice[]; capabilities: ProcurementUiCapabilities; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title={t("purchases.form.title")} className="h-[96dvh] max-w-5xl">
      <form onSubmit={createPurchase} className="grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-5" data-dtsc-guided-form="purchase">
        <Field label={t("purchases.form.subject")} help={guide("subject")} required><Input name="title" required /></Field>
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 md:grid-cols-2">
          <Field label={t("purchases.form.sourceRequest")} help={guide("sourceRequest")}><NativeSelect name="requestId" items={requests} /></Field>
          <Field label={t("purchases.form.supplier")} help={guide("supplier")} required><NativeSelect name="supplierId" items={suppliers} required /></Field>
          <Field label={t("purchases.form.budgetLine")} help={guide("budgetLine")}><NativeSelect name="budgetLineId" items={budgetLines} /></Field>
          <Field label={t("purchases.form.buyer")} help={guide("buyer")}><NativeSelect name="buyerUserId" items={members} /></Field>
          <Field label={t("purchases.form.department")} help={guide("department")}><NativeSelect name="departmentId" items={departments} /></Field>
          <Field label={t("purchases.form.priority")} help={guide("priority")}><NativeSelect name="priority" defaultValue="NORMAL" items={priorityChoices(locale)} /></Field>
          <Field label={t("purchases.form.currency")} help={guide("currency")} required><NativeSelect name="currency" defaultValue="USD" items={currencyChoices(locale)} required /></Field>
          <Field label={t("purchases.form.expectedDelivery")} help={guide("expectedDelivery")}><Input name="expectedAt" type="date" /></Field>
        </div>
        <Field label={t("purchases.form.description")} help={guide("description")}><textarea name="description" className="min-h-24 max-w-full rounded-xl border border-dtsc-border bg-dtsc-surface p-3 text-base text-dtsc-ink md:text-sm" /></Field>
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 border-y border-dtsc-border py-4">
          <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <strong className="block break-words text-dtsc-ink">{t("purchases.form.items")}</strong>
              <p className="mt-1 max-w-3xl break-words text-sm leading-6 text-dtsc-muted">{guide("items")}</p>
            </div>
            <Button type="button" variant="outline" onClick={() => setDraftItems((items) => [...items, { description: "", quantity: "1", unit: "unit", unitPrice: "0", taxRate: "0" }])}><Plus className="h-4 w-4" />{t("purchases.form.line")}</Button>
          </div>
          {draftItems.map((item, index) => <div key={index} className="grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-3 rounded-2xl border border-dtsc-border bg-dtsc-page p-3 md:grid-cols-[minmax(0,2fr)_minmax(0,.7fr)_minmax(0,.9fr)_minmax(0,1fr)_minmax(0,.8fr)]">
            ...
            <Field label={guide("unit")} help={guide("unitHelp")} required><NativeSelect value={item.unit} onChange={(value) => setDraftItems((items) => items.map((entry, itemIndex) => itemIndex === index ? { ...entry, unit: value } : entry))} items={unitChoices(locale)} required /></Field>
