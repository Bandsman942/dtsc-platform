...
type PendingMeetingAction = { meeting: Meeting; action: string };
...
export function EnterpriseMeetingsWorkspace({ organizationId, members, departments, canCreate, canManage, locale, legacyRecords = [] }: { organizationId: string; members: EnterpriseChoice[]; departments: EnterpriseChoice[]; canCreate: boolean; canManage: boolean; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  async function runAction() {
    ...
    try {
      ...
      setPendingAction(null); setActionComment(""); setDetail(null); setRefreshKey((value) => value + 1); setMessage(enterpriseCoreT(locale, "meetings.meeting.action.saved"));
    } catch (error) { setMessage(error instanceof Error ? error.message : "ACTION_FAILED"); }
  }
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title={enterpriseCoreT(locale, "meetings.new.meeting")} presentation="editor" className="h-[96dvh] max-w-4xl"><EnterpriseMeetingForm locale={locale} members={members} departments={departments} onSubmit={createMeeting} /></Dialog>
    <Dialog open={Boolean(edit)} onClose={() => setEdit(null)} title={enterpriseCoreT(locale, "meetings.edit.meeting")} presentation="editor" className="h-[96dvh] max-w-4xl">{edit ? <EnterpriseMeetingForm locale={locale} members={members} departments={departments} value={{ ...edit, participantIds: edit.participants.map((item) => item.userId) }} onSubmit={updateMeeting} /> : null}</Dialog>
    <Dialog open={Boolean(detail)} onClose={() => setDetail(null)} title={detail?.title || ""} presentation="editor" className="h-[96dvh] max-w-5xl">{detail ? <MeetingDetail organizationId={organizationId} meeting={detail} members={members} locale={locale} onDecision={() => setDecisionMeeting(detail)} onTask={(decision) => setTaskDecision({ meeting: detail, decision })} onChanged={() => setRefreshKey((value) => value + 1)} /> : null}</Dialog>
    <Dialog open={Boolean(pendingAction)} onClose={closePendingAction} title={enterpriseCoreT(locale, "meetings.confirm.meeting.action")} presentation="editor" className="h-[92dvh] max-w-3xl">{pendingAction ? <div className="grid gap-4"><div className="rounded-xl border border-dtsc-border bg-dtsc-page p-4"><div className="flex flex-wrap gap-2"><StatusBadge tone={statusTone(pendingAction.meeting.status)}>{statusLabel(locale, pendingAction.meeting.status)}</StatusBadge><StatusBadge>{meetingLocationModeLabel(locale, pendingAction.meeting.locationMode)}</StatusBadge></div><p className="mt-3 font-black text-dtsc-ink">{meetingActionLabel(locale, pendingAction.action)}</p><p className="mt-1 text-sm text-dtsc-muted">{pendingAction.meeting.title}</p><p className="mt-2 text-xs text-dtsc-muted">{locale === "en" ? `Revision ${pendingAction.meeting.revision} · ${pendingAction.meeting.participants.length} participant(s)` : `Révision ${pendingAction.meeting.revision} · ${pendingAction.meeting.participants.length} participant(s)`}</p>{pendingAction.action === "COMPLETE" ? <p className="mt-3 text-xs leading-5 text-dtsc-muted">{locale === "en" ? "Completing freezes the agenda. Versioned minutes and follow-up tasks remain available after completion." : "Terminer fige l’ordre du jour. Le compte rendu versionné et les actions de suivi restent disponibles après la réunion."}</p> : null}</div>{["CANCEL", "ARCHIVE"].includes(pendingAction.action) ? <label className="grid gap-2 text-sm font-black text-dtsc-ink">{locale === "en" ? "Reason *" : "Motif *"}<textarea value={actionComment} onChange={(event) => setActionComment(event.target.value)} aria-invalid={!actionComment.trim()} className="min-h-28 rounded-xl border border-dtsc-border bg-dtsc-surface p-3 font-normal" /><span className="text-xs font-normal text-dtsc-muted">{locale === "en" ? "The reason is kept in the operational audit trail and is preserved if the mutation fails." : "Le motif est conservé dans la piste d’audit opérationnelle et reste saisi si la mutation échoue."}</span></label> : null}<Button onClick={() => void runAction()} className="bg-dtsc-blue text-white">{enterpriseCoreT(locale, "common.confirm")}</Button></div> : null}</Dialog>
    <Dialog open={Boolean(decisionMeeting)} onClose={() => setDecisionMeeting(null)} title={enterpriseCoreT(locale, "meetings.record.a.decision")} presentation="editor" className="h-[90dvh] max-w-3xl"><form onSubmit={createDecision} className="grid gap-4"><Field label={enterpriseCoreT(locale, "meetings.decision.title")}><Input name="title" required minLength={3} /></Field><Field label={enterpriseCoreT(locale, "tasks.form.description")}><textarea name="description" className="min-h-28 w-full rounded-xl border border-dtsc-border bg-dtsc-surface p-3" /></Field><Button className="bg-dtsc-blue text-white">{enterpriseCoreT(locale, "meetings.save.decision")}</Button></form></Dialog>
    <Dialog open={Boolean(taskDecision)} onClose={() => setTaskDecision(null)} title={enterpriseCoreT(locale, "meetings.create.action.from.decision")} description={taskDecision?.decision.title} presentation="editor" className="h-[92dvh] max-w-4xl"><EnterpriseTaskForm locale={locale} members={members} departments={departments} onSubmit={createDecisionTask} /></Dialog>
  </div>;
}
...
function meetingActions(meeting: Meeting, canManage: boolean, currentUserId: string, locale: string | null | undefined, detail: (item: Meeting) => void, edit: (item: Meeting) => void, decision: (item: Meeting) => void, action: (value: PendingMeetingAction) => void): BusinessContextAction[] {
  ...
  if (canOperate && ["SCHEDULED", "IN_PROGRESS"].includes(meeting.status)) items.push({ id: "edit", label: enterpriseCoreT(locale, "common.edit"), icon: Pencil, onSelect: () => edit(meeting) });
