...
export function EnterpriseMeetingsWorkspace({ organizationId, members, departments, canCreate, canManage, locale, legacyRecords = [] }: { organizationId: string; members: EnterpriseChoice[]; departments: EnterpriseChoice[]; canCreate: boolean; canManage: boolean; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  }, [collection.items, collection.loading, deepLinkResolved, deepLinkedMeetingId, locale, organizationId]);

  function meetingPayload(form: FormData) {
    const payload = Object.fromEntries(form.entries());
    delete payload.participantIds;
    const locationMode = String(payload.locationMode || "ONLINE");
    return {
      ...payload,
      physicalLocation: locationMode === "ONLINE" ? "" : String(form.get("physicalLocation") || ""),
      meetingLink: locationMode === "PHYSICAL" ? "" : String(form.get("meetingLink") || ""),
      participants: form.getAll("participantIds").map((userId) => ({ userId: String(userId) })),
    };
  }
  ...
  async function runAction() {
    ...
    const needsReason = ["CANCEL", "ARCHIVE"].includes(pendingAction.action);
    if (needsReason && actionComment.trim().length < 3) {
      setActionError(enterpriseCoreT(locale, "meetings.action.reason.required"));
      return;
    }
    try {
      await enterpriseV2Mutation(`/api/enterprise/${organizationId}/meetings/${pendingAction.meeting.id}/actions`, "POST", { action: pendingAction.action, revision: pendingAction.meeting.revision, comment: actionComment.trim() || undefined });
      setPendingAction(null); setActionComment(""); setActionError(""); setDetail(null); setRefreshKey((value) => value + 1); setMessage(enterpriseCoreT(locale, "meetings.meeting.action.saved"));
    } catch (error) { setActionError(error instanceof Error ? error.message : "ACTION_FAILED"); }
  }
  ...
  function openAction(meeting: Meeting, action: string) {
    setActionComment("");
    setActionError("");
    setPendingAction({ meeting, action });
  }
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    {legacyRecords.length ? <ModuleSection title={enterpriseCoreT(locale, "meetings.historical.meetings")} description={enterpriseCoreT(locale, "meetings.legacy.meetings.and.minutes.records.remain.read.only")}><BusinessList ariaLabel={enterpriseCoreT(locale, "meetings.legacy.aria")}>{legacyRecords.map((record) => <BusinessListItem key={record.id} title={record.title} status={<StatusBadge>{enterpriseCoreT(locale, "tasks.historyBadge")}</StatusBadge>} meta={`${record.recordType} · ${statusLabel(locale, record.status)}`} description={record.description || formatEnterpriseDate(record.updatedAt, locale)} />)}</BusinessList></ModuleSection> : null}
    <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title={enterpriseCoreT(locale, "meetings.new.meeting")} presentation="editor" className="h-[94dvh] max-w-4xl"><EnterpriseMeetingForm locale={locale} members={members} departments={departments} onSubmit={createMeeting} /></Dialog>
    <Dialog open={Boolean(edit)} onClose={() => setEdit(null)} title={enterpriseCoreT(locale, "meetings.edit.meeting")} presentation="editor" className="h-[94dvh] max-w-4xl">{edit ? <EnterpriseMeetingForm locale={locale} members={members} departments={departments} value={{ ...edit, participantIds: edit.participants.map((item) => item.userId) }} onSubmit={updateMeeting} /> : null}</Dialog>
    <Dialog open={Boolean(detail)} onClose={() => setDetail(null)} title={detail?.title || ""} presentation="editor" className="h-[94dvh] max-w-5xl">{detail ? <MeetingDetail organizationId={organizationId} meeting={detail} members={members} locale={locale} onDecision={() => setDecisionMeeting(detail)} onTask={(decision) => setTaskDecision({ meeting: detail, decision })} onChanged={() => setRefreshKey((value) => value + 1)} /> : null}</Dialog>
    <Dialog open={Boolean(pendingAction)} onClose={() => { setPendingAction(null); setActionComment(""); setActionError(""); }} title={enterpriseCoreT(locale, "meetings.confirm.meeting.action")} presentation="editor" className="h-[94dvh] max-w-3xl"><div className="grid gap-4"><p className="text-sm text-dtsc-muted">{pendingAction ? meetingActionLabel(locale, pendingAction.action) : ""} · {pendingAction?.meeting.title}</p>{pendingAction && ["CANCEL", "ARCHIVE"].includes(pendingAction.action) ? <Field label={enterpriseCoreT(locale, "meetings.action.reason")}><textarea value={actionComment} onChange={(event) => { setActionComment(event.target.value); setActionError(""); }} minLength={3} required aria-invalid={Boolean(actionError)} className="min-h-32 w-full rounded-xl border border-dtsc-border bg-dtsc-surface p-3 text-sm" /></Field> : null}{actionError ? <p className="rounded-xl border border-red-300 bg-red-50 p-3 text-sm text-red-800" role="alert">{actionError}</p> : null}<Button onClick={() => void runAction()} className="bg-dtsc-blue text-white">{enterpriseCoreT(locale, "common.confirm")}</Button></div></Dialog>
    <Dialog open={Boolean(decisionMeeting)} onClose={() => setDecisionMeeting(null)} title={enterpriseCoreT(locale, "meetings.record.a.decision")} presentation="editor" className="h-[94dvh] max-w-3xl"><form onSubmit={createDecision} className="grid gap-4"><Field label={enterpriseCoreT(locale, "meetings.decision.title")}><Input name="title" required minLength={3} /></Field><Field label={enterpriseCoreT(locale, "tasks.form.description")}><textarea name="description" className="min-h-28 w-full rounded-xl border border-dtsc-border bg-dtsc-surface p-3" /></Field><Button className="bg-dtsc-blue text-white">{enterpriseCoreT(locale, "meetings.save.decision")}</Button></form></Dialog>
    <Dialog open={Boolean(taskDecision)} onClose={() => setTaskDecision(null)} title={enterpriseCoreT(locale, "meetings.create.action.from.decision")} description={taskDecision?.decision.title} presentation="editor" className="h-[94dvh] max-w-4xl"><EnterpriseTaskForm locale={locale} members={members} departments={departments} onSubmit={createDecisionTask} /></Dialog>
  </div>;
}
...
function MeetingDetail({ organizationId, meeting, members, locale, onDecision, onTask, onChanged }: { organizationId: string; meeting: Meeting; members: EnterpriseChoice[]; locale?: string | null; onDecision: () => void; onTask: (decision: Decision) => void; onChanged: () => void }) {
  const canRecordDecision = ["IN_PROGRESS", "COMPLETED"].includes(meeting.status);
  return <div className="grid gap-5 text-sm"><div className="flex flex-wrap gap-2"><StatusBadge tone={statusTone(meeting.status)}>{statusLabel(locale, meeting.status)}</StatusBadge><StatusBadge>{meetingLocationModeLabel(locale, meeting.locationMode)}</StatusBadge></div><p className="leading-6 text-dtsc-muted">{meeting.agenda || enterpriseCoreT(locale, "meetings.no.agenda")}</p><div className="grid gap-2 border-y border-dtsc-border py-3 sm:grid-cols-2"><span>{enterpriseCoreT(locale, "meetings.start")} : {formatEnterpriseDate(meeting.startAt, locale)}</span><span>{enterpriseCoreT(locale, "meetings.end")} : {formatEnterpriseDate(meeting.endAt, locale)}</span>{meeting.physicalLocation ? <span>{enterpriseCoreT(locale, "meetings.location")} : {meeting.physicalLocation}</span> : null}{meeting.meetingLink ? <span className="min-w-0 break-all">{enterpriseCoreT(locale, "meetings.form.meeting.link")} : {meeting.meetingLink}</span> : null}<span>{enterpriseCoreT(locale, "tasks.revision")} : {meeting.revision}</span></div><section><h3 className="mb-2 font-black">{enterpriseCoreT(locale, "meetings.participants.2")}</h3><div className="flex flex-wrap gap-2">{meeting.participants.map((participant) => <StatusBadge key={participant.id}>{members.find((item) => item.id === participant.userId)?.label || participant.userId} · {statusLabel(locale, participant.responseStatus)}</StatusBadge>)}</div></section><section><h3 className="mb-2 font-black">{enterpriseCoreT(locale, "meetings.minutes")}</h3><p className="whitespace-pre-wrap text-dtsc-muted">{meeting.minutes || enterpriseCoreT(locale, "meetings.no.minutes.yet")}</p></section><section className="grid gap-2"><div className="flex items-center justify-between gap-2"><h3 className="font-black">{enterpriseCoreT(locale, "meetings.decisions")}</h3>{canRecordDecision ? <Button variant="outline" onClick={onDecision}><Plus className="h-4 w-4" />{enterpriseCoreT(locale, "meetings.decision")}</Button> : null}</div>{meeting.decisions.length ? meeting.decisions.map((decision) => <div key={decision.id} className="flex min-w-0 items-start justify-between gap-3 border-t border-dtsc-border py-3"><div className="min-w-0"><p className="font-semibold">{decision.title}</p><p className="text-dtsc-muted">{decision.description || formatEnterpriseDate(decision.decidedAt, locale)}</p></div>{decision.taskId ? <StatusBadge tone="success">{enterpriseCoreT(locale, "meetings.task.linked")}</StatusBadge> : <Button variant="outline" size="sm" onClick={() => onTask(decision)}><SquareCheckBig className="h-4 w-4" />{enterpriseCoreT(locale, "meetings.create.task")}</Button>}</div>) : <p className="text-dtsc-muted">{enterpriseCoreT(locale, "meetings.no.decision.recorded")}</p>}</section><MeetingCoordinationPanel organizationId={organizationId} meetingId={meeting.id} members={members} locale={locale} onChanged={onChanged} /></div>;
}

function meetingLocationModeLabel(locale: string | null | undefined, mode: string) {
...
function meetingActions(meeting: Meeting, canManage: boolean, currentUserId: string, locale: string | null | undefined, detail: (item: Meeting) => void, edit: (item: Meeting) => void, decision: (item: Meeting) => void, action: (meeting: Meeting, action: string) => void): BusinessContextAction[] {
  ...
  if (canOperate && ["SCHEDULED", "IN_PROGRESS"].includes(meeting.status)) items.push({ id: "edit", label: enterpriseCoreT(locale, "meetings.edit.meeting"), icon: Pencil, onSelect: () => edit(meeting) });
  if (canOperate && meeting.status === "SCHEDULED") items.push({ id: "start", label: enterpriseCoreT(locale, "common.start"), icon: Play, onSelect: () => action(meeting, "START") });
  if (canOperate && meeting.status === "IN_PROGRESS") items.push({ id: "complete", label: enterpriseCoreT(locale, "common.complete"), icon: CheckCircle2, onSelect: () => action(meeting, "COMPLETE") });
  if (canOperate && ["IN_PROGRESS", "COMPLETED"].includes(meeting.status)) items.push({ id: "decision", label: enterpriseCoreT(locale, "meetings.record.decision"), icon: FileCheck2, onSelect: () => decision(meeting) });
  if (canOperate && ["SCHEDULED", "IN_PROGRESS"].includes(meeting.status)) items.push({ id: "cancel", label: enterpriseCoreT(locale, "common.cancel"), icon: XCircle, destructive: true, separatorBefore: true, onSelect: () => action(meeting, "CANCEL") });
  if (canManage) items.push({ id: "archive", label: enterpriseCoreT(locale, "common.archive"), icon: Archive, separatorBefore: true, onSelect: () => action(meeting, "ARCHIVE") });
  return items;
}
