...
export function EnterpriseMeetingsWorkspace({ organizationId, members, departments, canCreate, canManage, locale, legacyRecords = [] }: { organizationId: string; members: EnterpriseChoice[]; departments: EnterpriseChoice[]; canCreate: boolean; canManage: boolean; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    <Dialog open={Boolean(pendingAction)} onClose={() => setPendingAction(null)} title={enterpriseCoreT(locale, "meetings.confirm.meeting.action")}><p className="text-sm text-dtsc-muted">{pendingAction ? meetingActionLabel(locale, pendingAction.action) : ""} · {pendingAction?.meeting.title}</p><Button onClick={() => void runAction()} className="mt-4 bg-dtsc-blue text-white">{enterpriseCoreT(locale, "common.confirm")}</Button></Dialog>
...
function meetingActions(meeting: Meeting, canManage: boolean, currentUserId: string, locale: string | null | undefined, detail: (item: Meeting) => void, edit: (item: Meeting) => void, decision: (item: Meeting) => void, action: (value: { meeting: Meeting; action: string }) => void): BusinessContextAction[] {
