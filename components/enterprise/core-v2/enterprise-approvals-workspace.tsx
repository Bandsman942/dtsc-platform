...
export function EnterpriseApprovalsWorkspace({ organizationId, locale, legacyRecords = [] }: { organizationId: string; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    <Dialog open={Boolean(detail)} onClose={() => setDetail(null)} title={detail?.target?.title || (detail ? approvalTargetLabel(locale, detail.targetEntityType) : "")} presentation="editor" className="h-[96dvh] max-w-5xl">{detail ? <div className="grid gap-4 text-sm"><div className="rounded-xl border border-dtsc-border bg-dtsc-page p-4"><div className="flex flex-wrap items-center gap-2"><StatusBadge tone={statusTone(detail.status)}>{statusLabel(locale, detail.status)}</StatusBadge><StatusBadge>{approvalTargetLabel(locale, detail.targetEntityType)}</StatusBadge></div><p className="mt-3 text-dtsc-muted">{enterpriseCoreT(locale, "approvals.requested")} : {formatEnterpriseDate(detail.requestedAt, locale)}</p><p className="mt-1 text-dtsc-muted">{enterpriseCoreT(locale, "approvals.target")} : {detail.targetEntityId}</p>{detail.decisionComment ? <p className="mt-3 rounded-xl border border-dtsc-border bg-dtsc-surface p-3 text-dtsc-muted">{detail.decisionComment}</p> : null}</div><p className="text-xs leading-5 text-dtsc-muted">{locale === "en" ? "Review the source, immutable submission versions and decision history below before approving, rejecting, requesting a correction or delegating." : "Vérifiez la source, les versions immuables soumises et l’historique ci-dessous avant d’approuver, rejeter, demander une correction ou déléguer."}</p><ApprovalCoordinationPanel organizationId={organizationId} approvalId={detail.id} locale={locale} onChanged={() => { setRefreshKey((value) => value + 1); }} /></div> : null}</Dialog>
  </div>;
}
...
function approvalActions(approval: Approval, locale: string | null | undefined, detail: (item: Approval) => void): BusinessContextAction[] {
  return [{ id: "open", label: approval.status === "PENDING" ? (locale === "en" ? "Review and decide" : "Examiner et décider") : enterpriseCoreT(locale, "common.open"), icon: Eye, onSelect: () => detail(approval) }];
}
