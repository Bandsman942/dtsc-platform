...
function approvalActions(approval: Approval, locale: string | null | undefined, detail: (item: Approval) => void): BusinessContextAction[] {
  return [{ id: "open", label: approval.status === "PENDING" ? enterpriseCoreT(locale, "approvals.open.approval") : enterpriseCoreT(locale, "common.open"), icon: Eye, onSelect: () => detail(approval) }];
}
