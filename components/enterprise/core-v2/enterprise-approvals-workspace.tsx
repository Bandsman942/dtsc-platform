...
export function EnterpriseApprovalsWorkspace({ organizationId, locale, legacyRecords = [] }: { organizationId: string; locale?: string | null; legacyRecords?: LegacyRecord[] }) {
  ...
  async function decide() {
    ...
    try {
      await enterpriseV2Mutation(`/api/enterprise/${organizationId}/approvals/${pending.approval.id}/actions`, "POST", { action: pending.action, revision: pending.approval.revision, decisionComment: comment.trim() || undefined });
  ...
  return <div className="grid min-w-0 gap-5">
    <ModuleMetrics label={en ? "Approval indicators" : "Indicateurs validations"}>
      ...
      <ModuleMetric label={en ? "Approved here" : "Approuvées ici"} value={collection.items.filter((item) => item.status === "APPROVED").length} />
    ...
    <ModuleSection title={en ? "Approval queue" : "File des validations"} description={en ? "The default view prioritizes decisions assigned to you." : "La vue par défaut privilégie les décisions qui vous sont attribuées."} count={`${collection.pagination.total}`}>
      <div className="grid gap-2 border-y border-dtsc-border py-3 md:grid-cols-3">
        <div className="flex min-w-0 gap-2"><Button variant={queue === "pending" ? "default" : "outline"} onClick={() => { setQueue("pending"); setPage(1); }} className="flex-1">{en ? "To process" : "À traiter"}</Button><Button variant={queue === "treated" ? "default" : "outline"} onClick={() => { setQueue("treated"); setPage(1); }} className="flex-1">{en ? "Processed" : "Traitées"}</Button></div>
