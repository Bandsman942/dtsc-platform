...
function mapOptions(kind: ReferenceKind, items: unknown[], locale?: string | null): FinanceReferenceOption[] {
  const en = locale === "en";
  if (kind === "budget-line") {
    return (items as Array<{ id: string; name: string; code?: string | null; budget: { reference: string; title?: string; currency: string } }>).map((item) => ({
      id: item.id,
      label: `${item.budget.reference} · ${item.code ? `${item.code} · ` : ""}${item.name} · ${item.budget.currency}`,
      currency: item.budget.currency,
    }));
  }
  if (kind === "supplier") {
    return (items as Array<{ id: string; legalName: string; displayName?: string | null; businessPartyId?: string | null }>).map((item) => ({
      id: item.id,
  ...
  if (kind === "purchase") {
    return (items as Array<{ id: string; reference: string; title: string; status: string; supplierId?: string | null; budgetLineId?: string | null; currency?: string | null; totalAmount?: string | number | null }>).filter((item) => !["REJECTED", "CANCELLED"].includes(item.status)).map((item) => ({
      id: item.id,
  ...
  if (kind === "document") {
    return (items as Array<{ id: string; title: string; documentType?: string }>).map((item) => ({
      id: item.id,
      label: item.documentType ? `${item.title} · ${item.documentType}` : item.title,
    }));
  }
  if (kind === "customer") {
    return (items as Array<{ id: string; code?: string | null; legalName: string; displayName?: string | null }>).map((item) => ({
      id: item.id,
      label: `${item.code || ""} ${item.displayName || item.legalName}`.trim(),
      businessPartyId: item.id,
    }));
  }
  if (kind === "sales-order") {
    return (items as Array<{ id: string; reference: string; title: string; businessPartyId: string; currency?: string | null; totalAmount?: string | number | null }>).map((item) => ({
      id: item.id,
      label: `${item.reference} · ${item.title}`,
      businessPartyId: item.businessPartyId,
      currency: item.currency,
  ...
  if (kind === "contract") {
    return (items as Array<{ id: string; reference: string; title: string; businessPartyId?: string | null; currency?: string | null; indicativeAmount?: string | number | null }>).map((item) => ({
      id: item.id,
      label: `${item.reference} · ${item.title}`,
      businessPartyId: item.businessPartyId,
      currency: item.currency,
      amount: item.indicativeAmount,
    }));
  }
  ...
  if (kind === "financial-account") {
    return (items as Array<{ id: string; code: string; name: string; accountType: string; currencyCode: string; maskedReference?: string | null }>).map((item) => ({
      id: item.id,
      label: `${item.code} · ${item.name} · ${item.currencyCode}${item.maskedReference ? ` · ${item.maskedReference}` : ""}`,
      currency: item.currencyCode,
    }));
  }
  if (kind === "payroll-run") {
    return (items as Array<{ id: string; reference: string; currency: string; netAmount: string | number; payrollPeriod: { code: string; name: string } }>).map((item) => ({
      id: item.id,
      label: `${item.reference} · ${item.payrollPeriod.code} · ${item.payrollPeriod.name} · ${item.currency}`,
      currency: item.currency,
...
export function FinanceReferenceSelect({
  organizationId,
  kind,
  name,
  label,
  ...
  return (
    <div className="grid min-w-0 gap-2">
      ...
      <select
        ...
        onChange={(event) => {
          const next = options.find((item) => item.id === event.target.value) || null;
          setSelected(next);
          onOptionChange?.(next);
        }}
        className="h-11 w-full min-w-0 max-w-full truncate rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-base text-dtsc-ink outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:opacity-60 md:text-sm"
      >
