...
export function statusLabel(locale: string | null | undefined, status: string) {
  return (locale === "en" ? EN_STATUS : FR_STATUS)[status] || status;
}

export function priorityLabel(locale: string | null | undefined, priority: string) {
  return (locale === "en" ? EN_PRIORITY : FR_PRIORITY)[priority] || priority;
}

export function statusTone(status: string): StatusBadgeTone {
  if (/REJECTED|CANCELLED|BLOCKED|CRITICAL/i.test(status)) return "danger";
  if (/PENDING|SUBMITTED|TODO|SCHEDULED/i.test(status)) return "warning";
  if (/APPROVED|DONE|COMPLETED|FULFILLED/i.test(status)) return "success";
  if (/IN_PROGRESS|IN_REVIEW/i.test(status)) return "info";
  return "neutral";
}

export function formatEnterpriseDate(value: string | Date | null | undefined, locale?: string | null) {
  if (!value) return locale === "en" ? "Not specified" : "Non précisée";
...
export function Field({ label, help, children }: { label: string; help?: string; children: ReactNode }) {
  return (
    <label className="grid min-w-0 gap-1">
      <span className="flex min-w-0 items-center gap-1 text-xs font-black uppercase text-dtsc-muted">
        <span className="min-w-0 break-words">{label}</span>
        {help ? <span title={help} aria-label={`${label} : ${help}`} className="shrink-0 cursor-help"><CircleHelp className="h-3.5 w-3.5" /></span> : null}
      </span>
      {children}
    </label>
  );
...
export function NativeSelect({ name, items, required, defaultValue, value, onChange, disabled }: { name?: string; items: EnterpriseChoice[]; required?: boolean; defaultValue?: string; value?: string; onChange?: (value: string) => void; disabled?: boolean }) {
  return (
    <select name={name} required={required} defaultValue={value === undefined ? defaultValue : undefined} value={value} onChange={onChange ? (event) => onChange(event.target.value) : undefined} disabled={disabled} className="h-11 w-full min-w-0 truncate rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm text-dtsc-ink disabled:opacity-60">
      <option value="">—</option>
...
export const priorityChoicesFr: EnterpriseChoice[] = [
  { id: "LOW", label: "Faible" },
...
export const priorityChoicesEn: EnterpriseChoice[] = [
  { id: "LOW", label: "Low" },
