...
export function EnterpriseFinancePaymentsWorkspace(props: Props) {
  ...
  async function createPayment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await financeMutation(`/api/enterprise/${organizationId}/payments`, {
        ...
        businessPartyId: ["CUSTOMER_PAYMENT", "SUPPLIER_PAYMENT"].includes(paymentType) ? businessPartyId || undefined : undefined,
      ...
      setCreateOpen(false); resetCreate(); setRefreshKey((value) => value + 1); setMessage(t("paymentSavedDraft"));
    } catch (error) {
  ...
  return <ModuleWorkspace>
    ...
    <Dialog open={createOpen} onClose={() => { if (!busy) { setCreateOpen(false); resetCreate(); } }} title={t("newPayment")} description={t("amountsAccountsControlled")} presentation="editor" className="max-w-4xl">
      <form onSubmit={createPayment} className="grid gap-6">
        <ProfessionalFormSection title={t("paymentNature")}>
          ...
          <Field label={t("direction")}><NativeSelect name="direction" value={direction} onChange={setDirection} disabled={busy || ["CUSTOMER_PAYMENT", "SUPPLIER_PAYMENT", "PAYROLL_PAYMENT"].includes(paymentType)} required items={["INBOUND", "OUTBOUND"].map((id) => ({ id, label: financeEnumLabel(id, locale) }))} /></Field>
    ...
    <Dialog open={Boolean(actionTarget)} onClose={() => { if (!busy) setActionTarget(null); }} title={actionTarget ? `${actionTarget.label} · ${actionTarget.record.number}` : ""} description={t("sodAndPeriodChecked")} presentation="editor" className="max-w-xl">
      {actionTarget ? <form onSubmit={transition} className="grid gap-4">
        ...
        {["CANCEL", "REVERSE"].includes(actionTarget.action) ? <Field label={t("decisionReasonComment")}><textarea name="reason" minLength={4} rows={4} required disabled={busy} className="w-full rounded-xl border border-dtsc-border bg-dtsc-surface px-3 py-2 disabled:opacity-60" /><p className="mt-2 text-xs leading-5 text-dtsc-muted">{t("rejectionReasonMinimumHint")}</p></Field> : <Field label={t("decisionReasonComment")}><textarea name="reason" rows={3} disabled={busy} className="w-full rounded-xl border border-dtsc-border bg-dtsc-surface px-3 py-2 disabled:opacity-60" /><p className="mt-2 text-xs leading-5 text-dtsc-muted">{t("decisionCommentOptionalHint")}</p></Field>}
