...
export function EnterpriseFinanceAdvancedWorkspaceHotfix(props: Props) {
  ...
  useEffect(() => {
    ...
    void fetchOperationalFinanceRecord<Item>(`/api/enterprise/${organizationId}/${config.endpoint}`, recordId)
      ...
      .catch((error) => setErrorMessage(safeFinanceError(error, t("accountingLoadFailed"), locale)));
  }, [config.deepLink, config.endpoint, locale, organizationId, searchParams, t]);
  ...
  return <ModuleWorkspace>
    ...
    <Dialog open={Boolean(closeAction)} onClose={() => { if (!busy) setCloseAction(null); }} title={closeAction ? financeEnumLabel(closeAction.action, locale) : t("details")} description={t("finalControlsServer")} presentation="editor"><form onSubmit={submitCloseAction} className="grid gap-4">{closeAction?.action === "REOPEN" ? <Input name="reason" required minLength={8} placeholder={t("detailedReopeningReason")} disabled={busy} /> : null}<Button type="submit" disabled={busy}>{t("save")}</Button></form></Dialog>
