...
const DEFAULT_FORM: FormState = {
  ...
  capitalizationSourceType: "OPERATIONAL_ASSET",
  throughDate: "",
  reason: "",
};
...
const SECTIONS: Record<string, SectionDefinition[]> = {
  ...
  FINANCE_TAX: [
    { key: "taxes", labelFr: "Codes et taux fiscaux", labelEn: "Tax codes and rates", endpoint: "taxes" },
  ],
  FINANCE_CLOSE: [
    { key: "close", labelFr: "Clôtures financières", labelEn: "Financial closes", endpoint: "financial-close" },
  ],
  FINANCE_STATEMENTS: [
    { key: "statements", labelFr: "Versions générées et publiées", labelEn: "Generated and published versions", endpoint: "financial-statements" },
  ],
  FINANCE_ASSETS: [
    { key: "assets", labelFr: "Registre des immobilisations", labelEn: "Fixed asset register", endpoint: "asset-accounting" },
  ],
  FINANCE_INVENTORY: [
    { key: "inventory", labelFr: "Valorisation du stock", labelEn: "Inventory valuation", endpoint: "inventory-valuation" },
  ],
};

const BUSINESS_LABELS: Record<string, { fr: string; en: string }> = {
  ASSET: { fr: "Actif", en: "Asset" },
  LIABILITY: { fr: "Passif", en: "Liability" },
  EQUITY: { fr: "Capitaux propres", en: "Equity" },
  REVENUE: { fr: "Produits", en: "Revenue" },
  EXPENSE: { fr: "Charges", en: "Expense" },
  OTHER_INCOME: { fr: "Autres produits", en: "Other income" },
  OTHER_EXPENSE: { fr: "Autres charges", en: "Other expense" },
  GENERAL: { fr: "Opérations diverses", en: "General" },
  SALES: { fr: "Ventes", en: "Sales" },
  PURCHASES: { fr: "Achats", en: "Purchases" },
  PAYROLL: { fr: "Paie", en: "Payroll" },
  INVENTORY: { fr: "Stock", en: "Inventory" },
  ASSETS: { fr: "Immobilisations", en: "Fixed assets" },
  ...
  VAT: { fr: "Taxe sur la valeur ajoutée", en: "Value-added tax" },
  SALES_TAX: { fr: "Taxe sur les ventes", en: "Sales tax" },
  WITHHOLDING: { fr: "Retenue à la source", en: "Withholding tax" },
  EXEMPT: { fr: "Exonéré", en: "Exempt" },
  ZERO_RATED: { fr: "Taux zéro", en: "Zero-rated" },
  STRAIGHT_LINE: { fr: "Linéaire", en: "Straight-line" },
  WEIGHTED_AVERAGE: { fr: "Coût moyen pondéré", en: "Weighted average" },
};

function businessLabel(value: unknown, locale: FinanceLocale) {
  const text = String(value || "");
  if (!text) return "";
  const known = BUSINESS_LABELS[text];
  if (known) return locale === "fr" ? known.fr : known.en;
  const financeKnown = financeEnumLabel(text, locale);
  if (financeKnown !== text) return financeKnown;
  return text.toLowerCase().replaceAll("_", " ").replace(/^./, (letter) => letter.toUpperCase());
}
...
function nestedText(item: Item, parent: string, key: string) {
...
async function requestJson(endpoint: string, method: "GET" | "POST" = "GET", body?: unknown) {
  ...
  if (!response.ok) {
    throw new Error(String(payload?.message || payload?.error || "L’opération comptable n’a pas pu être terminée."));
  }
  return payload || {};
}
...
export function EnterpriseAdvancedFinanceWorkspace({ organizationId, organizationName, definition, locale: rawLocale, canManage }: Props) {
  ...
  useEffect(() => {
    if (!sections.some((section) => section.key === activeKey)) setActiveKey(sections[0]?.key || "overview");
  }, [activeKey, sections]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPayload({
        ...
        pagination: body.pagination || { page: 1, pageSize: 25, total: Array.isArray(body.items) ? body.items.length : 0, pageCount: 1 },
      });
  ...
  const loadLookups = useCallback(async () => {
    ...
    const requests = await Promise.allSettled([
      requestJson(`${base}/charts-of-accounts`),
      requestJson(`${base}/fiscal-years`),
      requestJson(`${base}/fiscal-periods`),
      requestJson(`${base}/journals`),
      requestJson(`${base}/ledger-accounts?page=1&pageSize=500`),
      requestJson(`${base}/asset-accounting?page=1&pageSize=250`),
    ]);
    const itemsAt = (index: number) => requests[index].status === "fulfilled" && Array.isArray((requests[index] as PromiseFulfilledResult<Record<string, unknown>>).value.items)
      ? ((requests[index] as PromiseFulfilledResult<Record<string, unknown>>).value.items as Item[])
      : [];
    ...
    setLookups({
      ...
      journals: itemsAt(3),
      accounts: itemsAt(4),
      assets: Array.isArray(assetValue) ? assetValue as Item[] : [],
    });
  }, [organizationId]);
  ...
  const metrics = useMemo(() => {
    ...
    for (const item of items) {
      const key = text(item, "status") || "TOTAL";
  ...
  async function mutate(endpoint: string, body: unknown, success: string) {
    ...
    try {
      ...
      setError(safeFinanceError(mutationError, locale === "fr" ? "Action comptable impossible." : "Accounting action failed."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function submitForm(event: FormEvent) {
    ...
    const base = `/api/enterprise/${organizationId}`;
    if (reopenClose) {
      await mutate(`${base}/financial-close/${reopenClose.id}/transition`, {
        action: "REOPEN",
        reason: form.reason,
        revision: reopenClose.revision,
      }, locale === "fr" ? "Période réouverte avec motif et piste d’audit." : "Period reopened with reason and audit trail.");
      return;
    }
    if (reversalEntryId) {
      await mutate(`${base}/journal-entries/${reversalEntryId}/reverse`, {
        accountingDate: form.accountingDate,
        reason: form.reason,
      }, locale === "fr" ? "Écriture contrepassée sans modifier l’original." : "Entry reversed without modifying the original.");
      return;
    }
    if (activeKey === "charts") {
      await mutate(`${base}/charts-of-accounts`, { code: form.code, nameFr: form.nameFr, nameEn: form.nameEn }, locale === "fr" ? "Plan comptable créé." : "Chart of accounts created.");
    } else if (activeKey === "years") {
      await mutate(`${base}/fiscal-years`, { code: form.code, startDate: form.startDate, endDate: form.endDate }, locale === "fr" ? "Exercice créé." : "Fiscal year created.");
    } else if (activeKey === "periods") {
      await mutate(`${base}/fiscal-periods`, { fiscalYearId: form.fiscalYearId, code: form.code, startDate: form.startDate, endDate: form.endDate }, locale === "fr" ? "Période créée." : "Period created.");
    } else if (activeKey === "accounts") {
      await mutate(`${base}/ledger-accounts`, {
        ...
        isControlAccount: Boolean(form.isControlAccount),
        isSystemAccount: false,
        allowDirectPosting: Boolean(form.allowDirectPosting),
      }, locale === "fr" ? "Compte créé." : "Account created.");
    } else if (activeKey === "journals") {
      await mutate(`${base}/journals`, {
        code: form.code,
        nameFr: form.nameFr,
        nameEn: form.nameEn,
      ...
      }, locale === "fr" ? "Journal créé." : "Journal created.");
    } else if (activeKey === "entries") {
      await mutate(`${base}/journal-entries`, {
        journalId: form.journalId,
        fiscalPeriodId: form.fiscalPeriodId,
        accountingDate: form.accountingDate,
        reference: form.reference || undefined,
        description: form.description,
        idempotencyKey: `${organizationId}:manual-entry:${Date.now()}`,
        lines: [
          { ledgerAccountId: form.debitAccountId, debit: form.amount, credit: "0", transactionCurrencyCode: form.currencyCode, transactionAmount: form.amount },
          { ledgerAccountId: form.creditAccountId, debit: "0", credit: form.amount, transactionCurrencyCode: form.currencyCode, transactionAmount: form.amount },
        ],
      }, locale === "fr" ? "Écriture équilibrée créée." : "Balanced journal entry created.");
    } else if (activeKey === "taxes") {
      await mutate(`${base}/taxes`, {
        code: form.code,
        nameFr: form.nameFr,
        nameEn: form.nameEn,
        ...
        recoverableAccountId: form.recoverableAccountId || undefined,
        roundingRule: "HALF_UP",
        rate: form.rate,
        effectiveFrom: form.effectiveFrom,
      }, locale === "fr" ? "Code fiscal et taux historisé créés." : "Tax code and effective rate created.");
    } else if (activeKey === "close") {
      await mutate(`${base}/financial-close`, { fiscalPeriodId: form.fiscalPeriodId }, locale === "fr" ? "Checklist de clôture préparée." : "Closing checklist prepared.");
    } else if (activeKey === "statements") {
      await mutate(`${base}/financial-statements`, {
        statementType: form.statementType,
        periodStart: form.periodStart,
        periodEnd: form.periodEnd,
        currencyCode: form.currencyCode,
        publish: Boolean(form.publish),
      }, Boolean(form.publish) ? (locale === "fr" ? "Version publiée et non modifiable créée." : "Immutable published version created.") : (locale === "fr" ? "Aperçu financier généré." : "Financial preview generated."));
    } else if (activeKey === "assets") {
      await mutate(`${base}/asset-accounting`, {
        assetId: form.assetId,
        capitalizationSourceType: form.capitalizationSourceType,
        currencyCode: form.currencyCode,
      ...
      }, locale === "fr" ? "Actif capitalisé et plan d’amortissement créé." : "Asset capitalized and depreciation schedule created.");
    } else if (activeKey === "inventory") {
      await mutate(`${base}/financial-statements`, {
        statementType: "INVENTORY_VALUATION",
        periodStart: form.periodStart,
        periodEnd: form.periodEnd,
        currencyCode: form.currencyCode,
        publish: true,
      }, locale === "fr" ? "Valorisation publiée et verrouillée comme version non modifiable." : "Valuation published as an immutable version.");
    }
  }
  ...
  async function transitionClose(item: Item, action: "SUBMIT" | "APPROVE" | "CLOSE") {
    await mutate(`/api/enterprise/${organizationId}/financial-close/${item.id}/transition`, { action, revision: item.revision }, locale === "fr" ? "Workflow de clôture mis à jour." : "Financial close workflow updated.");
  }

  async function runDepreciation() {
    await mutate(`/api/enterprise/${organizationId}/asset-depreciation/run`, { throughDate: form.throughDate }, locale === "fr" ? "Amortissements exigibles comptabilisés de manière idempotente." : "Due depreciation posted idempotently.");
  }
  ...
  return (
    <ModuleWorkspace>
      <ModuleHeader
        eyebrow={`${organizationName} · ${locale === "fr" ? "Finance avancée" : "Advanced finance"}`}
        title={locale === "fr" ? definition.labelFr : definition.labelEn}
        description={locale === "fr"
          ? "Workspace professionnel relié au moteur comptable unique : partie double, périodes, contrôles, publication et piste d’audit."
          : "Professional workspace connected to the single accounting engine: double-entry, periods, controls, publication, and audit trail."}
        primaryAction={actionAvailable ? <Button onClick={() => { setShowForm((current) => !current); setReversalEntryId(""); setReopenClose(null); }}><Plus className="h-4 w-4" />{showForm ? (locale === "fr" ? "Fermer" : "Close") : (locale === "fr" ? "Nouvelle action" : "New action")}</Button> : undefined}
        secondaryActions={<Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />{locale === "fr" ? "Actualiser" : "Refresh"}</Button>}
      />
      ...
      <nav className="flex touch-pan-x gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label={locale === "fr" ? "Sections comptables" : "Accounting sections"}>
        {sections.map((section) => (
          <button
            key={section.key}
            type="button"
            onClick={() => { setActiveKey(section.key); setPage(1); setSearch(""); setStatus(""); setShowForm(false); setReversalEntryId(""); setReopenClose(null); }}
            className={`shrink-0 rounded-full border px-4 py-2 text-sm font-black transition ${section.key === activeKey ? "border-dtsc-blue bg-dtsc-blue text-white" : "border-dtsc-border bg-dtsc-page text-dtsc-ink hover:bg-dtsc-soft"}`}
          >
            {locale === "fr" ? section.labelFr : section.labelEn}
          </button>
        ))}
      </nav>

      {Object.keys(metrics).length ? (
        <ModuleMetrics label={locale === "fr" ? "Indicateurs comptables" : "Accounting metrics"}>
          {Object.entries(metrics).slice(0, 8).map(([key, value]) => (
            <ModuleMetric key={key} label={key === "TOTAL" ? "Total" : businessLabel(key, locale)} value={value} />
          ))}
        </ModuleMetrics>
      ) : null}

      <ModuleToolbar
        search={<div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-dtsc-muted" /><Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder={locale === "fr" ? "Rechercher sans identifiant technique…" : "Search without technical identifiers…"} className="pl-9" /></div>}
        controls={<div className="flex min-w-0 flex-wrap gap-2"><Input value={status} onChange={(event) => { setStatus(event.target.value.toUpperCase()); setPage(1); }} placeholder={locale === "fr" ? "Statut" : "Status"} className="min-w-36 flex-1 sm:w-48" /><Button variant="outline" onClick={() => { setSearch(""); setStatus(""); setPage(1); }}><RotateCcw className="h-4 w-4" />{locale === "fr" ? "Réinitialiser" : "Reset"}</Button></div>}
        summary={`${pagination.total} ${locale === "fr" ? "élément(s)" : "item(s)"}`}
      />
      ...
      {(showForm || reversalEntryId || reopenClose) && actionAvailable ? (
        <ModuleSection title={reversalEntryId ? (locale === "fr" ? "Contrepasser l’écriture" : "Reverse journal entry") : reopenClose ? (locale === "fr" ? "Réouvrir la période" : "Reopen period") : (locale === "fr" ? "Action professionnelle" : "Professional action")} description={locale === "fr" ? "Les contrôles définitifs restent exécutés côté serveur." : "Final controls remain server-authoritative."}>
          <form onSubmit={submitForm} className="grid gap-4 rounded-2xl border border-dtsc-border bg-dtsc-page/70 p-4 sm:grid-cols-2 lg:grid-cols-3">
            {reversalEntryId ? <>
              <Field label={locale === "fr" ? "Date autorisée" : "Authorized date"}><Input type="date" value={String(form.accountingDate)} onChange={(event) => updateForm("accountingDate", event.target.value)} required /></Field>
              <Field label={locale === "fr" ? "Motif détaillé" : "Detailed reason"} className="sm:col-span-2"><Input value={String(form.reason)} onChange={(event) => updateForm("reason", event.target.value)} minLength={8} required /></Field>
            </> : reopenClose ? <><Field label={locale === "fr" ? "Motif détaillé de réouverture" : "Detailed reopening reason"} className="sm:col-span-2 lg:col-span-3"><Input value={String(form.reason)} onChange={(event) => updateForm("reason", event.target.value)} minLength={8} required /></Field></> : <ActionFields activeKey={activeKey} locale={locale} form={form} updateForm={updateForm} lookups={lookups} />}
            <div className="flex flex-wrap gap-2 sm:col-span-2 lg:col-span-3">
              <Button type="submit" disabled={saving}><Save className="h-4 w-4" />{saving ? (locale === "fr" ? "Traitement…" : "Processing…") : (locale === "fr" ? "Enregistrer" : "Save")}</Button>
              <Button type="button" variant="outline" onClick={() => { setShowForm(false); setReversalEntryId(""); setReopenClose(null); }}>{locale === "fr" ? "Annuler" : "Cancel"}</Button>
              {activeKey === "assets" && !reversalEntryId ? <Button type="button" variant="outline" onClick={() => void runDepreciation()} disabled={saving}><Settings2 className="h-4 w-4" />{locale === "fr" ? "Exécuter les amortissements exigibles" : "Run due depreciation"}</Button> : null}
            </div>
          </form>
        </ModuleSection>
      ) : null}
      ...
      <ModuleContent>
        <ModuleSection
          title={activeSection ? (locale === "fr" ? activeSection.labelFr : activeSection.labelEn) : definition.labelFr}
          description={locale === "fr" ? "Données filtrées par entreprise, paginées côté serveur et présentées en langage métier." : "Organization-scoped, server-paginated data presented in business language."}
          count={pagination.total}
        >
          {loading ? (
            <div className="space-y-3">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-xl bg-dtsc-soft" />)}</div>
          ) : items.length === 0 ? (
            <div className="flex min-h-48 flex-col items-center justify-center border-y border-dashed border-dtsc-border py-10 text-center"><BookOpen className="h-8 w-8 text-dtsc-muted" /><p className="mt-3 font-black text-dtsc-ink">{locale === "fr" ? "Aucune donnée pour cette vue" : "No data for this view"}</p><p className="mt-1 max-w-xl text-sm text-dtsc-muted">{locale === "fr" ? "Utilisez la prochaine action utile ou vérifiez les filtres. Aucun résultat technique artificiel n’est affiché." : "Use the next useful action or review filters. No artificial technical result is displayed."}</p></div>
          ) : (
            <div className="divide-y divide-dtsc-border border-y border-dtsc-border">
              {items.map((item) => (
                <article key={item.id || text(item, "inventoryItemId") + text(item, "warehouseId") + text(item, "currencyCode")} className="py-4 first:pt-0 last:pb-0">
                  <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex min-w-0 flex-wrap items-center gap-2"><h3 className="min-w-0 break-words font-black text-dtsc-ink">{itemTitle(item, activeKey, locale)}</h3>{item.status ? <StatusBadge tone={financeStatusTone(String(item.status))}>{financeStatusLabel(String(item.status), locale)}</StatusBadge> : null}</div>
                      <p className="mt-1 break-words text-sm text-dtsc-muted">{itemDescription(item, activeKey, locale)}</p>
                      {amountSummary(item, activeKey, locale) ? <p className="mt-2 text-sm font-black tabular-nums text-dtsc-ink">{amountSummary(item, activeKey, locale)}</p> : null}
                      <BusinessDetails item={item} activeKey={activeKey} locale={locale} />
                    </div>
...
function Field({ label, children, className = "" }: { label: string; children: ReactNode; className?: string }) {
  return <label className={`min-w-0 text-sm font-bold text-dtsc-ink ${className}`}>{label}<div className="mt-1.5">{children}</div></label>;
}
...
function SelectField({ label, value, onChange, children, required = false }: { label: string; value: string; onChange: (value: string) => void; children: ReactNode; required?: boolean }) {
  return <Field label={label}><select value={value} onChange={(event) => onChange(event.target.value)} required={required} className="h-10 w-full rounded-md border border-input bg-background px-3 text-base sm:text-sm">{children}</select></Field>;
}
...
function BusinessDetails({ item, activeKey, locale }: { item: Item; activeKey: string; locale: FinanceLocale }) {
  ...
  if (activeKey === "close") {
    ...
    details.push([locale === "fr" ? "Blocages" : "Blockers", blockers.length ? blockers.map(([key, value]) => `${businessLabel(key, locale)}: ${value}`).join(" · ") : (locale === "fr" ? "Aucun blocage détecté" : "No blocker detected")]);
  }
  ...
  if (activeKey === "assets") {
    const schedules = Array.isArray(item.schedules) ? item.schedules as Item[] : [];
    details.push([locale === "fr" ? "Valeur résiduelle" : "Residual value", financeMoney(item.residualValue, text(item, "currencyCode") || "USD", locale)], [locale === "fr" ? "Échéances comptabilisées" : "Posted periods", String(schedules.filter((schedule) => schedule.status === "POSTED").length)], [locale === "fr" ? "Prochaine échéance" : "Next schedule", financeDate(schedules.find((schedule) => schedule.status === "PLANNED")?.scheduledDate, locale)]);
  }
