...
export function EnterprisePayrollOperationsWorkspace({ organizationId, organizationName, definition }: { organizationId: string; organizationName: string; definition: EnterpriseModuleDefinition }) {
  ...
  return <ModuleWorkspace>
    ...
    <Dialog open={Boolean(detail)} onClose={() => setDetail(null)} title={detail ? `${detail.reference} · ${detail.payrollPeriod.name}` : "Détail de la paie"} className="h-[94dvh] max-w-5xl">{detail ? <div className="grid gap-5"><div className="flex flex-wrap gap-2"><StatusBadge tone={statusTone(detail.status)}>{STATUS_LABELS[detail.status] || detail.status}</StatusBadge><StatusBadge>{money(detail.netAmount, detail.currency)} net</StatusBadge></div><BusinessList ariaLabel="Collaborateurs de la paie">{detail.items.map((item) => <BusinessListItem key={item.id} title={`${item.employee.employeeNumber} · ${item.employee.displayName}`} status={<StatusBadge tone={item.payslip ? "success" : "neutral"}>{item.payslip ? `${item.payslip.payslipNumber} · ${STATUS_LABELS[item.payslip.status] || item.payslip.status}` : "Bulletin non généré"}</StatusBadge>} meta={`${money(item.netAmount, detail.currency)} net · ${minutesLabel(item.approvedTimeMinutes)}`} description={`Base ${money(item.baseGrossAmount, detail.currency)} · Prime ${money(item.bonusAmount, detail.currency)} · Retenue ${money(item.deductionAmount, detail.currency)}`} />)}</BusinessList><div data-responsive-actions>{actionsFor(detail).filter((action) => action.id !== "open").map((action) => <Button key={action.id} type="button" variant={action.destructive ? "outline" : "default"} onClick={() => action.onSelect?.()}>{action.label}</Button>)}</div></div> : null}</Dialog>
  </ModuleWorkspace>;
}
