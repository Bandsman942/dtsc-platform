...
type ChartDatum = { key: string; label: string; labelFr?: string; labelEn?: string; value: number; amount?: string; currencyCode?: string };
type Payload = { items?: Item[]; metrics?: Record<string, number>; pagination?: Pagination; charts?: { workflow?: ChartDatum[]; journals?: ChartDatum[] }; range?: string };
...
const DEFAULT_FORM: FormState = {
  ...
  journalType: "GENERAL", sequencePrefix: "", requiresApproval: true, isActive: true, journalId: "", fiscalPeriodId: "",
  accountingDate: new Date().toISOString().slice(0, 10), reference: "", description: "", debitAccountId: "", creditAccountId: "", amount: "",
};
const VALID_TABS: TabKey[] = ["setup", "overview", "charts", "accounts", "years", "periods", "journals", "entries", "ledger", "trial", "rules", "anomalies"];
...
function mutableSegment(tab: TabKey) {
  if (tab === "years") return "fiscal-years";
  if (tab === "periods") return "fiscal-periods";
  if (tab === "journals") return "journals";
  return null;
...
export function EnterpriseAccountingWorkspace({ organizationId, organizationName, definition, locale: rawLocale, canManage }: Props) {
  ...
  const initialTab = requestedTab && VALID_TABS.includes(requestedTab) ? requestedTab : "setup";
  ...
  useEffect(() => {
    const next = requestedTab && VALID_TABS.includes(requestedTab) ? requestedTab : "setup";
  ...
  const loadLookups = useCallback(async () => {
    ...
    const items = (index: number) => results[index].status === "fulfilled" && Array.isArray((results[index] as PromiseFulfilledResult<Record<string, unknown>>).value.items)
      ? (results[index] as PromiseFulfilledResult<Record<string, unknown>>).value.items as Item[] : [];
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const endpoint = endpointFor(activeTab, range);
      const separator = endpoint.includes("?") ? "&" : "?";
      const params = new URLSearchParams({ page: String(page), pageSize: "25" });
      if (search.trim()) params.set("search", search.trim());
      if (status.trim()) params.set("status", status.trim());
      const body = await requestJson(`/api/enterprise/${organizationId}/${endpoint}${separator}${params.toString()}`, copy.loadFailed) as unknown as Payload;
  ...
  const filteredItems = useMemo(() => {
    ...
    return allItems.filter((item) => {
      ...
      const rowStatus = itemStatus(item);
      return matchesSearch && (!status || rowStatus === status);
    });
  ...
  const selectedChart = activeTab === "charts" ? filteredItems.find((item) => itemStatus(item) === "ACTIVE") || null : null;

  function updateForm(key: string, value: string | boolean) { setForm((current) => ({ ...current, [key]: value })); }
  function openCreateForm() {
    setEditingItem(null);
    setForm({ ...DEFAULT_FORM, accountingDate: new Date().toISOString().slice(0, 10) });
    setFormOpen(true);
  }
  function openEditForm(item: Item) {
    setEditingItem(item);
    if (activeTab === "years") setForm({ ...DEFAULT_FORM, code: stringValue(item, "code"), startDate: inputDate(item.startDate), endDate: inputDate(item.endDate) });
    else if (activeTab === "periods") setForm({ ...DEFAULT_FORM, fiscalYearId: nestedValue(item, "fiscalYear", "id") || stringValue(item, "fiscalYearId"), code: stringValue(item, "code"), startDate: inputDate(item.startDate), endDate: inputDate(item.endDate) });
    else if (activeTab === "journals") setForm({ ...DEFAULT_FORM, code: stringValue(item, "code"), nameFr: stringValue(item, "nameFr"), nameEn: stringValue(item, "nameEn"), journalType: stringValue(item, "journalType") || "GENERAL", sequencePrefix: stringValue(item, "sequencePrefix"), requiresApproval: booleanValue(item, "requiresApproval"), isActive: item.isActive !== false });
    else return;
    setOpenMenuId(null);
    setFormOpen(true);
  }
  ...
  async function submitForm(event: FormEvent) {
    ...
    const base = `/api/enterprise/${organizationId}`;
    try {
      if (editingItem && activeTab === "years") await requestJson(`${base}/fiscal-years/${editingItem.id}`, copy.saveFailed, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ code: form.code, startDate: form.startDate, endDate: form.endDate, revision: editingItem.revision }) });
      else if (editingItem && activeTab === "periods") await requestJson(`${base}/fiscal-periods/${editingItem.id}`, copy.saveFailed, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ fiscalYearId: form.fiscalYearId, code: form.code, startDate: form.startDate, endDate: form.endDate, revision: editingItem.revision }) });
      else if (editingItem && activeTab === "journals") await requestJson(`${base}/journals/${editingItem.id}`, copy.saveFailed, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ code: form.code, nameFr: form.nameFr, nameEn: form.nameEn, journalType: form.journalType, sequencePrefix: form.sequencePrefix || null, requiresApproval: Boolean(form.requiresApproval), isActive: Boolean(form.isActive), revision: editingItem.revision }) });
      else if (activeTab === "charts") await requestJson(`${base}/charts-of-accounts`, copy.saveFailed, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ code: form.code, nameFr: form.nameFr, nameEn: form.nameEn }) });
      ...
      setFormOpen(false);
      setEditingItem(null);
      setToast({ tone: "success", title: copy.successTitle, message: editingItem ? actions.updated : copy.successTitle });
  ...
  async function openFiscalYear(item: Item) {
    ...
    try {
      ...
      setToast({ tone: "success", title: copy.successTitle, message: copy.configurationReady });
      setOpenMenuId(null);
      await Promise.all([load(), loadLookups()]);
  ...
  async function toggleJournal(item: Item) {
    setSaving(true);
    try {
      await requestJson(`/api/enterprise/${organizationId}/journals/${item.id}`, copy.saveFailed, {
        method: "PATCH", headers: { "content-type": "application/json" },
        body: JSON.stringify({ code: stringValue(item, "code"), nameFr: stringValue(item, "nameFr"), nameEn: stringValue(item, "nameEn"), journalType: stringValue(item, "journalType"), sequencePrefix: stringValue(item, "sequencePrefix") || null, requiresApproval: booleanValue(item, "requiresApproval"), isActive: item.isActive === false, revision: item.revision }),
      });
      setToast({ tone: "success", title: copy.successTitle, message: actions.statusChanged });
      setOpenMenuId(null);
      await Promise.all([load(), loadLookups()]);
    } catch (error) { setToast({ tone: "error", title: copy.errorTitle, message: safeFinanceError(error, copy.saveFailed, locale) }); }
    finally { setSaving(false); }
  ...
  async function deleteItem() {
    ...
    const segment = mutableSegment(activeTab);
    if (!segment) return;
    setSaving(true);
    try {
      await requestJson(`/api/enterprise/${organizationId}/${segment}/${deleteCandidate.id}`, copy.saveFailed, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ revision: deleteCandidate.revision }) });
      setDeleteCandidate(null);
      setToast({ tone: "success", title: copy.successTitle, message: actions.deleted });
      await Promise.all([load(), loadLookups()]);
    } catch (error) { setToast({ tone: "error", title: copy.errorTitle, message: safeFinanceError(error, copy.saveFailed, locale) }); }
    finally { setSaving(false); }
  }
  ...
  return (
    <ModuleWorkspace>
        ...
        <>
          ...
          <ModuleContent>
            <ModuleSection title={tabs.find((tab) => tab.id === activeTab)?.label || copy.details} description={sectionDescription} count={pagination.total}>
              {activeTab === "overview" ? <OverviewView payload={payload} range={range} setRange={setRange} locale={locale} copy={copy} selectTab={selectTab} /> : loading ? <ProfessionalLoading /> : filteredItems.length === 0 ? <EmptyState copy={copy} /> : (
                <div className="min-w-0 space-y-4">
                  {selectedChart ? <SelectedChart item={selectedChart} locale={locale} copy={copy} help={actions.selectedChartHelp} onOpen={setDetailItem} /> : null}
                  {isTableTab(activeTab) ? <AccountingTable tab={activeTab} items={filteredItems} locale={locale} copy={copy} onOpen={setDetailItem} /> : <BusinessList tab={activeTab} items={filteredItems} locale={locale} copy={copy} actions={actions} canManage={canManage} openMenuId={openMenuId} setOpenMenuId={setOpenMenuId} onOpen={setDetailItem} onEdit={openEditForm} onDelete={setDeleteCandidate} onOpenFiscalYear={openFiscalYear} onToggleJournal={toggleJournal} saving={saving} />}
                </div>
              )}
      ...
      <Dialog open={formOpen} title={editingItem ? `${actions.editItem} · ${itemTitle(editingItem, activeTab, locale, copy)}` : copy.newItem} description={sectionDescription} onClose={() => { setFormOpen(false); setEditingItem(null); }} className="h-[92dvh] sm:max-w-5xl" footer={<><Button type="button" variant="outline" onClick={() => { setFormOpen(false); setEditingItem(null); }}>{copy.cancel}</Button><Button type="submit" form="accounting-professional-form" disabled={saving}><Save className="h-4 w-4" />{saving ? copy.processing : copy.save}</Button></>}>
        <form id="accounting-professional-form" onSubmit={submitForm} className="min-w-0 space-y-5"><p className="text-sm leading-6 text-dtsc-muted">{copy.formHelp}</p><AccountingFormFields tab={activeTab} form={form} updateForm={updateForm} lookups={lookups} locale={locale} copy={copy} editing={Boolean(editingItem)} /></form>
      </Dialog>

      <Dialog open={Boolean(detailItem)} title={detailItem ? itemTitle(detailItem, activeTab, locale, copy) : copy.details} description={sectionDescription} onClose={() => setDetailItem(null)} className="h-[94dvh] sm:max-w-6xl" footer={<Button type="button" variant="outline" onClick={() => setDetailItem(null)}>{actions.backToList}</Button>}>
        {detailItem ? <AccountingDetail tab={activeTab} item={detailItem} organizationId={organizationId} locale={locale} copy={copy} /> : null}
      </Dialog>

      <Dialog open={Boolean(deleteCandidate)} title={actions.deleteTitle} description={actions.deleteDescription} onClose={() => setDeleteCandidate(null)} className="sm:max-w-xl" footer={<><Button variant="outline" onClick={() => setDeleteCandidate(null)}>{copy.cancel}</Button><Button variant="destructive" disabled={saving} onClick={() => void deleteItem()}><Trash2 className="h-4 w-4" />{saving ? copy.processing : actions.deleteConfirm}</Button></>}>
        {deleteCandidate ? <p className="text-sm leading-6 text-dtsc-ink"><strong>{itemTitle(deleteCandidate, activeTab, locale, copy)}</strong><br />{actions.deleteDescription}</p> : null}
      </Dialog>
    </ModuleWorkspace>
  );
}
...
function SelectedChart({ item, locale, copy, help, onOpen }: { item: Item; locale: FinanceLocale; copy: ReturnType<typeof getAccountingWorkspaceCopy>; help: string; onOpen: (item: Item) => void }) {
  return <div className="flex min-w-0 flex-col gap-3 border-y border-dtsc-blue/30 bg-dtsc-blue/5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.12em] text-dtsc-blue">{copy.currentChart}</p><p className="mt-1 break-words font-black text-dtsc-ink">{stringValue(item, "code")} · {localName(item, locale)}</p><p className="mt-1 text-xs leading-5 text-dtsc-muted">{help}</p></div><Button variant="outline" onClick={() => onOpen(item)}>{copy.openDetails}</Button></div>;
}
...
function OverviewView({ payload, range, setRange, locale, copy, selectTab }: { payload: Payload; range: string; setRange: (value: string) => void; locale: FinanceLocale; copy: ReturnType<typeof getAccountingWorkspaceCopy>; selectTab: (tab: TabKey) => void }) {
  ...
  const workflow = payload.charts?.workflow || [
    { key: "DRAFT", label: "DRAFT", value: Number(metrics.draftEntries || 0) },
    { key: "PENDING_APPROVAL", label: "PENDING_APPROVAL", value: Number(metrics.pendingApproval || 0) },
    { key: "POSTED", label: "POSTED", value: Number(metrics.postedEntries || 0) },
    { key: "FAILED", label: "FAILED", value: Number(metrics.failedPostings || 0) },
  ];
  const maxWorkflow = Math.max(1, ...workflow.map((item) => item.value));
  const interpretations: Array<{ text: string; tab?: TabKey }> = [];
  if (Number(metrics.failedPostings || 0) > 0) interpretations.push({ text: copy.failedInterpretation, tab: "anomalies" }); else interpretations.push({ text: copy.healthyInterpretation });
  ...
  const kpis = [
    [copy.entries, Number(metrics.postedEntries || 0), "entries" as TabKey],
    [financeStatusLabel("PENDING_APPROVAL", locale), Number(metrics.pendingApproval || 0), "entries" as TabKey],
    [copy.anomalies, Number(metrics.failedPostings || 0), "anomalies" as TabKey],
    [copy.periods, Number(metrics.openPeriods || 0), "periods" as TabKey],
  ] as const;
  ...
  return <div className="min-w-0 space-y-6">
    ...
    <div className="grid min-w-0 grid-cols-2 gap-3 lg:grid-cols-4">{kpis.map(([label, value, tab]) => <button key={label} type="button" onClick={() => selectTab(tab)} className="min-w-0 border-l-2 border-dtsc-blue py-1 pl-3 text-left"><p className="text-xs font-black uppercase tracking-[0.1em] text-dtsc-muted">{label}</p><p className="mt-1 text-2xl font-black tabular-nums text-dtsc-ink">{value}</p></button>)}</div>
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 lg:grid-cols-2">
      <div className="min-w-0 border-y border-dtsc-border py-5"><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-dtsc-blue" /><h3 className="font-black text-dtsc-ink">{copy.workflow}</h3></div><div className="mt-5 space-y-4">{workflow.map((item) => <button key={item.key} type="button" onClick={() => selectTab(item.key === "FAILED" ? "anomalies" : "entries")} className="block w-full text-left"><div className="flex justify-between gap-3 text-sm"><span className="font-bold text-dtsc-ink">{financeStatusLabel(item.key, locale)}</span><span className="font-black tabular-nums">{item.value}</span></div><div className="mt-1.5 h-2 overflow-hidden rounded-full bg-dtsc-soft"><div className="h-full rounded-full bg-dtsc-blue transition-[width]" style={{ width: `${Math.max(3, Math.round((item.value / maxWorkflow) * 100))}%` }} /></div></button>)}</div></div>
      <div className="min-w-0 border-y border-dtsc-border py-5"><h3 className="font-black text-dtsc-ink">{copy.activityByJournal}</h3>{payload.charts?.journals?.length ? <div className="mt-5 space-y-4">{payload.charts.journals.map((item) => <div key={item.key}><div className="flex min-w-0 justify-between gap-3 text-sm"><span className="min-w-0 truncate font-bold text-dtsc-ink">{locale === "fr" ? item.labelFr || item.label : item.labelEn || item.label}</span><span className="shrink-0 font-black tabular-nums">{item.value}</span></div>{item.amount ? <p className="mt-1 text-xs font-bold text-dtsc-muted">{financeMoney(item.amount, item.currencyCode || "USD", locale)}</p> : null}</div>)}</div> : <p className="mt-4 text-sm text-dtsc-muted">{copy.noDataHelp}</p>}</div>
    </div>
...
function BusinessList({ tab, items, locale, copy, actions, canManage, openMenuId, setOpenMenuId, onOpen, onEdit, onDelete, onOpenFiscalYear, onToggleJournal, saving }: { tab: TabKey; items: Item[]; locale: FinanceLocale; copy: ReturnType<typeof getAccountingWorkspaceCopy>; actions: ReturnType<typeof getAccountingActionCopy>; canManage: boolean; openMenuId: string | null; setOpenMenuId: (id: string | null) => void; onOpen: (item: Item) => void; onEdit: (item: Item) => void; onDelete: (item: Item) => void; onOpenFiscalYear: (item: Item) => void; onToggleJournal: (item: Item) => void; saving: boolean }) {
  const contextual = ["years", "periods", "journals"].includes(tab);
  return <div className="divide-y divide-dtsc-border border-y border-dtsc-border">{items.map((item) => <article key={item.id} className="relative min-w-0 py-4"><div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><button type="button" className="min-w-0 flex-1 text-left" onClick={() => onOpen(item)}><div className="flex min-w-0 flex-wrap items-center gap-2"><h3 className="min-w-0 break-words font-black text-dtsc-ink">{itemTitle(item, tab, locale, copy)}</h3><StatusBadge tone={financeStatusTone(itemStatus(item))}>{financeStatusLabel(itemStatus(item), locale)}</StatusBadge></div><p className="mt-1 text-sm leading-6 text-dtsc-muted">{itemSubtitle(item, tab, locale, copy)}</p></button><div data-responsive-actions className="flex shrink-0 flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => onOpen(item)}>{copy.openDetails}</Button>{canManage && contextual ? <div className="relative"><Button size="icon" variant="ghost" aria-label={copy.moreActions} title={copy.moreActions} aria-expanded={openMenuId === item.id} onClick={() => setOpenMenuId(openMenuId === item.id ? null : item.id)}><Ellipsis className="h-4 w-4" /></Button>{openMenuId === item.id ? <div className="absolute right-0 z-30 mt-1 w-64 rounded-xl border border-dtsc-border bg-dtsc-surface p-1 shadow-xl"><MenuButton onClick={() => { onOpen(item); setOpenMenuId(null); }}>{copy.openDetails}</MenuButton><MenuButton onClick={() => onEdit(item)}>{actions.editItem}</MenuButton>{tab === "years" && itemStatus(item) === "DRAFT" ? <MenuButton disabled={saving} onClick={() => void onOpenFiscalYear(item)}>{copy.openFiscalYear}</MenuButton> : null}{tab === "periods" ? <Link className="block rounded-lg px-3 py-2 text-sm font-bold hover:bg-dtsc-soft" href="/enterprise-modules/FINANCE_CLOSE">{copy.operationalControls}</Link> : null}{tab === "journals" ? <MenuButton disabled={saving} onClick={() => void onToggleJournal(item)}>{item.isActive === false ? actions.activateJournal : actions.deactivateJournal}</MenuButton> : null}<MenuButton destructive onClick={() => { setOpenMenuId(null); onDelete(item); }}>{actions.deleteItem}</MenuButton></div> : null}</div> : null}</div></div></article>)}</div>;
}

function MenuButton({ children, onClick, disabled, destructive = false }: { children: ReactNode; onClick: () => void; disabled?: boolean; destructive?: boolean }) {
  return <button type="button" disabled={disabled} className={`w-full rounded-lg px-3 py-2 text-left text-sm font-bold hover:bg-dtsc-soft disabled:cursor-not-allowed disabled:opacity-50 ${destructive ? "text-red-600 dark:text-red-300" : "text-dtsc-ink"}`} onClick={onClick}>{children}</button>;
}
...
function tableCells(item: Item, tab: TabKey, locale: FinanceLocale): ReactNode[] {
...
function AccountingFormFields({ tab, form, updateForm, lookups, locale, copy, editing }: { tab: TabKey; form: FormState; updateForm: (key: string, value: string | boolean) => void; lookups: LookupState; locale: FinanceLocale; copy: ReturnType<typeof getAccountingWorkspaceCopy>; editing: boolean }) {
...
function AccountingDetail({ tab, item, organizationId, locale, copy }: { tab: TabKey; item: Item; organizationId: string; locale: FinanceLocale; copy: ReturnType<typeof getAccountingWorkspaceCopy> }) {
  const [chartDetail, setChartDetail] = useState<Item | null>(null);
  const [loadingAccounts, setLoadingAccounts] = useState(false);
  useEffect(() => {
    if (tab !== "charts") { setChartDetail(null); return; }
    setLoadingAccounts(true);
    requestJson(`/api/enterprise/${organizationId}/charts-of-accounts/${encodeURIComponent(item.id)}`, copy.loadFailed)
      .then((body) => setChartDetail(body.item && typeof body.item === "object" ? body.item as Item : null))
      .catch(() => setChartDetail(null))
      .finally(() => setLoadingAccounts(false));
  }, [copy.loadFailed, item.id, organizationId, tab]);
  const currency = stringValue(item, "currencyCode") || stringValue(item, "functionalCurrencyCode") || "USD";
  const accounts = chartDetail && Array.isArray(chartDetail.accounts) ? chartDetail.accounts as Item[] : [];
  return <BusinessDetail>
    ...
    {tab === "charts" ? <BusinessDetailSection title={copy.chartAccounts} description={copy.chartsDescription}>{loadingAccounts ? <ProfessionalLoading rows={3} /> : accounts.length ? <div className="divide-y divide-dtsc-border border-y border-dtsc-border">{accounts.map((account) => <div key={account.id} className="grid min-w-0 gap-1 py-3 md:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] md:gap-4"><p className="font-black text-dtsc-ink">{stringValue(account, "code")} · {localName(account, locale)}</p><p className="text-sm leading-6 text-dtsc-muted">{accountMeaning(stringValue(account, "accountType"), locale)} {accountUsage(account, locale, copy)}</p></div>)}</div> : <p className="text-sm text-dtsc-muted">{copy.noAccountsInChart}</p>}</BusinessDetailSection> : null}
    ...
    {tab === "rules" ? <BusinessDetailSection title={copy.ruleExplanation} description={copy.rulesDescription}><p className="text-sm leading-6 text-dtsc-ink">{copy.ruleExplanation}</p><p className="mt-3 text-sm leading-6 text-dtsc-muted">{stringValue(item, "sourceModule") || "—"} → {stringValue(item, "mappingKey")} → {stringValue(item, "accountCode")} · {locale === "fr" ? stringValue(item, "accountNameFr") : stringValue(item, "accountNameEn")}</p></BusinessDetailSection> : null}
  </BusinessDetail>;
}
