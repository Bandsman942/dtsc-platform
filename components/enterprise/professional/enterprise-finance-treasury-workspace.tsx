...
export function EnterpriseFinanceTreasuryWorkspace({ organizationId, organizationName, locale: rawLocale, canManage }: Props) {
  ...
  return <ModuleWorkspace>
    <ModuleHeader eyebrow={`${t("eyebrow")} · ${organizationName}`} title={t("title")} description={t("description")}
      ...
      secondaryActions={<div className="flex flex-wrap gap-2">{canManage ? <Button variant="outline" onClick={() => setAccountDialog(true)}><Plus className="h-4 w-4" />{t("newAccount")}</Button> : null}<Link href="/enterprise-modules/FINANCE_TREASURY/exchange-rates"><Button variant="outline">{locale === "fr" ? "Taux de change" : "Exchange rates"}</Button></Link></div>} />
    <ModuleContent>
