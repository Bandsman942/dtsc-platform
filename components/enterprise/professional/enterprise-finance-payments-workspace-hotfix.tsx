  ...
  return <ModuleWorkspace>
    ...
    <Dialog open={Boolean(actionTarget)} onClose={() => { if (!busy) setActionTarget(null); }} title={actionTarget ? `${actionTarget.label} · ${actionTarget.record.number}` : ""} description={t("sodAndPeriodChecked")} presentation="editor" className="max-w-xl">
      {actionTarget ? <form onSubmit={transition} className="grid gap-4">
        ...
        {["CANCEL", "REVERSE"].includes(actionTarget.action) ? <Field label={t("decisionReasonComment")}><textarea name="reason" minLength={4} rows={4} required disabled={busy} className="w-full rounded-xl border border-dtsc-border bg-dtsc-surface px-3 py-2 disabled:opacity-60" /></Field> : <Field label={t("decisionReasonComment")}><textarea name="reason" rows={3} disabled={busy} className="w-full rounded-xl border border-dtsc-border bg-dtsc-surface px-3 py-2 disabled:opacity-60" /></Field>}
