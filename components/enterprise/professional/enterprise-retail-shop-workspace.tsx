...
type CatalogItem = { id: string; code: string; sku: string | null; name: string; itemType: string; indicativeSalePrice: string | number | null; indicativeCost: string | number | null; currency: string | null; trackInventory: boolean; inventoryItemId?: string | null; allowNegativeStock?: boolean; availableQuantity?: string | number | null };
...
function moneyValue(value: string | number | null | undefined, currency?: string) {
  ...
  const browserLocale = typeof document !== "undefined" && document.documentElement.lang.toLowerCase().startsWith("en") ? "en-US" : "fr-FR";
  return `${Number.isFinite(amount) ? amount.toLocaleString(browserLocale, { maximumFractionDigits: 2 }) : "0"}${currency ? ` ${currency}` : ""}`;
}
...
function statusLabel(status: string, locale: "fr" | "en") {
  const labels: Record<string, { fr: string; en: string }> = {
    COMPLETED: { fr: "Terminée", en: "Completed" }, CONFIRMED: { fr: "Confirmée", en: "Confirmed" }, SUCCESS: { fr: "Réussie", en: "Success" }, APPROVED: { fr: "Approuvée", en: "Approved" }, CLOSED: { fr: "Fermée", en: "Closed" }, OPEN: { fr: "Ouverte", en: "Open" }, SUBMITTED: { fr: "Soumise", en: "Submitted" }, PENDING_VALIDATION: { fr: "En validation", en: "Pending validation" }, CLOSING: { fr: "En clôture", en: "Closing" }, REVERSED: { fr: "Annulée", en: "Reversed" }, FAILED: { fr: "Échouée", en: "Failed" }, REJECTED: { fr: "Refusée", en: "Rejected" },
  };
...
function PosPanel({ organizationId, dashboard, locale, busyAction, mutate }: { organizationId: string; dashboard: Dashboard; locale: "fr" | "en"; busyAction: string | null; mutate: Mutate }) {
  ...
  useEffect(() => {
    if (!warehouseId) { setProducts([]); setSearchError(""); return; }
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!response.ok || !body) throw new Error(body?.message || body?.error || (locale === "en" ? "Product search failed." : "Recherche produit impossible."));
        setProducts(body.items || []);
      } catch (searchFailure) {
        if (controller.signal.aborted) return;
        setProducts([]);
        setSearchError(searchFailure instanceof Error ? searchFailure.message : (locale === "en" ? "Product search failed." : "Recherche produit impossible."));
      } finally {
  ...
  function addItem(item: CatalogItem) {
    ...
    setCart((current) => {
      ...
      return [...current, { catalogItemId: item.id, name: item.name, quantity: 1, unitPrice: Number(item.indicativeSalePrice || 0), discountAmount: 0, taxAmount: 0, currencyCode: itemCurrency, inventoryItemId: item.inventoryItemId || null }];
    });
  }
  ...
  return <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-5">
    ...
    <ModuleSection title={locale === "en" ? "Counter sale" : "Vente comptoir"} description={locale === "en" ? "Search products on the server, build one multi-item basket, then collect payment." : "Recherchez les articles côté serveur, construisez un panier multi-articles, puis encaissez en une seule fois."}>
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
        <div className="min-w-0 rounded-2xl border border-dtsc-border bg-dtsc-page p-4">
          ...
          <div className="mt-4 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-2 sm:grid-cols-2">{searchLoading ? <div className="sm:col-span-2"><ProfessionalLoading rows={3} /></div> : products.map((item) => { const stock = available(item); return <button key={item.id} type="button" disabled={Boolean(busyAction) || !warehouseId || (item.trackInventory && !item.allowNegativeStock && (stock === null || stock <= 0))} onClick={() => addItem(item)} className="min-w-0 rounded-xl border border-dtsc-border bg-dtsc-surface p-3 text-left transition hover:border-cyan-400 disabled:opacity-50"><p className="break-words font-black text-dtsc-ink">{item.name}</p><p className="mt-1 text-xs font-semibold text-dtsc-muted">{item.sku || item.code} · {moneyValue(item.indicativeSalePrice, item.currency || currency)}</p><p className="mt-1 text-xs font-bold text-dtsc-muted">{item.trackInventory ? `${locale === "en" ? "Available" : "Disponible"}: ${stock ?? "—"}${item.allowNegativeStock ? ` · ${locale === "en" ? "negative allowed" : "négatif autorisé"}` : ""}` : (locale === "en" ? "Service / no stock" : "Service / sans stock")}</p></button>; })}{!searchLoading && !products.length && !searchError ? <div className="sm:col-span-2"><EmptyState compact title={locale === "en" ? "No product" : "Aucun produit"} description={locale === "en" ? "Try another name, SKU or code." : "Essayez un autre nom, SKU ou code."} /></div> : null}</div>
        </div>
