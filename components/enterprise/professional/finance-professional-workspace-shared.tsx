...
type FinanceCollectionPayload<T extends FinanceRecord> = {
  items: T[];
...
export type FinanceLookupParty = {
  id: string;
  code?: string;
  legalName: string;
...
export type FinanceOperationalLookups = {
  parties: FinanceLookupParty[];
  suppliers: FinanceLookupSupplier[];
  members: FinanceLookupMember[];
  sites: FinanceLookupSite[];
  employees: FinanceLookupEmployee[];
...
export type FinanceAccountLookup = {
  id: string;
  code: string;
  name: string;
  accountType: string;
...
export type BankStatementLookup = {
  id: string;
  reference: string;
  currencyCode: string;
  statementDate: string;
...
const EMPTY_LOOKUPS: FinanceOperationalLookups = {
  parties: [],
  suppliers: [],
  members: [],
  sites: [],
...
export function useFinanceCollection<T extends FinanceRecord>({
  endpoint,
  page,
  pageSize = 25,
  search,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !body?.items || !body.pagination) {
        throw new Error(body?.message || body?.error || "Chargement des données financières impossible.");
      }
      setItems(body.items);
      ...
      setItems([]);
      setError(safeFinanceError(loadError, "Chargement des données financières impossible."));
    } finally {
      setLoading(false);
    }
  }, [endpoint, query]);
...
async function readCollection<T>(endpoint: string): Promise<T[]> {
  ...
  const body = await response.json().catch(() => null) as { items?: T[]; message?: string; error?: string } | null;
  if (!response.ok || !body) throw new Error(body?.message || body?.error || "Sélecteurs financiers indisponibles.");
  return body.items || [];
}
...
export function useFinanceLookups(organizationId: string, moduleCode: string, refreshKey: number) {
  ...
  useEffect(() => {
    let active = true;
    setError("");
    const operational = fetch(`/api/enterprise/${organizationId}/operational-lookups?module=${encodeURIComponent(moduleCode)}`, { cache: "no-store" })
      .then(async (response) => {
        const body = await response.json().catch(() => null) as FinanceOperationalLookups & { message?: string; error?: string } | null;
        if (!response.ok || !body) throw new Error(body?.message || body?.error || "Référentiels métier indisponibles.");
        return body;
      });
    const work: Array<Promise<unknown>> = [operational];
    work.push(readCollection<FinanceAccountLookup>(`/api/enterprise/${organizationId}/financial-accounts?page=1&pageSize=200&status=ACTIVE`));
    if (moduleCode === "FINANCE_TREASURY") {
      work.push(readCollection<LedgerAccountLookup>(`/api/enterprise/${organizationId}/ledger-accounts?page=1&pageSize=500&status=ACTIVE`));
    } else {
    ...
    if (moduleCode === "FINANCE_PAYMENTS") {
      ...
      work.push(Promise.resolve([]), Promise.resolve([]));
    }
    ...
    void Promise.all(work).then(([operationalBody, accountItems, ledgerItems, receivableItems, payableItems, paymentItems, statementItems]) => {
      if (!active) return;
      setLookups(operationalBody as FinanceOperationalLookups);
      setAccounts(accountItems as FinanceAccountLookup[]);
      setLedgerAccounts(ledgerItems as LedgerAccountLookup[]);
      setReceivables(receivableItems as OpenBalanceLookup[]);
      setPayables(payableItems as OpenBalanceLookup[]);
      setPayments(paymentItems as FinanceRecord[]);
      setBankStatements(statementItems as BankStatementLookup[]);
    }).catch((lookupError) => {
      if (active) setError(safeFinanceError(lookupError, "Les sélecteurs financiers sont indisponibles."));
    });
    return () => { active = false; };
...
export async function financeMutation(endpoint: string, payload: unknown, method: "POST" | "PATCH" | "DELETE" = "POST") {
  ...
  if (!response.ok) throw new Error(body?.message || body?.error || "L’opération financière n’a pas pu être terminée.");
  return body || {};
}
...
export function financeRecordDate(item: FinanceRecord, locale: FinanceLocale) {
  return financeDate(item.invoiceDate || item.paymentDate || item.transferDate || item.statementDate || item.openedAt || item.createdAt || item.updatedAt, locale);
}
...
export function FinanceRecordList<T extends FinanceRecord>({
  items,
  locale,
  emptyTitle,
  emptyDescription,
  ...
  return (
    <BusinessList ariaLabel={emptyTitle}>
      {items.map((item) => (
        <BusinessListItem
          key={item.id}
          title={financeRecordTitle(item, locale)}
          meta={`${financeRecordDate(item, locale)}${financeRecordAmount(item, locale) ? ` · ${financeRecordAmount(item, locale)}` : ""}`}
          description={financeRecordDescription(item, locale)}
          status={item.status ? <StatusBadge tone={financeStatusTone(item.status)}>{financeStatusLabel(item.status, locale)}</StatusBadge> : undefined}
          onOpen={() => onOpen(item)}
          openLabel={`${locale === "fr" ? "Ouvrir" : "Open"} ${financeRecordTitle(item, locale)}`}
          actions={actions ? actions(item) : <Button size="sm" variant="outline" onClick={() => onOpen(item)}><Eye className="h-4 w-4" />{locale === "fr" ? "Détail" : "Details"}</Button>}
        />
      ))}
    </BusinessList>
  );
...
export function FinancePaginationControls({
  pagination,
  page,
  onPage,
  locale,
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-dtsc-border pt-3 text-sm text-dtsc-muted">
      <span>{pagination.total} {locale === "fr" ? "élément(s)" : "item(s)"} · Page {pagination.page}/{pagination.pageCount}</span>
      <div className="flex gap-2">
        <Button variant="outline" disabled={page <= 1} onClick={() => onPage(Math.max(1, page - 1))}>{locale === "fr" ? "Précédent" : "Previous"}</Button>
        <Button variant="outline" disabled={page >= pagination.pageCount} onClick={() => onPage(Math.min(pagination.pageCount, page + 1))}>{locale === "fr" ? "Suivant" : "Next"}</Button>
      </div>
    </div>
  );
}

export function FinanceDetailGrid({ children }: { children: ReactNode }) {
  return <dl className="grid gap-3 rounded-xl border-y border-dtsc-border py-4 sm:grid-cols-2 lg:grid-cols-3">{children}</dl>;
}

export function FinanceDetailValue({ label, children }: { label: string; children: ReactNode }) {
  return <div className="min-w-0"><dt className="text-xs font-black uppercase text-dtsc-muted">{label}</dt><dd className="mt-1 break-words text-sm font-semibold text-dtsc-ink">{children}</dd></div>;
}

const ENTITY_TYPE_BY_MODULE: Record<string, string> = {
  FINANCE_RECEIVABLES: "EnterpriseSalesInvoice",
  FINANCE_PAYABLES: "EnterpriseSupplierInvoice",
  FINANCE_PAYMENTS: "EnterprisePayment",
  FINANCE_TREASURY: "EnterpriseFinancialAccount",
  FINANCE_CASH: "EnterpriseCashSession",
  FINANCE_BANK: "EnterpriseBankStatement",
  FINANCE_RECONCILIATION: "EnterpriseReconciliationSession",
};

export function FinanceCollaboration({
  organizationId,
  moduleCode,
  record,
  locale,
  ...
  return (
    <>
      <section className="border-t border-dtsc-border pt-5">
        <h3 className="font-black text-dtsc-ink">{locale === "fr" ? "Documents financiers" : "Financial documents"}</h3>
        <p className="mt-1 text-sm leading-6 text-dtsc-muted">
          {locale === "fr"
            ? "Les justificatifs sont téléversés dans le stockage privé commun, versionnés et liés à cette opération."
            : "Supporting documents are uploaded to shared private storage, versioned and linked to this operation."}
        </p>
        <Link
          className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-xl border border-dtsc-border px-4 text-sm font-black text-dtsc-blue"
          href={`/enterprise-modules/DOCUMENTS?sourceEntityType=${encodeURIComponent(entityType)}&sourceEntityId=${encodeURIComponent(record.id)}&sourceReference=${encodeURIComponent(sourceReference)}&action=upload`}
        >
          <FileUp className="h-4 w-4" />{locale === "fr" ? "Téléverser ou ouvrir les documents liés" : "Upload or open linked documents"}
        </Link>
      </section>
      <ProfessionalWorkflowComments
        ...
        title={locale === "fr" ? "Commentaires financiers" : "Finance comments"}
        description={locale === "fr"
          ? "Les décisions structurées restent dans le workflow ; ce fil sert aux précisions, demandes de correction et justifications."
          : "Structured decisions remain in the workflow; this thread is for clarifications, correction requests and explanations."}
      />
    </>
  );
}
