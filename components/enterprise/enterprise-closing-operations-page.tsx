...
import { ClosingOperationsPanel } from "@/components/enterprise/professional/closing-operations-panel";
...
export async function EnterpriseClosingOperationsPage() {
  ...
  const [capabilities, membership, organization] = await Promise.all([
    resolveEnterpriseModuleCapabilities({ userId: user.id, organizationId, moduleCode: "FINANCE_CLOSE" }),
      ...
      <main className="mx-auto w-full max-w-[1600px] space-y-5 px-4 pb-10 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-dtsc-border pb-4">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 text-2xl font-black text-dtsc-ink">{locale === "en" ? "Closing operations" : "Opérations de clôture"}</h1>
          </div>
          <Button asChild variant="outline">
            <Link href="/enterprise-modules/FINANCE_CLOSE"><ArrowLeft className="h-4 w-4" />{locale === "en" ? "Back to Financial close" : "Retour à Clôture financière"}</Link>
          </Button>
        </div>
        <ClosingOperationsPanel organizationId={organizationId} locale={locale} canManage={capabilities.canManage} />
      </main>
    </AppShell>
  );
}
