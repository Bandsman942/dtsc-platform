...
const textFields = [
  "name", "genericName", "internalCode", "barcode", "manufacturer", "brand", "shortDescription", "subcategory", "dosage", "packaging",
  "saleWarningMessage", "defaultLocation", "shelf", "storageNotes", "deactivationReason", "notes",
] as const;
...
const booleanFields = ["prescriptionRequired", "pharmacistValidationRequired", "controlledProduct", "otcAllowed", "genericSubstitutionAllowed", "stockTrackingEnabled", "lightSensitive", "humiditySensitive", "refrigerated", "priceEditableAtSale", "discountAllowed"] as const;
...
function Text({ form, field, labelText, change, required = false }: { form: ProductForm; field: typeof textFields[number]; labelText: string; change: (key: string, value: string | boolean) => void; required?: boolean }) { return <Field labelText={labelText}><Input value={String(form[field])} onChange={(event) => change(field, event.target.value)} required={required} /></Field>; }
...
function Checks({ form, fields, change }: { form: ProductForm; fields: Array<typeof booleanFields[number]>; change: (key: string, value: string | boolean) => void }) { return <div className="flex flex-wrap gap-3">{fields.map((field) => <label key={field} className="flex items-center gap-2 text-sm font-bold text-dtsc-ink"><input type="checkbox" checked={form[field] === true} onChange={(event) => change(field, event.target.checked)} />{field}</label>)}</div>; }
