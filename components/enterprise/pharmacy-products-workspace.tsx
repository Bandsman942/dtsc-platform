...
function Section({ title, children }: { title: string; children: ReactNode }) { return <section className="min-w-0 max-w-full space-y-3 overflow-hidden rounded-2xl border border-dtsc-border bg-dtsc-page p-3 sm:p-4"><h3 className="break-words text-sm font-black uppercase tracking-[0.14em] text-emerald-600">{title}</h3>{children}</section>; }
...
function Field({ labelText, hint, children }: { labelText: string; hint?: string; children: ReactNode }) { return <label className="grid min-w-0 max-w-full gap-1 text-sm font-black text-dtsc-ink"><span className="flex min-w-0 items-center gap-1 text-xs uppercase tracking-[0.12em] text-dtsc-muted"><span className="min-w-0 break-words">{labelText}</span>{hint && <span tabIndex={0} title={hint} aria-label={`${labelText} : ${hint}`} className="shrink-0 cursor-help"><CircleHelp className="h-3.5 w-3.5" /></span>}</span>{children}</label>; }
function Text({ form, field, labelText, change, required = false }: { form: ProductForm; field: TextField; labelText: string; change: (key: string, value: string | boolean) => void; required?: boolean }) { return <Field labelText={labelText} hint={FIELD_HELP[field]?.hint}><Input value={String(form[field])} title={FIELD_HELP[field]?.hint} onChange={(event) => change(field, event.target.value)} required={required} className="w-full min-w-0 max-w-full" /></Field>; }
function NumberField({ form, field, labelText, change }: { form: ProductForm; field: typeof numberFields[number]; labelText?: string; change: (key: string, value: string | boolean) => void }) { const meta = FIELD_HELP[field]; return <Field labelText={labelText || meta.label} hint={meta.hint}><Input type="number" step="any" value={String(form[field])} title={meta.hint} onChange={(event) => change(field, event.target.value)} className="w-full min-w-0 max-w-full" /></Field>; }
function Choice({ form, field, labelText, options, change }: { form: ProductForm; field: string; labelText: string; options: readonly string[]; change: (key: string, value: string | boolean) => void }) { const hint = FIELD_HELP[field]?.hint; return <Field labelText={labelText} hint={hint}><select value={String(form[field])} title={hint} onChange={(event) => change(field, event.target.value)} className="h-11 w-full min-w-0 max-w-full truncate rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-dtsc-ink">{options.map((option) => <option key={option} value={option}>{label(option)}</option>)}</select></Field>; }
function Checks({ form, fields, change }: { form: ProductForm; fields: BooleanField[]; change: (key: string, value: string | boolean) => void }) { return <div className="flex min-w-0 flex-wrap gap-3">{fields.map((field) => { const meta = FIELD_HELP[field]; return <label key={field} title={meta.hint} className="flex min-w-0 items-center gap-2 text-sm font-bold text-dtsc-ink"><input type="checkbox" className="shrink-0" checked={form[field] === true} onChange={(event) => change(field, event.target.checked)} /><span className="min-w-0 break-words">{meta.label}</span><span tabIndex={0} title={meta.hint} aria-label={`${meta.label} : ${meta.hint}`} className="shrink-0 cursor-help text-dtsc-muted"><CircleHelp className="h-3.5 w-3.5" /></span></label>; })}</div>; }
function Filter({ value, onChange, options, label: placeholder }: { value: string; onChange: (value: string) => void; options: readonly string[]; label: string }) { return <select value={value} onChange={(event) => onChange(event.target.value)} className="h-11 w-full min-w-0 max-w-full truncate rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm font-bold text-dtsc-ink"><option value="">{placeholder}</option>{options.map((option) => <option key={option} value={option}>{label(option)}</option>)}</select>; }
