      ...
      {definition.code === "FINANCE_OVERVIEW" && canManage ? (
        <section className="rounded-3xl border bg-card/70 p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <label className="text-sm">{language === "en" ? "Inventory valuation" : "Valorisation du stock"}<select value={valuationMethod} onChange={(event) => setValuationMethod(event.target.value)} className="mt-1 h-10 w-full rounded-md border bg-background px-3"><option value="WEIGHTED_AVERAGE">Weighted average</option><option value="FIFO">FIFO</option></select></label>
      ...
      {definition.code === "FINANCE_STATEMENTS" && canManage ? (
        <section className="rounded-3xl border bg-card/70 p-5 sm:p-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <label className="text-sm">Type<select value={statementType} onChange={(event) => setStatementType(event.target.value)} className="mt-1 h-10 w-full rounded-md border bg-background px-3"><option value="TRIAL_BALANCE">Trial balance</option><option value="GENERAL_LEDGER">General ledger</option><option value="JOURNALS">Journals</option><option value="INCOME_STATEMENT">Income statement</option><option value="BALANCE_SHEET">Balance sheet</option><option value="CASH_FLOW">Cash flow</option><option value="TREASURY">Treasury statement</option><option value="AR_AGING">Aged receivables</option><option value="AP_AGING">Aged payables</option><option value="BUDGET_VS_ACTUAL">Budget vs actual</option><option value="TAX">Tax summary</option><option value="ASSET_REGISTER">Asset register</option><option value="INVENTORY_VALUATION">Inventory valuation</option></select></label>
          ...
          <label className="mt-4 flex items-center gap-2 text-sm"><input type="checkbox" checked={publishStatement} onChange={(event) => setPublishStatement(event.target.checked)} />{language === "en" ? "Publish immutable snapshot" : "Publier un snapshot immuable"}</label>
