  ...
  return (
    <div className="space-y-5">
      <div className="dtsc-glass-card overflow-hidden rounded-[1.6rem] p-5">
        {!paymentAvailable && (
          <div className="mb-4 flex items-start gap-3 rounded-2xl border border-amber-300/45 bg-amber-300/10 p-4 text-sm leading-6 text-dtsc-ink backdrop-blur-xl">
            ...
            <div>
              ...
              <p className="font-medium text-dtsc-ink">
        ...
        <p className="mt-2 text-xs font-medium leading-5 text-dtsc-ink">
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        {plans.map((plan) => {
          ...
          return (
            <article
              key={plan.id}
              className={`relative flex min-h-full min-w-0 flex-col overflow-hidden rounded-[1.6rem] border p-5 shadow-[0_18px_55px_rgba(0,43,91,0.10)] transition hover:-translate-y-1 ${
                active
            ...
            >
              ...
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[0.68rem] font-black uppercase tracking-[0.12em] ${active ? "bg-white/10 text-cyan-100" : "bg-dtsc-soft text-dtsc-blue"}`}>
                    <Sparkles className="h-3.5 w-3.5" />
                    {active ? "Plan actif" : "Offre DTSC"}
                  </span>
                  <h2 className={`mt-3 break-normal text-xl font-black leading-tight ${active ? "text-white" : "text-dtsc-ink"}`}>{plan.name}</h2>
                  <p className={`mt-3 text-sm font-medium leading-6 ${active ? "text-slate-100" : "text-dtsc-ink"}`}>{plan.description}</p>
                </div>
                {active && <CheckCircle2 className="h-5 w-5 shrink-0 text-cyan-300" />}
              </div>
