...
export function CalendarAdvancedToolsPanel({
  ...
  async function refreshResources() {
    ...
    else toastError(body?.message || "Impossible d’actualiser les ressources du calendrier.");
  }
  ...
  async function createResource(event: FormEvent<HTMLFormElement>) {
    ...
    } else toastError(body?.message || "La ressource n’a pas pu être créée.");
    setSaving(false);
  }
  ...
  async function archiveResource(resource: CalendarResourceItem) {
    const confirmation = await confirmSensitiveAction({
      ...
      description: `La ressource « ${resource.name} » ne sera plus proposée et ses réservations encore actives seront annulées.`,
      confirmLabel: "Archiver la ressource",
      tone: "danger",
    });
    ...
    const body = (await response.json().catch(() => null)) as { message?: string } | null;
    if (response.ok) {
      toastSuccess("Ressource archivée.");
      await refreshResources();
    } else toastError(body?.message || "La ressource n’a pas pu être archivée.");
  }
  ...
  async function reserveResource(resourceId: string, eventId: string) {
    ...
    const body = (await response.json().catch(() => null)) as { message?: string } | null;
    if (response.ok) {
      toastSuccess("Ressource réservée pour l’événement.");
      await refreshResources();
    } else toastError(body?.message || "La réservation n’a pas pu être enregistrée.");
  }
  ...
  async function requestSuggestions(event: FormEvent<HTMLFormElement>) {
    ...
    if (response.ok) {
      ...
      toastInfo(body?.suggestions?.length ? "Créneaux compatibles proposés." : "Aucun créneau compatible trouvé dans cette période.");
    } else toastError(body?.message || "La recherche de créneaux n’a pas pu être terminée.");
    setSaving(false);
  }
  ...
  async function requestExternalConnection(provider: "GOOGLE" | "MICROSOFT") {
    ...
    const nextMessage = body?.message || (response.ok ? `État : ${body?.state?.status || "préparé"}` : featureStatuses.externalCalendar.message);
    ...
    <section className="dtsc-card min-w-0 overflow-hidden p-4 sm:p-6">
      <div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-2xl font-black text-dtsc-ink">Créneaux, ressources et calendriers connectés</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-dtsc-muted">Gérez les ressources internes et recherchez des créneaux compatibles. Les calendriers externes restent disponibles uniquement lorsque la connexion correspondante est activée.</p>
        </div>
      ...
      <div className="mt-5 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-dtsc-border bg-dtsc-page p-4">
          ...
          <p className="mt-4 flex items-start gap-2 text-xs leading-5 text-dtsc-muted"><Shapes className="mt-0.5 h-4 w-4 shrink-0" />La synchronisation respecte les invitations et les droits du calendrier interne.</p>
        </section>
      </div>
      ...
      <Dialog open={suggestionDialog} title="Proposer automatiquement des créneaux" description="DTSC analyse les conflits et disponibilités des participants sur une période de 14 jours maximum." onClose={() => setSuggestionDialog(false)} className="h-[94dvh] max-w-4xl">
