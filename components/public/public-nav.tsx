...
export function PublicNav() {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <div className="hidden items-center gap-2 md:flex"><PublicSearchDialog compact /><ProductPreferencesControls /><Link href={getSignInUrl("/dashboard")} className="rounded-xl border border-dtsc-border px-3 py-2 text-sm font-semibold text-dtsc-blue transition hover:bg-dtsc-soft">Accès client</Link><Link href="/contact" className="rounded-xl bg-dtsc-blue px-4 py-2 text-sm font-semibold text-white transition hover:bg-[var(--dtsc-brand-secondary-hover)]">Consultation</Link></div>
      <button type="button" onClick={() => setOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm font-semibold text-dtsc-blue shadow-sm md:hidden" aria-expanded={open} aria-controls="public-mobile-navigation"><Menu className="h-4 w-4" /> Menu</button>
      {open ? (
