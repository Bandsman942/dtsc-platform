...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isSignUp && body?.otpRequired) {
      ...
      toastInfo("Un code OTP vient d’être envoyé à votre adresse email.", "Vérification email");
      return;
    }
    ...
    <form onSubmit={onSubmit} className="space-y-5" noValidate>
      ...
      {isSignUp && !pendingRegistration ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="grid gap-2 text-sm font-semibold text-dtsc-ink sm:col-span-2">Nom complet<Input name="name" autoComplete="name" required /></label>
          <label className="grid gap-2 text-sm font-semibold text-dtsc-ink">Organisation <span className="sr-only">optionnelle</span><Input name="companyName" autoComplete="organization" /></label>
          <label className="grid gap-2 text-sm font-semibold text-dtsc-ink">Téléphone <span className="sr-only">optionnel</span><Input name="phone" autoComplete="tel" /></label>
        </div>
      ) : null}
      ...
      {pendingRegistration ? (
        <>
          <div className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 p-4">
            <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-cyan-500" /><div><p className="font-semibold text-dtsc-ink">Vérification email</p><p className="mt-1 text-sm leading-6 text-dtsc-muted">Entrez le code à 6 chiffres envoyé à {String(pendingRegistration.email)}.</p>{otpExpiresAt ? <p className="mt-2 text-xs text-dtsc-muted">Le code expire prochainement.</p> : null}</div></div>
          </div>
          <label className="grid gap-2 text-sm font-semibold text-dtsc-ink">Code OTP<Input name="otp" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} autoComplete="one-time-code" required /></label>
        </>
      ) : (
        <>
          <label className="grid gap-2 text-sm font-semibold text-dtsc-ink">Email professionnel<Input name="email" type="email" autoComplete="email" required value={emailValue} onChange={(event) => setEmailValue(event.target.value)} /></label>
          <label className="grid gap-2 text-sm font-semibold text-dtsc-ink">Mot de passe<PasswordInput name="password" autoComplete={isSignUp ? "new-password" : "current-password"} required value={passwordValue} onChange={(event) => setPasswordValue(event.target.value)} /></label>
          {isSignUp ? (
            <>
              <label className="grid gap-2 text-sm font-semibold text-dtsc-ink">Confirmer le mot de passe<PasswordInput name="confirmPassword" autoComplete="new-password" required /></label>
              <label className="flex items-start gap-3 text-sm leading-6 text-dtsc-muted"><input name="legalConsent" type="checkbox" required className="mt-1 h-4 w-4 rounded border-dtsc-border" /><span>J’accepte les conditions d’utilisation et la politique de confidentialité applicables.</span></label>
            </>
          ) : (
            <div className="space-y-3 rounded-2xl border border-dtsc-border bg-dtsc-page p-4">
              ...
              {organizations.length ? <select name="organizationId" value={selectedOrganizationId} onChange={(event) => setSelectedOrganizationId(event.target.value)} className="h-11 w-full rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm font-semibold text-dtsc-ink"><option value="">Espace personnel</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name} · {organization.role}</option>)}</select> : <input type="hidden" name="organizationId" value="" />}
              {pendingInvitations.length ? <p className="text-xs leading-5 text-dtsc-blue">Invitation en attente : {pendingInvitations.map((item) => item.name).join(", ")}.</p> : null}
            </div>
          )}
        </>
      )}

      <Button type="submit" className="h-11 w-full rounded-xl bg-dtsc-blue text-white shadow-[var(--dtsc-shadow-md)] hover:bg-[var(--dtsc-brand-secondary-hover)]" disabled={isPending}>
        {isPending ? "Traitement…" : pendingRegistration ? "Vérifier et créer le compte" : isSignUp ? "Créer mon compte" : "Se connecter"}<ArrowRight className="h-4 w-4" />
      </Button>
