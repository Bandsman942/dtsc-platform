...
export function ActivityDetail({
  ...
  const loadComments = useCallback(async (cursor?: string | null) => {
    if (cursor) setIsLoadingOlderComments(true);
  ...
  return (
    <div className="min-w-0 space-y-5">
      <div className="min-w-0">
        ...
        {item.entityType === "COLLAB_REQUEST" ? <CollaboratorRequestConversation item={item} response={visibleRequestResponse} /> : item.body ? <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-dtsc-muted">{item.body}</p> : null}
        {item.href ? <a href={item.href} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-xl bg-dtsc-blue px-4 py-2 text-sm font-black text-white transition hover:opacity-90">{item.hrefLabel || "Ouvrir le document"}</a> : null}
      </div>
      ...
      {item.entityType === "COLLAB_REQUEST" && item.canRespond ? (
        <div className="border-y border-dtsc-border py-4">
          ...
          <textarea value={requestResponse} onChange={(event) => setRequestResponse(event.target.value)} placeholder="Réponse, précision ou information transmise..." className="mt-3 min-h-24 w-full rounded-xl border border-dtsc-border bg-dtsc-page px-3 py-2 text-sm text-dtsc-ink" />
      ...
      <CollapsibleThread count={comments.length} label="commentaire(s)" defaultOpen={false} forceOpen={Boolean(replyingTo || editingComment || deletingComment)} className="border-t border-dtsc-border pt-4">
        <div ref={commentsThreadRef} className="max-h-80 space-y-3 overflow-y-auto overscroll-contain pr-1">
          {hasOlderComments ? (
            <div className="flex justify-center">
              <Button type="button" variant="outline" size="sm" onClick={() => void loadComments(commentsCursor)} disabled={isLoadingOlderComments} className="rounded-xl border-dtsc-border bg-dtsc-surface text-dtsc-blue">{isLoadingOlderComments ? "Chargement..." : "Charger les précédents"}</Button>
            </div>
          ) : null}
          ...
          {comments.length === 0 ? <p className="text-sm text-dtsc-muted">Aucun commentaire pour le moment.</p> : null}
        </div>
        {replyingTo ? (
        ...
        <form onSubmit={addComment} className="relative mt-3 flex flex-col gap-2 sm:flex-row">
          {mentionSuggestions.length > 0 ? (
            <div className="absolute bottom-14 left-0 z-20 w-[min(26rem,100%)] rounded-2xl border border-dtsc-border bg-dtsc-surface p-2 shadow-[0_18px_60px_rgba(0,23,54,0.18)]">
              {mentionSuggestions.map((collaborator) => <button key={collaborator.id} type="button" onClick={() => insertMention(collaborator)} className="block w-full rounded-xl px-3 py-2 text-left text-sm font-bold text-dtsc-ink hover:bg-dtsc-soft">@{collaborator.label}</button>)}
            </div>
          ) : null}
  ...
  return (
    <div className={`grid gap-2 ${className}`}>
      ...
      {attachments.map((attachment) => {
        ...
        return (
          <div key={`${attachment.url}-${attachment.name}`} className="flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-dtsc-border py-3 last:border-b-0">
            <span className="flex min-w-0 items-center gap-2 text-sm font-bold text-dtsc-ink"><FileText className="h-4 w-4 shrink-0 text-cyan-500" /><span className="min-w-0 truncate">{attachment.name}</span><span className="shrink-0 text-xs text-dtsc-muted">{formatFileSize(attachment.size)}</span></span>
            <span className="flex flex-wrap gap-2">
              {canPreviewImage || canPreviewPdf ? <Button type="button" size="sm" variant="outline" onClick={() => setPreview(attachment)} className="rounded-xl border-dtsc-border bg-dtsc-surface text-dtsc-blue"><Eye className="h-4 w-4" /> Aperçu</Button> : null}
      ...
      <Dialog open={Boolean(preview)} title={preview ? `Aperçu: ${preview.name}` : "Aperçu"} onClose={() => setPreview(null)} className="max-w-5xl">
        {preview && (preview.type?.startsWith("image/") || /\.(png|jpe?g|webp)(\?|$)/i.test(preview.url)) ? <img src={preview.url} alt={preview.name} className="mx-auto max-h-[70vh] max-w-full rounded-2xl border border-dtsc-border object-contain" /> : preview ? <iframe src={preview.url} title={preview.name} className="h-[70vh] w-full rounded-2xl border border-dtsc-border bg-white" /> : null}
      </Dialog>
    </div>
  );
}
  ...
  return (
    <>
      {content.split(pattern).map((part, index) => {
        ...
        return <button key={`${mention.id}-${index}`} type="button" onClick={() => copyText(mention.name)} className="font-black text-cyan-600 underline decoration-cyan-300 underline-offset-4 transition hover:text-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-300" title="Copier le nom du collaborateur mentionné">@{mention.name}</button>;
      })}
    </>
  );
}
