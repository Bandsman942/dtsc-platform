...
export function CollaborationMeetingMessageContent({
  ...
  const t = useCallback((key: SharedWorkKey) => translateSharedWork(preferences.locale, key), [preferences.locale]);
      ...
      <div className="min-w-[min(19rem,78vw)] rounded-xl border border-cyan-400/35 bg-cyan-400/8 p-3 text-dtsc-ink">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-black uppercase tracking-wide text-cyan-700 dark:text-cyan-200">{t("collaboration.meeting.scheduled")}</p>
        ...
        >
          {joining ? t("collaboration.meeting.opening") : closed ? t("collaboration.meeting.ended") : meetingLink.canJoin ? t("collaboration.meeting.join") : t("collaboration.meeting.availableAtTime")}
        </Button>
      </div>
    );
  }
    ...
    return (
      <>
        <div className="min-w-[min(19rem,78vw)] rounded-xl border border-amber-400/35 bg-amber-400/8 p-3 text-dtsc-ink">
          <div className="flex items-start gap-2"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-amber-500/14 text-amber-700 dark:text-amber-200"><FileText className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-wide text-amber-700 dark:text-amber-200">{t("collaboration.meeting.followUp")}</p><p className="mt-1 whitespace-pre-wrap text-sm leading-5">{content}</p></div></div>
          {meetingFollowUp.canCreateMinutes ? <Button type="button" variant={alreadyCreated ? "outline" : "default"} className="mt-3 w-full" onClick={() => setMinutesOpen(true)}>{alreadyCreated ? t("collaboration.meeting.updateMinutes") : t("collaboration.meeting.writeMinutes")}</Button> : null}
        </div>
        <Dialog open={minutesOpen} title={t("collaboration.meeting.minutesTitle")} description={meetingFollowUp.meetingTitle || undefined} onClose={() => setMinutesOpen(false)} className="h-[92dvh] max-w-3xl">
          <form onSubmit={saveMinutes} className="grid gap-4">
            <label className="grid gap-1 text-sm font-bold text-dtsc-ink">{t("collaboration.meeting.detailedMinutes")}<textarea name="content" minLength={10} maxLength={10000} required defaultValue="" className="min-h-[45dvh] resize-y rounded-xl border border-dtsc-border bg-dtsc-page p-3 text-sm leading-6 text-dtsc-ink" placeholder={t("collaboration.meeting.minutesPlaceholder")} /></label>
            <label className="grid gap-1 text-sm font-bold text-dtsc-ink">{t("collaboration.meeting.summaryOptional")}<textarea name="summary" maxLength={2000} defaultValue={meetingFollowUp.summary || ""} className="min-h-28 resize-y rounded-xl border border-dtsc-border bg-dtsc-page p-3 text-sm leading-6 text-dtsc-ink" placeholder={t("collaboration.meeting.summaryPlaceholder")} /></label>
            <Button type="submit" disabled={savingMinutes}>{savingMinutes ? t("collaboration.meeting.saving") : t("collaboration.meeting.saveMinutes")}</Button>
          </form>
        </Dialog>
      </>
    );
  ...
  if (messageType === "MEETING_SUMMARY") {
    return <div className="min-w-[min(19rem,78vw)] rounded-xl border border-emerald-400/35 bg-emerald-400/8 p-3 text-dtsc-ink"><p className="text-xs font-black uppercase tracking-wide text-emerald-700 dark:text-emerald-200">{t("collaboration.meeting.summary")}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{content}</p></div>;
  }
  ...
  async function joinMeeting() {
    ...
    if (!response.ok || !body?.call?.id) {
      onError(body?.message || t("collaboration.meeting.joinError"));
      await onChanged();
      return;
    }
  ...
  async function saveMinutes(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      onError(body?.message || t("collaboration.meeting.saveError"));
      return;
    }
