...
import { OperationalSlaPanel } from "@/components/admin/operational-sla-panel";
...
export function AdminAccessPanel({ access }: { access: AdminRoleAccess }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = Object.fromEntries(configurableRoles.map((role) => [role, form.getAll(role).map(String)]));
      ...
      <section className="dtsc-card min-w-0 overflow-hidden p-4 sm:p-6">
        <div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <span className="rounded-2xl bg-cyan-400/10 p-3 text-cyan-300"><ShieldCheck className="h-5 w-5" /></span>
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-dtsc-muted">Le rôle ADMIN conserve toujours tous les blocs. Vous pouvez ouvrir uniquement les sections utiles aux rôles MANAGER et SUPPORT. Les permissions nominatives ci-dessous ajoutent ensuite une dérogation précise sans modifier le rôle global.</p>
            </div>
          </div>
              ...
              <div className="mt-4 grid min-w-0 gap-3">
                {adminBlocks.map((block) => (
                  <label key={`${role}-${block.id}`} title={block.description} className="flex min-w-0 items-start justify-between gap-4 rounded-xl border border-dtsc-border bg-dtsc-surface px-4 py-3 text-sm text-dtsc-ink">
                    <span className="min-w-0"><span className="block break-words font-black">{block.label}</span><span className="mt-1 block break-words text-xs leading-5 text-dtsc-muted">{block.description}</span></span>
                    <input name={role} value={block.id} type="checkbox" defaultChecked={access[role].includes(block.id)} className="mt-1 h-4 w-4 shrink-0 accent-cyan-500" />
                  </label>
                ))}
              </div>
            </div>
          ))}
          <div className="lg:col-span-2"><Button className="rounded-xl bg-[#002b5b] text-white hover:bg-[#001736]" title="Enregistrer les droits d'accès aux blocs Administration.">Enregistrer les droits RBAC</Button></div>
        </form>
      </section>

      <DtscIndividualPermissionsPanel />
      <OperationalSlaPanel />
    </div>
  );
}
