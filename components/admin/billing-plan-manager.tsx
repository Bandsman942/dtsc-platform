    ...
    <section className="dtsc-card min-w-0 overflow-hidden p-4 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-600">{english ? "DTSC offer catalog" : "Catalogue des offres DTSC"}</p>
          <h2 className="mt-1 text-xl font-black text-dtsc-ink">{english ? "ERP plans and included services" : "Offres ERP et services inclus"}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-dtsc-muted">
            {english
              ? "Each plan follows a clear progression from essential management to advanced accounting and regulated sector capabilities."
              : "Chaque offre suit une progression claire, des fondamentaux de gestion jusqu’à la comptabilité avancée et aux secteurs réglementés."}
          </p>
        </div>
          ...
          <article key={plan.id} className="min-w-0 rounded-2xl border border-dtsc-border bg-dtsc-page p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex shrink-0 items-start gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-black ${plan.isActive ? "bg-emerald-400/14 text-emerald-700 dark:text-emerald-300" : "bg-slate-400/16 text-slate-700 dark:text-slate-300"}`}>
                  {plan.isActive ? (english ? "Available" : "Disponible") : (english ? "Unavailable" : "Non commercialisée")}
                </span>
                {canManage && (
                  <ActionMenu
                    label={`${english ? "Offer actions" : "Actions de l’offre"} ${plan.name}`}
                    items={[{ key: "edit", label: english ? "Edit pricing" : "Modifier la tarification", icon: Edit3, onSelect: () => setEditingPlan(plan) }]}
                  />
                )}
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-dtsc-border bg-dtsc-surface p-3">
              <div className="flex items-center justify-between gap-3">
                <span className="flex items-center gap-2 text-sm font-black text-dtsc-ink"><Layers3 className="h-4 w-4 text-cyan-600" />{english ? "Included ERP modules" : "Modules ERP inclus"}</span>
      ...
      <Dialog open={Boolean(editingPlan)} title={english ? "Edit offer pricing" : "Modifier la tarification de l’offre"} description={editingPlan ? editingPlan.name : undefined} onClose={() => setEditingPlan(null)} className="h-[92dvh] max-w-4xl">
        {editingPlan && (
