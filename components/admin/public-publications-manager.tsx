...
export function PublicPublicationsManager({ publications }: { publications: Publication[] }) {
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <div>
          ...
          <form onSubmit={(event) => submit(event)} className="mt-5 grid gap-3">
            <Input name="title" placeholder="Titre de la publication" required />
            <Input name="slug" placeholder="slug-url-sans-accents" pattern="[a-z0-9]+(-[a-z0-9]+)*" required />
            <div className="grid gap-3 sm:grid-cols-2">
              <select name="category" className="h-10 rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm font-bold text-dtsc-ink">
                {categories.map((category) => (
                  <option key={category} value={category}>{category}</option>
                ))}
              </select>
              <Input name="coverLabel" placeholder="Label visuel court" />
            </div>
            <textarea name="excerpt" placeholder="Résumé court visible dans la carte publique" className="min-h-20 rounded-xl border border-dtsc-border bg-dtsc-surface px-3 py-2 text-sm text-dtsc-ink" required />
            <textarea name="content" placeholder="Contenu long de la publication" className="min-h-40 rounded-xl border border-dtsc-border bg-dtsc-surface px-3 py-2 text-sm text-dtsc-ink" required />
            <label className="flex items-center justify-between rounded-xl border border-dtsc-border bg-dtsc-page px-4 py-3 text-sm font-bold text-dtsc-ink">
              ...
              <input name="published" type="checkbox" className="h-4 w-4 accent-cyan-500" />
            </label>
            ...
            <article key={publication.id} className="rounded-2xl border border-dtsc-border bg-dtsc-page p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => setEditing(publication)} className="rounded-xl">
                    <Edit3 className="h-4 w-4" />
                  </Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => remove(publication)} className="rounded-xl text-red-400">
