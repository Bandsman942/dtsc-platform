...
export function OperationalSlaPanel() {
  ...
  const load = useCallback(async () => {
    ...
    } else setMessage(body?.message || "Chargement des SLA impossible.");
    setLoading(false);
  }, []);
  ...
  async function createPolicy(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(response.ok ? "Politique SLA créée." : body?.message || "Création impossible.");
    if (response.ok) {
  ...
  async function evaluate() {
    ...
    setMessage(response.ok ? `${body?.results?.length || 0} instance(s) SLA évaluée(s).` : body?.message || "Évaluation impossible.");
    if (response.ok) await load();
    setSaving(false);
  }

  async function archive(policyId: string) {
    if (!window.confirm("Archiver cette politique et clôturer ses instances actives ?")) return;
    setSaving(true);
    const response = await fetch("/api/operations/sla", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "ARCHIVE_POLICY", policyId }) });
    const body = (await response.json().catch(() => null)) as { message?: string } | null;
    setMessage(response.ok ? "Politique SLA archivée." : body?.message || "Archivage impossible.");
    if (response.ok) await load();
    setSaving(false);
  }
  ...
  return (
    <section className="dtsc-card min-w-0 overflow-hidden p-4 sm:p-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <Metric label="Avertissement" value={metrics.warning} icon={ShieldAlert} />
      ...
      <div className="mt-6 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        <form onSubmit={createPolicy} className="grid gap-3 rounded-2xl border border-dtsc-border bg-dtsc-page p-4">
          <h3 className="font-black text-dtsc-ink">Nouvelle politique SLA</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <FormField label="Type d'objet" hint="Domaine auquel appliquer la règle."><select name="objectType" className="h-12 rounded-xl border border-dtsc-border bg-dtsc-surface px-3 text-sm text-dtsc-ink">{OBJECT_TYPES.map((value) => <option key={value}>{value}</option>)}</select></FormField>
            ...
            <FormField label="Statut de départ" hint="Statut qui déclenche le SLA."><Input name="startStatus" maxLength={80} placeholder="SUBMITTED" className="h-12 rounded-xl bg-dtsc-surface" /></FormField>
            <FormField label="Statuts d'arrêt" hint="Séparés par des virgules."><Input name="stopStatuses" placeholder="COMPLETED, CLOSED" className="h-12 rounded-xl bg-dtsc-surface" /></FormField>
          ...
          <Button type="submit" disabled={saving} className="rounded-xl bg-dtsc-blue text-white"><Plus className="h-4 w-4" /> Créer la politique</Button>
        </form>
