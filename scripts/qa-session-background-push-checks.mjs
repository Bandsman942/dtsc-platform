...
check("notifications DB enqueue Web Push durable sans attendre le réseau", all(notifications, ["prisma.$transaction", "enqueueWebPushNotification", "buildWebPushDomainEventData"]) && !notifications.includes("dispatchPushForNotification") && all(pushSender, ["dispatchStoredPushNotification", "Promise.allSettled", "result.status === 404", "result.status === 410", "pushSubscription.deleteMany"]));
