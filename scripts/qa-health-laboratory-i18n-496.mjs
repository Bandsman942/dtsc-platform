...
const workspaceLabKeys = [...new Set([...workspace.matchAll(/["'`](lab\.[A-Za-z0-9_.-]+)["'`]/g)].map((match)=>match[1]))].sort();
const missingFrKeys = workspaceLabKeys.filter((key)=>!(key in fr));
const missingEnKeys = workspaceLabKeys.filter((key)=>!(key in en));
const foreignFrKeys = frKeys.filter((key)=>!key.startsWith("lab."));
const foreignEnKeys = enKeys.filter((key)=>!key.startsWith("lab."));
check("Laboratoire #496: catalogues FR/EN complets", frKeys.length > 0 && enKeys.length > 0 && missingFrKeys.length === 0 && missingEnKeys.length === 0 && foreignFrKeys.length === 0 && foreignEnKeys.length === 0);
...
if (failures.length) {
  if (missingFrKeys.length || missingEnKeys.length || foreignFrKeys.length || foreignEnKeys.length) {
    console.error("Laboratoire #496 catalogue diagnostics", { missingFrKeys, missingEnKeys, foreignFrKeys, foreignEnKeys });
  }
  console.error(`FAIL i18n Health Laboratory #496 — ${failures.length} contrôle(s) en échec.`);
  process.exit(1);
}
console.log(`PASS i18n Health Laboratory #496 — ${frKeys.length} clés FR/EN symétriques, ${workspaceLabKeys.length} clés lab.* référencées couvertes, copie système convergée et données cliniques/résultats préservés verbatim.`);
