...
check(
  "formulaires mobiles évitent le zoom Safari sans intercepter le geste natif de saisie",
  containsAll(mobileCss, ["font-size: 16px !important", "touch-action: manipulation", "touch-action: auto", "safe-area-inset-top"])
);

check(
  "dialog partagé est porté au body et ajuste sa hauteur sans rerendre le champ focalisé",
...
check(
  "dialog partagé ne repositionne plus son overlay avec un offset VisualViewport iOS potentiellement périmé",
  !dialog.includes("visualViewportBounds.offsetTop") && !dialog.includes('top: `${visualViewport')
);

check(
  "dialog partagé ne bloque pas artificiellement le scroll du body ni le focus natif",
  !dialog.includes('document.body.style.overflow = "hidden"') && !dialog.includes("preventDefault()") && !dialog.includes(".focus()")
);

check(
  "chrome mobile se suspend pendant saisie, select ou combobox",
...
check(
  "navigation basse ne dépend plus d'un transform/backdrop compositor pendant les transitions clavier",
  containsAll(mobileCss, ['[data-mobile-bottom-nav]', "backdrop-filter: none !important", "transform: none !important", 'html[data-dtsc-mobile-input="active"]'])
);
