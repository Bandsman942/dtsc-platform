...
export function runIteration08Audit(scope = "all") {
  ...
  if (enabled("design-system")) { requireText("app/design-system.css", ["--dtsc-brand-primary", "--dtsc-background", "--dtsc-focus", "prefers-reduced-motion", ".dtsc-display", ".dtsc-animated-gradient", ".dtsc-mobile-menu"]); requireText("docs/DTSC_FRONTEND_DESIGN_SYSTEM.md", ["Tokens sémantiques", "Typographie", "Motion"]); }
  if (enabled("brand-tokens")) forbidText("lib/dtsc.ts", ["DtscAdmin2026!", 'password: process.env.DEFAULT_ADMIN_PASSWORD ||']);
  if (enabled("public-layout")) { requireText("components/public/public-shell.tsx", ["PublicHeader", "PublicNav", "dtsc-public-header"]); requireText("components/public/public-search-dialog.tsx", ["role=\"dialog\"", "PublicSiteSearch"]); forbidText("components/public/public-shell.tsx", ["<PublicSiteSearch />"]); }
  ...
  if (enabled("public-content")) { requireText("app/page.tsx", ["Sept leviers numériques", "Demander une consultation", "Aucun cas client", "getDashboardUrl"]); forbidText("app/page.tsx", ["https://app.dtsc-platform.com/dashboard"]); }
  ...
  if (enabled("account")) { requireText("components/auth/account-product-shell.tsx", ["AccountProductShell", "ProductPreferencesControls"]); requireText("components/auth/auth-form.tsx", ["Mot de passe oublié", "confirmPassword", "legalConsent"]); requireText("prisma/account-consent.prisma", ["AccountConsentRecord", "documentVersion", "statementDigest", "acceptedAt"]); forbidText("app/auth/sign-in/page.tsx", ["AppShell", "ProfessionalToolbox"]); }
  if (enabled("account-recovery")) { requireText("prisma/account-recovery.prisma", ["PasswordResetToken", "tokenHash", "usedAt", "expiresAt"]); requireText("app/api/auth/reset-password/route.ts", ["hashPasswordResetToken", "RESET_TOKEN_ALREADY_USED", "clearSessionCookie"]); requireText("app/api/auth/forgot-password/route.ts", ["deliveryAccepted", "RESET_REQUEST_ACCEPTED"]); forbidText("app/api/auth/forgot-password/route.ts", ["console.log(token", "metadata: { token"]); }
  if (enabled("support")) { requireText("components/support/support-product-shell.tsx", ["SupportProductShell", "ProductNavigation"]); requireText("app/support/page.tsx", ["PAGE_SIZE", "SupportPagination", "SupportGuestEntry", "supportTicketVisibilityWhere(session) || {}"]); forbidText("app/support/page.tsx", ["<AppShell", "take: canManageTickets ? 200 : 100"]); }
  ...
  if (enabled("pwa")) { requireText("app/manifest.webmanifest/route.ts", ["Vary", 'product.code === "APP" ? "/dashboard"', 'product.code === "SUPPORT" ? "/support"']); requireText("app/layout.tsx", ["<PWARegister enabled={product.pwa !== \"disabled\"}"]); requireText("components/pwa/pwa-register.tsx", ["getRegistrations", "registration.unregister", "caches.keys"]); forbidText("app/manifest.webmanifest/route.ts", ['product.code === "PUBLIC" ? "/dashboard"']); }
  if (enabled("accessibility")) requireText("components/public/public-nav.tsx", ["aria-modal=\"true\"", "Escape", "aria-current", "xl:hidden", "createPortal", "100dvh", "overscrollBehavior", "public-mobile-navigation"]);
