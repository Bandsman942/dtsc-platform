...
const reranking = read("lib/ai/reranking.ts");
...
for (const classification of [
  ...
  expect(enterprise.includes(classification), `Missing RAG classification: ${classification}`);
}
...
expect(reranking.includes("RerankerDefinition") && reranking.includes("registerReranker"), "Optional reranking contract is missing");
expect(reranking.includes("using hybrid score"), "Reranker failure must preserve hybrid fallback");
expect(enterprise.includes("rerankKnowledgeCandidates"), "Enterprise hybrid candidates must pass through optional reranking");
expect(enterprise.includes("LIMIT 18"), "Enterprise retrieval must bound pre-rerank candidates");
