...
const certified = parseArray(process.env.AI_OPENROUTER_CERTIFIED_MODELS_JSON)
  .filter((model) => model && typeof model === "object" && model.providerCode === "OPENROUTER" && model.providerModelId);
...
try {
  ...
  for (const model of certified) {
    ...
    if (!remote) {
      failures.push(`${model.code}: provider model ${model.providerModelId} is missing from OpenRouter catalog`);
      continue;
    }
  ...
  console.log(`OpenRouter certified catalog audit passed for ${certified.length} model(s).`);
} catch (error) {
