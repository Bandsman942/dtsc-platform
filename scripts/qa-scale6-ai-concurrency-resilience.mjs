...
expect(all(health, ["prisma.aiProviderAttempt", '"UNAVAILABLE"', '"DEGRADED"', "RECENT_ATTEMPTS_ALL_FAILED", "RECENT_SUCCESS_RATE_LOW"]), "existing health registry remains the canonical circuit-breaker signal");
...
expect(all(observability, ["AiProviderAttempt", "activeAttempts", "throttledAttempts", "timeoutAttempts", "providerUnavailableAttempts", "getAiConcurrencyPolicySnapshot", "totalTokens", "estimatedCostUsd", "AI_AGENT_SERVER_LIMITS"]), "CTO snapshot exposes secret-free SCALE-6 capacity, usage and budget signals");
expect(all(dashboard, ["snapshot.ai.capacity.activeAttempts", "snapshot.ai.capacity.throttledAttempts", "snapshot.ai.capacity.timeoutAttempts", "snapshot.ai.capacity.providerUnavailableAttempts", "snapshot.ai.usage.totalTokens", "snapshot.ai.usage.estimatedCostUsd", "snapshot.ai.usage.fallbackCount", "snapshot.ai.budgets.agent.ENTERPRISE", "aiChatConcurrency", "aiAgentConcurrency", "aiEmbeddingConcurrency"]), "CTO UI renders SCALE-6 capacity, usage, budgets and guardrails");
expect(all(i18n, ["Tentatives IA actives", "Active AI attempts", "Concurrence Agent", "Agent concurrency", "Tokens consommés", "Tokens consumed", "Budget Agent · Entreprise", "Agent budget · Enterprise", "garde-fous de concurrence", "concurrency guardrails"]), "SCALE-6 UI copy is FR/EN");
