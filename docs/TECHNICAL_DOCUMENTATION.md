...
Les formulaires longs ou sensibles doivent utiliser un dialogue haut responsive (`h-[92dvh]` en pratique) avec contenu scrollable interne. Le composant `components/ui/form-field.tsx` centralise les étiquettes visibles et aides contextuelles (`label`, `hint`) pour guider la saisie sur desktop et mobile. Il est utilisé sur les formulaires ciblés de cette itération: activités professionnelles du module Entreprise, ticket support, identité professionnelle, création de compte, gestion des entreprises clientes et abonnements, Activités DTSC et Activités [Entreprise]. Cette évolution est strictement UI: les routes et payloads existants restent inchangés et continuent d'appliquer leurs validations/RBAC côté serveur.
...
Les modules historiques internes DTSC (`/admin`, `/activities`, `/calendar` et routes `/api/admin*`, `/api/activities*`, `/api/calendar*`) exigent désormais une session `DTSC_INTERNAL` avec `activeOrganizationId = dtsc-internal`. En contexte `ORGANIZATION`, les modules `Dashboard`, `Chatbot`, `Entreprise`, `Documents`, `Paramètres`, `Notifications`, `Support` et `Mes collaborateurs` restent visibles, mais leurs données sont filtrées par le contexte actif: conversations, messages, projets de conversations, usage chat, profil entreprise, activités entreprise, documents RAG, chunks, notifications et tickets support portent ou vérifient `organizationId`. Les seuls modules réellement communs entre espaces restent `Abonnement`, `Annonces` et `Profil`; `Mes collaborateurs` peut créer des groupes transversaux uniquement par invitation acceptée. Les groupes internes d'une entreprise cliente sont bloqués si l'organisation n'a pas d'abonnement actif, sans bascule implicite vers les données DTSC.
