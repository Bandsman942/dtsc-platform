...
- bannières promotionnelles administrables depuis la Console DTSC, ciblees par role et surface, actives par defaut a la creation, affichables dans Chatbot, IA Assistant Entreprise, Mes collaborateurs et Annonces, avec fermeture persistante par utilisateur;
...
Les dialogs partagés via `components/ui/dialog.tsx` sont optimisés mobile/PWA avec un contenu interne scrollable et des header/footer non noyés dans le scroll. Les dialogues marqués comme hauts via `h-[90dvh]`, `h-[92dvh]`, `h-[94dvh]` ou `h-[96dvh]` sont traités comme des panneaux plein écran: hauteur bornée au viewport, largeur utile quasi complète sur desktop comme mobile, scroll interne, en-tête fixe visible, bouton de fermeture accessible, fermeture par Échap et fermeture au clic sur le fond. Les zones conversationnelles privées privilégient le contenu utile:
...
La navigation des modules entreprise transmet aussi le champ `EnterpriseModule.icon`. `lib/enterprise/enterprise-module-icons.ts` résout ce nom vers une icône Lucide adaptée au contenu du module, puis applique un repli par code de module et catégorie lorsque l’icône configurée est absente ou inconnue.
...
- Documents médicaux dédiés: `HealthDocument`, `HealthDocumentVersion` et `HealthDocumentAccessLog`, migration `20260613233000_healthcare_medical_documents`, liens vers les référentiels Santé, confidentialité explicite et stockage privé Supabase sous `health/{organizationId}/{documentId}`.
- API Documents médicaux: `GET|POST /api/enterprise/[organizationId]/healthcare/documents`, `GET|PATCH .../documents/[documentId]`, `POST .../[documentId]/versions` et `GET .../[documentId]/download`. Les fichiers acceptés sont PDF, JPG, PNG, DOCX, XLSX et TXT, limités à 10 Mo; le téléchargement est privé, non mis en cache et journalisé.
...
- `PATCH /api/enterprise/[organizationId]/members/[memberId]` permet de modifier le rôle organisationnel, le statut et le poste assigné d'un collaborateur. `DELETE` applique un retrait logique (`status = REMOVED`, `removedAt`) sans supprimer le compte utilisateur. Les deux méthodes exigent origine valide, session, rate limit, `canManageEnterpriseAdministration()`, même `organizationId`, membre non retiré et journalisation.
- Le statut `INVITED` reste réservé au workflow d'invitation: il n'est pas sélectionnable dans le formulaire d'affectation, un membre actif/suspendu ne peut pas être remis en invitation, et une invitation ne peut pas être convertie en adhésion active ou suspendue par une requête directe. Seule l'acceptation volontaire via `/enterprise-invitations` active le membership.
...
- `OrganizationMember` porte maintenant `positionId`, `positionCode` et `positionTitle` pour conserver l'affectation métier effective du collaborateur sans migration destructive ni suppression des memberships existants.
...
- `lib/enterprise/enterprise-navigation.ts` produit la navigation latérale à partir des modules de l'organisation qui sont simultanément activés, autorisés par les entitlements et accessibles au poste du collaborateur via `canAccessEnterpriseModule()`.
- `lib/enterprise-sector-templates.ts` centralise les permissions de poste: `EnterprisePosition.permissionsJson` peut ouvrir l'administration entreprise (`enterprise.admin.manage` ou `enterprise.admin.members.manage`), les activités (`enterprise.activities.manage` ou `enterprise.activities.submit`) et les modules sectoriels via leurs préfixes de permissions (`health.*`, `pharmacy.*`, `enterprise.ai.*`). Les rôles propriétaire/admin restent prioritaires; un collaborateur avec poste actif est filtré par permissions.
...
- `EnterpriseMembersPanel` et `EnterprisePositionsPanel` utilisent des dialogues plein écran, des comboboxes pour rôles/statuts/postes/départements, des menus `...` pour les actions CRUD ou de cycle de vie, et affichent les permissions avec libellés lisibles au lieu de codes bruts avec underscores.
...
- Les chargements serveur sont isolés dans `lib/enterprise/*`: `enterprise-activities-loader.ts`, `enterprise-activity-blocks-loader.ts`, `enterprise-activity-requests-loader.ts`, `enterprise-activity-members-loader.ts`, `enterprise-activity-workflows-loader.ts` et `enterprise-activity-healthcare-loader.ts`. `enterprise-activity-blocks-loader.ts` filtre aussi les blocs par `canAccessEnterpriseActivity()` quand un `userId` est fourni.
...
| `POST` | `/api/admin/promotional-banners` | `ADMIN` | Creation d'une banniere promotionnelle active par defaut avec surfaces, roles inclus/exclus, CTA, priorite et dates optionnelles |
