...
- contrat d'identité d'approbation aligné sur `User.id` (`String @id`) afin de prendre en charge les identités stables existantes sans affaiblir les contrôles d'éligibilité, tenant ou séparation des rôles ;
...
- messages d'erreur métier FR/EN pour l'éligibilité de l'approbateur, l'affectation, l'auto-validation et les étapes d'approbation ;
- QA structurale #511 intégrée à la régression, avec garde des flux UI caisse/rapprochement, des statuts de filtrage, du contrat `String @id` et des messages d'erreur FR/EN ;
