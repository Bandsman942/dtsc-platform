...
export async function getConsoleSupportDataset(input: {
  ...
  const [tickets, total, openCount, urgentCount, overdueCount, resolvedRows, assignees] = await Promise.all([
    ...
    prisma.user.findMany({ where: { role: { in: ["ADMIN", "SUPPORT", "MANAGER"] }, status: "ACTIVE", memberships: { some: { organization: { organizationType: "DTSC_INTERNAL", deletedAt: null }, status: "ACTIVE", removedAt: null } } }, select: { id: true, name: true, email: true, role: true }, orderBy: { name: "asc" }, take: 100 }),
  ]);
