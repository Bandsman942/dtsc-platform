...
const fr: Record<RetailGuideCode, ContextualUserGuide> = {
  ...
  MOBILE_MONEY_AGENCY: {
    ...
    updatedAt: "2026-08-07",
    capabilities: ["Opérateurs configurables", "Dépôts et retraits", "Compte CASH distinct du float MOBILE_MONEY", "Frais client séparés", "Commission opérateur suivie séparément", "Référence fournisseur", "Annulation contrôlée", "Historique par opérateur et période"],
    steps: [
      { title: "Configurer un opérateur", description: "Chaque opérateur peut être lié à son propre compte de float.", actions: ["Créer ou choisir un compte financier MOBILE_MONEY", "Ouvrir la section Opérateurs", "Définir le code et le libellé", "Associer le compte de float"] },
  ...
  TELCO_TOPUPS: {
    ...
    updatedAt: "2026-08-07",
    capabilities: ["Opérateurs configurables", "Forfaits reliables au catalogue", "Prix de vente et coût opérateur", "Marge calculée côté serveur", "Encaissement séparé du float fournisseur", "Statut SUCCESS/FAILED", "Référence opérateur", "Annulation auditée"],
    steps: [
      ...
      { title: "Configurer le float opérateur", description: "Associez l’opérateur à un compte MOBILE_MONEY ou CLEARING dédié aux recharges.", actions: ["Configurer l’opérateur", "Choisir le compte de float Télécom", "Vérifier la devise"] },
...
const en: Record<RetailGuideCode, ContextualUserGuide> = {
  ...
  MOBILE_MONEY_AGENCY: { ...fr.MOBILE_MONEY_AGENCY, title: "Mobile Money Agency guide", summary: "Run deposits and withdrawals while keeping physical cash, operator float, fees and commissions separate.", audience: "Store manager, Mobile Money agent, cashier and controller", steps: [{ title: "Configure the provider", description: "Link each provider to its own MOBILE_MONEY float account." }, { title: "Open cash", description: "Any operation touching cash requires an open cashier session." }, { title: "Record a deposit", description: "Cash increases while provider float decreases by the principal." }, { title: "Record a withdrawal", description: "Cash decreases while provider float increases by the principal." }, { title: "Reverse carefully", description: "Only reverse in DTSC after the provider-side operation has also been reversed." }] },
  TELCO_TOPUPS: { ...fr.TELCO_TOPUPS, title: "Telco & top-ups guide", summary: "Sell airtime and internet bundles with operator cost, margin, tender and provider float tracking.", audience: "Store manager, sales manager, seller and telco agent", steps: [{ title: "Prepare the catalog", description: "Reusable bundles can be catalog services." }, { title: "Configure provider float", description: "Link the provider to a MOBILE_MONEY or CLEARING account." }, { title: "Record the top-up", description: "Margin is calculated server-side and provider float is debited only for successful transactions." }] },
