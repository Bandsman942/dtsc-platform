...
import {
  toCanonicalEnterpriseUserGuide,
  type CanonicalUserGuide,
  type LegacyEnterpriseUserGuide,
} from "@/lib/user-guides/canonical-guide";

export type EnterpriseGuideSource = FinanceUserGuide & {
  relatedModules?: SectorUserGuide["relatedModules"];
  limitations?: string[];
};

const COMMON_ENTERPRISE_USER_GUIDES: Record<string, EnterpriseGuideSource> = {
  CRM_CUSTOMERS: {
...
export function getCanonicalEnterpriseUserGuide(code: string, locale?: string | null): CanonicalUserGuide | null {
  const source = getEnterpriseGuideSource(code);
  if (!source) return null;
  return toCanonicalEnterpriseUserGuide(code.trim().toUpperCase(), source as LegacyEnterpriseUserGuide, locale);
}
