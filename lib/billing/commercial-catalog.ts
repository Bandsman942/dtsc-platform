...
export type PublishedBillingOffer = {
  ...
  /** Alias de compatibilité UI : la valeur vient toujours de BillingPlan.description. */
...
export function formatPublishedBillingCatalogForAi(catalog: PublishedBillingCatalog) {
  const lines = catalog.offers.map((offer) => {
    const common = `${offer.name}: ${offer.description} ${offer.priceUsd === 0 ? "Gratuit" : `${offer.priceUsd.toFixed(2)} USD/mois`}; ${offer.dailyMessageLimit.toLocaleString("fr-FR")} messages IA/jour; ${offer.dailyTokenLimit.toLocaleString("fr-FR")} tokens/jour; ${offer.maxKnowledgeSources.toLocaleString("fr-FR")} sources de connaissance IA.`;
  ...
  return [
    ...
    "Ce catalogue est l’autorité commerciale courante pour les noms d’offres, descriptions, prix et quotas. Les prix ci-dessous sont des prix d’abonnement DTSC Platform, pas des devis de prestation de conseil.",
    ...lines,
