...
function assertTimeZone(timeZone: string) {
  try {
    ...
    throw new EnterpriseCoreV2Error("Le fuseau horaire sélectionné n’est pas valide.", 400, "REPORT_SCHEDULE_TIMEZONE_INVALID");
  }
}
...
function plusLocalDays(parts: LocalParts, days: number) {
...
function nextRunAtFor(input: Pick<ScheduleCreateInput, "frequency" | "timeZone" | "hour" | "minute" | "dayOfWeek" | "dayOfMonth">, after = new Date()) {
  ...
  if (input.frequency === "DAILY") {
    ...
    if (candidate.getTime() <= after.getTime()) {
      date = plusLocalDays(current, 1);
      candidate = zonedDateToUtc({ ...date, hour: input.hour, minute: input.minute }, input.timeZone);
    }
    return candidate;
  }
  ...
  if (input.frequency === "WEEKLY") {
    ...
    if (candidate.getTime() <= after.getTime()) {
      delta += 7;
      date = plusLocalDays(current, delta);
      candidate = zonedDateToUtc({ ...date, hour: input.hour, minute: input.minute }, input.timeZone);
    }
    return candidate;
  }
  ...
  if (candidate.getTime() <= after.getTime()) {
    ...
    candidate = zonedDateToUtc({ year, month, day, hour: input.hour, minute: input.minute }, input.timeZone);
  }
  return candidate;
}
...
function dateOnlyShift(year: number, month: number, day: number, delta: number) {
...
function resolvePeriod(filters: ScheduleFilters, dueAt: Date, timeZone: string) {
  ...
  const due = localParts(dueAt, timeZone);
  const dueDate = dateString(due.year, due.month, due.day);
...
export async function createEnterpriseReportSchedule(organizationId: string, userId: string, input: ScheduleCreateInput) {
  assertTimeZone(input.timeZone);
  await validateScheduleSourceAccess(organizationId, userId, input);
  if (input.deliveryChannels.includes("EMAIL") && !isReportEmailDeliveryConfigured()) {
    throw new EnterpriseCoreV2Error("La livraison des rapports par e-mail n’est pas configurée. Utilisez l’archivage interne ou configurez la messagerie DTSC.", 409, "REPORT_EMAIL_DELIVERY_UNAVAILABLE");
  }
  const nextRunAt = nextRunAtFor(input);
  return prisma.enterpriseReportSchedule.create({
    data: {
      organizationId,
      ...
      deliveryChannelsJson: input.deliveryChannels as unknown as Prisma.InputJsonValue,
      recipientEmailsJson: input.recipientEmails as unknown as Prisma.InputJsonValue,
      nextRunAt,
    },
  });
}
...
export async function updateEnterpriseReportSchedule(organizationId: string, scheduleId: string, input: ScheduleUpdateInput) {
  ...
  const nextRunAt = input.action === "ENABLE"
    ? nextRunAtFor({ frequency: schedule.frequency as ScheduleCreateInput["frequency"], timeZone: schedule.timeZone, hour: schedule.hour, minute: schedule.minute, dayOfWeek: schedule.dayOfWeek, dayOfMonth: schedule.dayOfMonth })
    : schedule.nextRunAt;
  const updated = await prisma.enterpriseReportSchedule.updateMany({
    where: { id: scheduleId, organizationId, revision: input.revision, archivedAt: null },
    data: input.action === "ARCHIVE"
      ? { isEnabled: false, archivedAt: new Date(), revision: { increment: 1 } }
      : { isEnabled: nextEnabled, nextRunAt, revision: { increment: 1 } },
  });
...
export async function listEnterpriseReportSchedules(organizationId: string) {
  return prisma.enterpriseReportSchedule.findMany({
...
function parseScheduleFilters(value: Prisma.JsonValue | null): ScheduleFilters {
  ...
  return {
    periodMode: ["ALL_AVAILABLE", "CURRENT_MONTH", "PREVIOUS_MONTH", "LAST_7_DAYS", "LAST_30_DAYS", "CUSTOM"].includes(String(row.periodMode)) ? row.periodMode as ScheduleFilters["periodMode"] : "PREVIOUS_MONTH",
    periodStart: typeof row.periodStart === "string" ? row.periodStart : "",
    periodEnd: typeof row.periodEnd === "string" ? row.periodEnd : "",
    currency: typeof row.currency === "string" ? row.currency : "",
    departmentId: typeof row.departmentId === "string" ? row.departmentId : "",
    supplierId: typeof row.supplierId === "string" ? row.supplierId : "",
    budgetId: typeof row.budgetId === "string" ? row.budgetId : "",
...
function reportInputForSchedule(schedule: Awaited<ReturnType<typeof listEnterpriseReportSchedules>>[number], dueAt: Date) {
  const filters = parseScheduleFilters(schedule.filtersJson);
  const period = resolvePeriod(filters, dueAt, schedule.timeZone);
  return {
    reportType: schedule.reportType as ScheduleCreateInput["reportType"],
    title: schedule.reportTitle,
    description: schedule.reportDescription || "",
...
async function enqueueDueSchedule(schedule: Awaited<ReturnType<typeof listEnterpriseReportSchedules>>[number]) {
  ...
  const run = await prisma.enterpriseReportScheduleRun.create({
    data: { organizationId: schedule.organizationId, scheduleId: schedule.id, dueAt, status: "QUEUED", deliveryStatus: "ARCHIVE_PENDING" },
  }).catch((error) => {
...
async function deliverCompletedRun(run: Awaited<ReturnType<typeof prisma.enterpriseReportScheduleRun.findFirst>> & {}) {
  if (!run?.schedule || !run.generationEventId) return;
  const event = await prisma.enterpriseDomainEvent.findFirst({ where: { id: run.generationEventId, organizationId: run.organizationId }, select: { processingStatus: true, payloadJson: true, lastError: true } });
  if (!event) return;
  if (event.processingStatus === "DEAD") {
    await prisma.enterpriseReportScheduleRun.update({ where: { id: run.id }, data: { status: "FAILED", deliveryStatus: "FAILED", errorCode: "REPORT_GENERATION_FAILED", completedAt: new Date() } });
    return;
  }
  ...
  let deliveryStatus = "ARCHIVED";
  let runStatus = "COMPLETED";
  let errorCode: string | null = null;
  if (channels.includes("EMAIL")) {
    const recipients = asStringArray(run.schedule.recipientEmailsJson);
    if (!isReportEmailDeliveryConfigured() || recipients.length === 0) {
      deliveryStatus = "EMAIL_UNAVAILABLE";
      runStatus = "DELIVERY_FAILED";
      errorCode = "REPORT_EMAIL_DELIVERY_UNAVAILABLE";
    } else {
      const result = await sendZohoOutboundMail({
        to: recipients,
        subject: `${report.reference} · ${report.title}`,
        heading: "Rapport DTSC Platform",
        source: "REPORTS",
        deliveryMode: "direct",
        message: `Le rapport planifié « ${report.title} » (${report.reference}) a été généré et archivé dans le module Rapports de votre entreprise. Ouvrez DTSC Platform pour consulter le détail, les graphiques et les exports professionnels.`,
      }).catch(() => ({ sent: false }));
      if (result.sent) deliveryStatus = "ARCHIVED_EMAIL_SENT";
      else {
        deliveryStatus = "EMAIL_FAILED";
        runStatus = "DELIVERY_FAILED";
        errorCode = "REPORT_EMAIL_DELIVERY_FAILED";
      }
    }
  }
...
export async function processEnterpriseReportSchedules({ batchSize = 20 }: { batchSize?: number } = {}) {
  ...
  const due = await prisma.enterpriseReportSchedule.findMany({
    where: { isEnabled: true, archivedAt: null, nextRunAt: { lte: new Date() } },
    orderBy: { nextRunAt: "asc" },
    take: safeBatch,
    include: { runs: { orderBy: { createdAt: "desc" }, take: 1 } },
  });
  let enqueued = 0;
  for (const schedule of due) if (await enqueueDueSchedule(schedule)) enqueued += 1;

  const pending = await prisma.enterpriseReportScheduleRun.findMany({
    where: { status: "QUEUED", generationEventId: { not: null } },
