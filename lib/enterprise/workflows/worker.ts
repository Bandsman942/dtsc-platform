...
export async function processPendingWorkflowEvents({batchSize=WORKFLOW_LIMITS.workerBatchSize,workerId=`workflow-${randomUUID()}`}:{batchSize?:number;workerId?:string}={}){
  const safeBatchSize=Math.max(1,Math.min(batchSize,WORKFLOW_LIMITS.workerBatchSize)); const queueBefore=await getWorkflowQueueSnapshot(); const claimed=await claimPendingEvents(workerId,safeBatchSize); const results:Array<{id:string;status:string;error?:string;projectionFailures?:number}>=[];
  for(const event of claimed){try{const projectionResult=await processCrossModuleProjections(event.id);await processWorkflowDomainEvent(event.id);await prisma.enterpriseDomainEvent.updateMany({where:{id:event.id,processingStatus:"PROCESSING",lockedBy:workerId},data:{processingStatus:"PROCESSED",processedAt:new Date(),lockedAt:null,lockedBy:null,lastError:null}});results.push({id:event.id,status:"PROCESSED",projectionFailures:projectionResult.failures})}catch(error){const failure=safeWorkflowFailureMessage(error);const dead=event.attemptCount>=WORKFLOW_LIMITS.maxAttempts||["SECURITY","TERMINAL"].includes(failure.category);const backoffSeconds=Math.min(300,10*Math.max(1,event.attemptCount));await prisma.enterpriseDomainEvent.updateMany({where:{id:event.id,processingStatus:"PROCESSING",lockedBy:workerId},data:{processingStatus:dead?"DEAD":"FAILED",availableAt:dead?new Date():new Date(Date.now()+backoffSeconds*1000),lockedAt:null,lockedBy:null,lastError:`${failure.code}: ${failure.message}`.slice(0,1000)}});results.push({id:event.id,status:dead?"DEAD":"FAILED",error:failure.code})}}
  const pendingProjections=await processPendingCrossModuleProjections(safeBatchSize);const resumedRuns=await resumeWaitingRuns();const queueAfter=await getWorkflowQueueSnapshot();return{workerId,claimed:claimed.length,results,pendingProjections:{processed:pendingProjections.processed,failures:pendingProjections.failures},resumedRuns:resumedRuns.map(run=>({id:run.id,status:run.status})),queueBefore,queueAfter,saturated:claimed.length===safeBatchSize&&queueAfter.available&&(queueAfter.ready||0)>0};
}
