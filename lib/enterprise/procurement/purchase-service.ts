...
async function preparePurchaseItems(tx: ProcurementTransaction, organizationId: string, items: PurchaseItemInput[]) {
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    prepared.push({ organizationId, description: item.description, quantity: itemQuantity, unit: item.unit, unitPrice, taxRate, lineSubtotal, taxAmount: lineTax, lineTotal, sourceEntityType: nullable(item.sourceEntityType), sourceEntityId: nullable(item.sourceEntityId), sortOrder: index });
  }
...
export async function updateEnterprisePurchase(organizationId: string, purchaseId: string, actorUserId: string, input: PurchaseUpdateInput) {
  return prisma.$transaction(async (tx) => {
    ...
    if (calculated) { await tx.enterprisePurchaseItem.deleteMany({ where: { organizationId, purchaseId } }); await tx.enterprisePurchaseItem.createMany({ data: calculated.items.map((item) => ({ ...item, purchaseId })) }); }
...
export async function receiveEnterprisePurchase(organizationId: string, purchaseId: string, actorUserId: string, input: PurchaseReceiptInput) {
  return prisma.$transaction(async (tx) => {
    ...
    for (const inputItem of input.items) { const ordered = quantity(itemMap.get(inputItem.purchaseItemId)!.quantity); const previous = receivedMap.get(inputItem.purchaseItemId) || quantity(0); const next = previous.add(quantity(inputItem.quantityReceived)); if (next.gt(ordered)) throw new EnterpriseCoreV2Error("La quantité reçue ne peut pas dépasser la quantité commandée.", 409, "PURCHASE_OVER_RECEIPT"); receivedMap.set(inputItem.purchaseItemId, next); }
    ...
    return { receipt, purchase: await tx.enterprisePurchase.findUnique({ where: { id: purchaseId }, include: { items: true, supplier: true, budgetLine: { include: { budget: true } }, receipts: { include: { items: true }, orderBy: { receivedAt: "desc" } } } }) };
  });
}
