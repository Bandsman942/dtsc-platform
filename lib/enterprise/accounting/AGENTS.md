...
30. `finance-readiness-service.ts` est l’unique autorité de diagnostic de préparation Finance. Aucun workspace, route ou service ne recrée sa propre checklist de readiness.
31. `EnterpriseFinanceConfiguration.readinessStatus` est une projection reconstruisible de compatibilité ; aucune comptabilisation ne lui fait confiance comme source de vérité.
32. Toute décision d’accès Finance dérive de `resolveEnterpriseModuleAccess()` / `resolveEnterpriseModuleCapabilities()`. Aucun `Set` local de rôles, et notamment aucun `ENTERPRISE_MANAGER_ROLES`, ne peut accorder `canManage`, approbation ou données sensibles.
33. Une erreur de lecture Finance n’est jamais transformée en zéro, liste vide ou état sain. Les surfaces critiques distinguent explicitement donnée vide, donnée indisponible et état dégradé.
34. `posting-registry-final.ts` et `posting-service.ts` restent les autorités uniques des événements de posting. Ne jamais créer une matrice ou un registre parallèle ; tout nouvel événement doit étendre ces autorités et les gates `qa-erp-cross-module-finance.mjs` / `qa-erp-stabilization-final.mjs`.
