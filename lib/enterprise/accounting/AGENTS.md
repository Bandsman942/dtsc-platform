...
27. `OHADA_SYSCOHADA` ne peut jamais passer à `PUBLISHED` tant que `templates/syscohada/source-manifest.json` n’atteste pas un fichier source vérifié, un dataset canonique fingerprinté et un statut d’utilisation `AUTHORIZED_FOR_DTSC_IMPLEMENTATION`. Une notice officielle ne suffit pas à elle seule à autoriser la reproduction du dataset.
