...
export async function createOpeningBalanceImport(
  ...
  return prisma.$transaction(async (tx) => {
    const period = await tx.enterpriseFiscalPeriod.findFirst({
      where: { id: input.fiscalPeriodId, organizationId, status: "OPEN" },
    });
    if (!period) throw new EnterpriseAccountingError("OPENING_PERIOD_INVALID", 409);

    const accountIds = [...new Set(input.lines.map((line) => line.ledgerAccountId))];
    ...
    if (accounts.length !== accountIds.length) throw new EnterpriseAccountingError("OPENING_ACCOUNT_INVALID", 409);

    const partyIds = [...new Set(input.lines.map((line) => line.businessPartyId).filter(Boolean) as string[])];
    if (partyIds.length) {
      const parties = await tx.enterpriseBusinessParty.count({
        where: { organizationId, id: { in: partyIds }, status: "ACTIVE", archivedAt: null },
      });
      if (parties !== partyIds.length) throw new EnterpriseAccountingError("OPENING_BUSINESS_PARTY_INVALID", 409);
    }
    ...
    const prepared = input.lines.map((line) => {
      ...
      if (
        ...
        throw new EnterpriseAccountingError("OPENING_LINE_INVALID", 400);
      }
      if (line.currencyCode !== input.currencyCode) {
        throw new EnterpriseAccountingError("OPENING_BALANCE_LINE_CURRENCY_MISMATCH", 409);
      }
    ...
    if (!totalDebit.equals(totalCredit)) {
      throw new EnterpriseAccountingError("OPENING_BALANCE_UNBALANCED", 409, {
        totalDebit: totalDebit.toFixed(),
        totalCredit: totalCredit.toFixed(),
      });
    }

    const opening = await tx.enterpriseOpeningBalanceImport.create({
      data: {
        organizationId,
        reference: input.reference || financeReference("OPEN"),
    ...
    await publishFinanceEvent(tx, {
      ...
      summary: `Opening balance ${opening.reference} created`,
      actorUserId,
      toStatus: "DRAFT",
      metadataJson: { totalDebit: totalDebit.toFixed(), totalCredit: totalCredit.toFixed(), currency: input.currencyCode },
    });
    return { ...opening, totalDebit, totalCredit };
  }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
}

export async function approveAndPostOpeningBalance(
  organizationId: string,
  openingId: string,
  actorUserId: string,
  revision: number,
  ...
  const approved = await prisma.$transaction(async (tx) => {
    ...
    if (opening.status !== "DRAFT" || opening.revision !== revision) {
      throw new EnterpriseAccountingError("OPENING_BALANCE_CONFLICT", 409);
    }
    assertIndependentActor({
      actorUserId,
      relatedUserIds: [opening.preparedByUserId],
      errorCode: "OPENING_BALANCE_SELF_APPROVAL_FORBIDDEN",
    });
    return tx.enterpriseOpeningBalanceImport.update({
      where: { id: opening.id },
  ...
  return prisma.enterpriseOpeningBalanceImport.update({
