...
export async function classifyAndPostExpense(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (expense.status !== "APPROVED" || expense.accountedAt || expense.supplierInvoiceId || expense.revision !== input.revision) {
      throw new EnterpriseAccountingError("EXPENSE_NOT_POSTABLE", 409);
    }
    const classified = await tx.enterpriseExpense.update({
      where: { id: expense.id },
      data: {
    ...
    await publishFinanceEvent(tx, {
      ...
      entityId: expense.id,
      eventType: "EXPENSE_ACCOUNTING_CLASSIFIED",
      summary: `Expense ${expense.reference} classified`,
      actorUserId,
      metadataJson: { accountingTreatment: input.accountingTreatment },
    });
    const posting = await postBusinessEventTx(tx, organizationId, actorUserId, {
      postingEvent: "EXPENSE_APPROVED",
      sourceEntityType: "EnterpriseExpense",
      sourceEntityId: classified.id,
    });
    const updated = await tx.enterpriseExpense.update({
