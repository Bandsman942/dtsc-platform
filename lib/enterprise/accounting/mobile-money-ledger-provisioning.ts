...
export async function ensureMobileMoneyTransactionLedgerMappingTx(
  tx: Prisma.TransactionClient,
  organizationId: string,
  actorUserId: string,
  ...
  const transaction = await tx.enterpriseMobileMoneyTransaction.findFirst({
    ...
    select: { id: true, providerCode: true, currencyCode: true, floatAccountId: true, occurredAt: true },
  });
...
export async function ensureMobileMoneyTransactionLedgerMapping(organizationId: string, actorUserId: string, transactionId: string) {
  return prisma.$transaction(
    (tx) => ensureMobileMoneyTransactionLedgerMappingTx(tx, organizationId, actorUserId, transactionId),
    { isolationLevel: Prisma.TransactionIsolationLevel.Serializable, maxWait: 10000, timeout: 30000 },
  );
}

export async function ensureMobileMoneyFxLedgerMappingsTx(
  tx: Prisma.TransactionClient,
  organizationId: string,
  actorUserId: string,
  transferId: string,
...
export async function ensureMobileMoneyFxLedgerMappings(organizationId: string, actorUserId: string, transferId: string) {
  return prisma.$transaction(
    (tx) => ensureMobileMoneyFxLedgerMappingsTx(tx, organizationId, actorUserId, transferId),
    { isolationLevel: Prisma.TransactionIsolationLevel.Serializable, maxWait: 10000, timeout: 30000 },
  );
}
