...
function activePromotionWhere(organizationId: string, at: Date) {
  return {
    organizationId,
    status: "ACTIVE",
    archivedAt: null,
...
export async function buildRetailOfflineSnapshot(args: {
  ...
  const now = new Date();
  const [site, warehouse, totalCatalogItems, catalogItems, activePromotionCount] = await Promise.all([
    prisma.enterpriseSite.findFirst({ where: { id: args.siteId, organizationId: args.organizationId, status: "ACTIVE", archivedAt: null }, select: { id: true, code: true, name: true } }),
    prisma.enterpriseWarehouse.findFirst({ where: { id: args.warehouseId, organizationId: args.organizationId, siteId: args.siteId, status: "ACTIVE", archivedAt: null }, select: { id: true, code: true, name: true } }),
    prisma.enterpriseCatalogItem.count({ where: { organizationId: args.organizationId, status: "ACTIVE", archivedAt: null, OR: [{ currency: null }, { currency: args.currencyCode }] } }),
    prisma.enterpriseCatalogItem.findMany({
      ...
      orderBy: [{ code: "asc" }],
      take: args.maxItems,
      select: { id: true, code: true, sku: true, name: true, description: true, itemType: true, currency: true, indicativeSalePrice: true, taxCode: true, taxRate: true, taxable: true, trackInventory: true, updatedAt: true },
    }),
  ...
  const [prices, inventoryItems] = await Promise.all([
    itemIds.length
      ? prisma.enterpriseCatalogPrice.findMany({
          ...
          select: { catalogItemId: true, unitPrice: true, taxCode: true, taxRate: true, priceIncludesTax: true, effectiveFrom: true, effectiveUntil: true },
        })
      : Promise.resolve([]),
    itemIds.length
      ? prisma.enterpriseInventoryItem.findMany({ where: { organizationId: args.organizationId, catalogItemId: { in: itemIds }, status: "ACTIVE", archivedAt: null }, select: { id: true, catalogItemId: true } })
      : Promise.resolve([]),
  ]);
  ...
  const [balances, reservations] = await Promise.all([
    inventoryItemIds.length
      ? prisma.enterpriseInventoryBalance.findMany({ where: { organizationId: args.organizationId, warehouseId: args.warehouseId, inventoryItemId: { in: inventoryItemIds } }, select: { inventoryItemId: true, quantityOnHand: true, quantityReserved: true } })
      : Promise.resolve([]),
    inventoryItemIds.length
      ? prisma.enterpriseInventoryReservation.findMany({ where: { organizationId: args.organizationId, warehouseId: args.warehouseId, inventoryItemId: { in: inventoryItemIds }, status: "ACTIVE", OR: [{ expiresAt: null }, { expiresAt: { gt: now } }] }, select: { inventoryItemId: true, quantity: true, fulfilledQuantity: true } })
      : Promise.resolve([]),
  ]);

  const firstPriceByItem = new Map<string, (typeof prices)[number]>();
  for (const price of prices) if (!firstPriceByItem.has(price.catalogItemId)) firstPriceByItem.set(price.catalogItemId, price);
  ...
  const items = catalogItems.map((item) => {
    const price = firstPriceByItem.get(item.id);
    const inventoryItemId = inventoryByCatalog.get(item.id) || null;
    const available = inventoryItemId
      ? Prisma.Decimal.max(decimal(), (onHandByInventory.get(inventoryItemId) || decimal()).minus(legacyReservedByInventory.get(inventoryItemId) || 0).minus(activeReservedByInventory.get(inventoryItemId) || 0))
      : decimal();
    const unitPrice = price?.unitPrice ?? item.indicativeSalePrice ?? null;
    return {
      ...
      currencyCode: args.currencyCode,
      unitPrice: unitPrice?.toFixed() ?? null,
      taxCode: price?.taxCode ?? item.taxCode,
      taxRate: (price?.taxRate ?? item.taxRate)?.toFixed() ?? null,
      priceIncludesTax: price?.priceIncludesTax ?? false,
      trackInventory: item.trackInventory,
      quantityAvailable: available.toFixed(),
      updatedAt: item.updatedAt.toISOString(),
      offlineEligible: Boolean(unitPrice) && (!item.trackInventory || Boolean(inventoryItemId)),
    };
  });
  ...
  const snapshotPayload = {
    ...
    policy: {
      saleEnabled: activePromotionCount === 0,
      blockingReason: activePromotionCount > 0 ? "ACTIVE_PROMOTIONS_REQUIRE_ONLINE" : null,
  ...
  await prisma.enterpriseRetailOfflineSnapshot.upsert({
    ...
    update: { validUntil },
    create: { organizationId: args.organizationId, version, siteId: args.siteId, warehouseId: args.warehouseId, currencyCode: args.currencyCode, payloadHash, itemCount: items.length, validFrom: now, validUntil, createdByUserId: args.actorUserId },
  });
  return { ...snapshotPayload, version, payloadHash };
}

async function updateSyncConflict(organizationId: string, operationUuid: string, status: "CONFLICT" | "REJECTED", conflictCode: string, conflictJson?: Prisma.InputJsonValue) {
...
export async function syncRetailOfflineSale(args: {
  ...
  if (existing) {
    ...
    await prisma.enterpriseRetailOfflineSyncOperation.create({
      data: { organizationId: args.organizationId, operationUuid: args.operationUuid, operationType: "SALE", siteId: args.siteId, warehouseId: args.warehouseId, snapshotVersion: args.snapshotVersion, payloadHash, receivedByUserId: args.actorUserId },
    });
  }

  const snapshot = await prisma.enterpriseRetailOfflineSnapshot.findFirst({ where: { organizationId: args.organizationId, version: args.snapshotVersion, siteId: args.siteId, warehouseId: args.warehouseId } });
  if (!snapshot) {
  ...
  if (!capturedAt || Number.isNaN(capturedAt.getTime()) || capturedAt < snapshot.validFrom || capturedAt > snapshot.validUntil) {
    const operation = await updateSyncConflict(args.organizationId, args.operationUuid, "CONFLICT", "OFFLINE_SNAPSHOT_EXPIRED", { validFrom: snapshot.validFrom.toISOString(), validUntil: snapshot.validUntil.toISOString() });
    return { operation, idempotent: false };
  }
  ...
  try {
    const preview = await previewRetailCommercialPricing(
      args.organizationId,
      { siteId: args.siteId, customerBusinessPartyId: null, currencyCode: parsed.data.currencyCode, soldAt: parsed.data.soldAt, lines: parsed.data.lines.map((line) => ({ catalogItemId: line.catalogItemId, quantity: line.quantity })) },
    ...
    const changedItems = parsed.data.lines.filter((line) => {
      const current = currentByItem.get(line.catalogItemId);
      if (!current) return true;
      return !decimal(line.unitPrice).equals(current.resolvedUnitPrice) || !decimal(line.discountAmount || 0).equals(current.discountAmount) || !decimal(line.taxAmount || 0).equals(current.taxAmount);
    ...
    const { result, accounting, loyalty, promotionCount } = await executeCanonicalRetailSale({ organizationId: args.organizationId, actorUserId: args.actorUserId, input: parsed.data, commercialContext: { ...context.data, couponCode: null, customerSegmentCode: null, channelCode: "POS", overrideReason: null }, permissions });
    const operation = await prisma.enterpriseRetailOfflineSyncOperation.update({
      ...
      data: { status: "SYNCED", serverEntityType: "EnterpriseRetailSale", serverEntityId: result.sale.id, conflictCode: null, conflictJson: Prisma.JsonNull, syncedAt: new Date() },
    });
