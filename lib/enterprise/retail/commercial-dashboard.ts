...
export async function getCommercialRetailDashboard(organizationId: string, userId: string, from?: Date, to?: Date) {
  ...
  const readiness = [
    { code: "PROFILE", label: "Profil Shop Retail actif", complete: configuration?.status === "ACTIVE" && configuration.profileCode === "RETAIL_TELCO_MOBILE_MONEY", deepLink: "/enterprise-admin?section=templates" },
