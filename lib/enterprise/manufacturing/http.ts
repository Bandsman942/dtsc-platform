...
export async function authorizeManufacturingRequest({
  request,
  organizationId,
  moduleCode,
  action = "read",
  ...
  if (mutate) {
    const limited = await rateLimit(
      getRateLimitKey(request, `enterprise-manufacturing:${organizationId}:${session.userId}`),
      180,
      60 * 60 * 1000,
    );
    if (!limited.ok) return { ok: false as const, response: NextResponse.json({ error: "Too many requests" }, { status: 429 }) };
  }
  const access = await getManufacturingAccess({ session, organizationId, moduleCode, action });
...
export function manufacturingErrorResponse(error: unknown, fallbackCode = "MANUFACTURING_OPERATION_FAILED") {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
    return NextResponse.json({ error: "MANUFACTURING_DUPLICATE", message: "Une donnée de production portant la même référence existe déjà." }, { status: 409 });
  }
  ...
  console.error("[manufacturing] unexpected operation failure", { fallbackCode, errorName, prismaCode });
  return NextResponse.json({
    error: fallbackCode,
    message: "Le service de production n’a pas pu terminer cette opération. Actualisez les données puis réessayez.",
  }, { status: 500 });
}
