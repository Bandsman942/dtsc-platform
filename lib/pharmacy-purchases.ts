...
export async function createPurchaseEntity(organizationId: string, userId: string, data: PurchaseInput) {
  ...
  const lines = data.lines.map((line) => { const price = Number(line.estimatedUnitPrice || 0); const discount = Number(line.discountRate || 0); return { ...line, totalLine: line.orderedQuantity * price * (1 - discount / 100) }; });
  const estimatedTotal = lines.reduce((sum, line) => sum + line.totalLine, 0);
  return prisma.$transaction(async (transaction) => {
    const order = await transaction.pharmacyPurchaseOrder.create({ data: { organizationId, orderNumber: data.orderNumber || `CMD-${new Date().getFullYear()}-${String(count + 1).padStart(5, "0")}`, supplierId: data.supplierId, requestId: nil(data.requestId), requestedById: data.requestedById, departmentId: nil(data.departmentId), priority: data.priority, orderDate: data.orderDate, expectedDeliveryDate: nil(data.expectedDeliveryDate), deliveryAddress: nil(data.deliveryAddress), deliveryMode: nil(data.deliveryMode), deliveryContact: nil(data.deliveryContact), budgetId: nil(data.budgetId), estimatedTotal, currency: data.currency, financialValidationRequired: data.financialValidationRequired, notes: nil(data.notes), createdById: userId, updatedById: userId } });
...
export async function createReceiptFromPurchaseOrder(organizationId: string, purchaseOrderId: string, userId: string) {
  return prisma.$transaction(async (transaction) => {
    ...
    const receipt = await transaction.pharmacyReceipt.create({ data: { organizationId, receiptNumber: `REC-${new Date().getFullYear()}-${String(count + 1).padStart(5, "0")}`, receiptType: order.status === "PARTIALLY_RECEIVED" ? "PARTIAL" : "PURCHASE_ORDER", supplierId: order.supplierId, purchaseOrderId: order.id, departmentId: order.departmentId, receivedById: userId, receivedAt: new Date(), totalItems: 0, totalAmount: 0, status: "DRAFT", notes: `Réception préparée depuis ${order.orderNumber}`, createdById: userId, updatedById: userId } });
...
export async function getPurchasesDataset(organizationId: string) {
  ...
  return { metrics, suppliers, supplierProducts, requests, orders, products, members: members.map((item) => item.user), departments, receipts, documents, alerts };
}
