...
export const pharmacySaleSchema = z.object({
  ...
  globalDiscount: z.coerce.number().min(0).max(100).default(0), taxAmount: z.coerce.number().min(0).default(0), paidAmount: z.coerce.number().min(0).max(0).default(0),
  ...
  lines: z.array(z.object({ productId: z.string().min(1), batchId: z.string().min(1), quantity: z.coerce.number().gt(0), unit: z.string().min(1).max(80), unitPrice: z.coerce.number().min(0), lineDiscount: z.coerce.number().min(0).max(100).default(0), notes: optionalText })).min(1).max(100),
});
