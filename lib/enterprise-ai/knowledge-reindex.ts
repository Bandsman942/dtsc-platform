...
import { indexPreparedEnterpriseAiKnowledgeSource, resolveKnowledgeDataClassification } from "@/lib/enterprise-ai/knowledge";
import { RAG_CHUNKING_VERSION } from "@/lib/rag";
import { prisma } from "@/lib/prisma";

export async function reindexEnterpriseAiKnowledgeSource({ sourceId, organizationId }: { sourceId: string; organizationId: string }) {
  const source = await prisma.enterpriseAiKnowledgeSource.findFirst({
    where: { id: sourceId, organizationId, archivedAt: null },
    select: { id: true, sectorCode: true, moduleCode: true, confidentiality: true, extractedText: true },
  });
  ...
  await prisma.$executeRawUnsafe(
    ...
    sourceId, organizationId, dataClassification, definition.providerCode, definition.modelCode,
    definition.dimension, indexVersion, RAG_CHUNKING_VERSION,
  );
