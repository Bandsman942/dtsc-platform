...
export function resolveKnowledgeDataClassification({ confidentiality, sectorCode, moduleCode }: { confidentiality: string; sectorCode?: string | null; moduleCode?: string | null }): AiDataClassification {
...
async function indexEnterpriseChunks({ organizationId, sourceId, sectorCode, moduleCode, language, sourceVersion, dataClassification, chunks }: {
  organizationId: string; sourceId: string; sectorCode?: string | null; moduleCode?: string | null; language: string; sourceVersion: number;
  dataClassification: AiDataClassification; chunks: ReturnType<typeof chunkKnowledgeText>;
}) {
  const batchSize = 48;
  for (let offset = 0; offset < chunks.length; offset += batchSize) {
    ...
    for (let index = 0; index < batch.length; index += 1) {
      ...
      await prisma.$executeRawUnsafe(
        ...
        embedded.definition.dimension, indexVersion, RAG_CHUNKING_VERSION, sourceVersion, chunk.contentHash,
      );
    }
  }
}

export async function prepareEnterpriseAiKnowledgeSource({ organizationId, assistantId, userId, sectorCode, moduleCode, title, confidentiality, language = "fr", file }: {
  organizationId: string; assistantId: string; userId: string; sectorCode?: string | null; moduleCode?: string | null; title?: string | null;
  confidentiality: string; language?: string; file: File;
}) {
  ...
  const source = await prisma.enterpriseAiKnowledgeSource.create({
    data: {
      organizationId, assistantId, sectorCode: normalizeOptional(sectorCode), moduleCode: normalizeOptional(moduleCode),
      title: normalizeOptional(title) || file.name, sourceType: "DOCUMENT", status: "PROCESSING", confidentiality,
      language: language === "en" ? "en" : "fr", fileName: file.name, mimeType: file.type || "application/octet-stream", sizeBytes: file.size, createdById: userId,
    },
  });
  ...
  try {
    ...
    await prisma.enterpriseAiKnowledgeSource.update({
      where: { id: source.id },
      data: { extractedText, storageBucket: storage?.bucket || null, storagePath: storage?.path || null, errorMessage: null },
    });
    await prisma.$executeRawUnsafe(
    ...
    return { id: source.id, title: source.title, status: "PROCESSING" as const, dataClassification, indexVersion };
  } catch (error) {
    await prisma.enterpriseAiKnowledgeSource.update({ where: { id: source.id }, data: { status: "FAILED", errorMessage: error instanceof Error ? error.message : "Enterprise AI knowledge preparation failed" } });
    throw error;
  }
}
...
async function readEnterpriseIndexMetadata(sourceId: string, organizationId: string) {
  const rows = await prisma.$queryRawUnsafe<EnterpriseIndexMetadata[]>(
    `SELECT "id","organizationId","sectorCode","moduleCode","confidentiality","language","sourceVersion","dataClassification","extractedText"
     FROM "EnterpriseAiKnowledgeSource" WHERE "id"=$1 AND "organizationId"=$2 LIMIT 1`, sourceId, organizationId,
  );
  return rows[0] || null;
}

export async function indexPreparedEnterpriseAiKnowledgeSource({ sourceId, organizationId }: { sourceId: string; organizationId: string }) {
  ...
  try {
    await prisma.enterpriseAiKnowledgeSource.update({ where: { id: sourceId }, data: { status: "PROCESSING", errorMessage: null } });
    const chunks = chunkKnowledgeText(source.extractedText);
    await indexEnterpriseChunks({
      organizationId, sourceId, sectorCode: source.sectorCode, moduleCode: source.moduleCode, language: source.language,
      sourceVersion: source.sourceVersion, dataClassification: source.dataClassification, chunks,
    });
    await prisma.$executeRawUnsafe(`UPDATE "EnterpriseAiKnowledgeSource" SET "indexedAt"=$3 WHERE "id"=$1 AND "organizationId"=$2`, sourceId, organizationId, new Date());
    return prisma.enterpriseAiKnowledgeSource.update({ where: { id: sourceId }, data: { status: "READY", errorMessage: null }, include: { _count: { select: { chunks: true } } } });
  } catch (error) {
    await prisma.enterpriseAiKnowledgeSource.update({ where: { id: sourceId }, data: { status: "FAILED", errorMessage: error instanceof Error ? error.message : "Enterprise AI knowledge indexing failed" } });
    throw error;
  }
}

export async function indexEnterpriseAiKnowledgeSource(input: Parameters<typeof prepareEnterpriseAiKnowledgeSource>[0]) {
  const prepared = await prepareEnterpriseAiKnowledgeSource(input);
  return indexPreparedEnterpriseAiKnowledgeSource({ sourceId: prepared.id, organizationId: input.organizationId });
}

export async function retrieveEnterpriseAiKnowledge({ organizationId, question, sectorCode, moduleCode, canReadSensitive, queryLocale }: {
  organizationId: string; question: string; sectorCode?: string | null; moduleCode?: string | null; canReadSensitive: boolean; queryLocale?: string | null;
}) {
  const allowedConfidentialities = canReadSensitive ? ["PUBLIC", "INTERNAL", "CONFIDENTIAL", "MANAGERS_ONLY"] : ["PUBLIC", "INTERNAL"];
  const readySources = await prisma.enterpriseAiKnowledgeSource.count({ where: { organizationId, status: "READY", archivedAt: null, confidentiality: { in: allowedConfidentialities } } });
  ...
  const rows = await prisma.$queryRawUnsafe<EnterpriseAiKnowledgeCitation[]>(
    `SELECT kc."sourceId", ks."title", ks."confidentiality", kc."dataClassification", kc."sourceVersion", kc."indexVersion",
     ...
     FROM "EnterpriseAiKnowledgeChunk" kc INNER JOIN "EnterpriseAiKnowledgeSource" ks ON ks."id" = kc."sourceId"
     WHERE kc."organizationId" = $2 AND ks."organizationId" = $2 AND ks."status" = 'READY' AND ks."archivedAt" IS NULL
       ...
       AND kc."indexVersion" = ks."indexVersion" AND (kc."indexVersion" = $7 OR kc."indexVersion" = 'legacy-openai-1536-v1')
       AND ((kc."embedding" <=> $1::vector) <= 0.55 OR to_tsvector('simple', kc."content") @@ plainto_tsquery('simple', $6))
     ORDER BY "hybridScore" DESC, distance ASC LIMIT 6`,
  ...
  return { context, citations: rows, queryLocale: queryLocale || null, dataClassifications: Array.from(new Set(rows.map((row) => row.dataClassification))) };
}
