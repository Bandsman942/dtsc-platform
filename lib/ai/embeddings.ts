...
async function createOpenAiEmbeddings(inputs: string[]): Promise<EmbeddingBatchResult> {
  ...
  const rows = Array.isArray(payload?.data) ? [...payload.data].sort((left, right) => Number(left?.index ?? 0) - Number(right?.index ?? 0)) : [];
...
export async function createEmbedding(input: string) {
  ...
  return { definition: result.definition, embedding: result.embeddings[0] };
}
