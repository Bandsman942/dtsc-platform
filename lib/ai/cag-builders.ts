...
export function registerBuiltInCagBuilders() {
  ...
  registerSectorCagBuilder("PHARMACY", {
    code: "sector:PHARMACY",
    version: async (context) => {
      if (!context.organization) return "2.0";
      const setting = await prisma.pharmacySetting.findUnique({
        where: { organizationId: context.organization.id },
        select: { settingsVersion: true },
      });
      return `2.${setting?.settingsVersion ?? 0}`;
    },
    build: async (context) => {
      if (!context.organization) return "";
      const settings = await getEffectivePharmacySettings(context.organization.id, context.userId);
      const safeSettings = {
  ...
  registerSectorCagBuilder("HEALTH_CARE", {
  ...
  registerSectorCagBuilder("COMMERCE_RETAIL", {
