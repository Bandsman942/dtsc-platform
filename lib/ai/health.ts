...
export async function getAiRuntimeHealth({
  ...
  const [attemptsResult, latencyResult] = await Promise.all([
    prisma.aiProviderAttempt
      .findMany({
        where: {
  ...
  const latencyValues = latencyResult.value
  ...
  if (provider.status === "DEGRADED" || model.status === "DEGRADED") {
    return {
      ...
      reason: "CONFIG_DEGRADED",
    };
  }

  // Health telemetry is a ranking signal, never an authorization source. If the
  // observability read is temporarily unavailable, stay fail-open for availability
  // while AI00 policy, plan and provider configuration remain fully eliminatory.
  if (!attemptsResult.ok) {
    return {
      status: "HEALTHY",
      sampleSize: 0,
      successRate: null,
      averageFirstTokenLatencyMs: latencyResult.ok ? averageFirstTokenLatencyMs : null,
  ...
  if (!attempts.length) {
    return {
      ...
      successRate: null,
      averageFirstTokenLatencyMs,
      reason: "NO_RECENT_FAILURE_EVIDENCE",
    };
  }
  ...
  if (attempts.length >= 3 && successes === 0) {
    return {
      ...
      successRate,
      averageFirstTokenLatencyMs,
      reason: "RECENT_ATTEMPTS_ALL_FAILED",
    };
  }
  ...
  return {
    ...
    successRate,
    averageFirstTokenLatencyMs,
    reason: "RECENT_SUCCESS_RATE_ACCEPTABLE",
  };
}